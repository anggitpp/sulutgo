...
$infoSetting = getField("SELECT CONCAT(t1.namaSetting, '~', t1.pelaksanaanMulai, '~', t1.pelaksanaanSelesai, '~', t2.kodePrespektif) FROM pen_setting_penilaian t1 JOIN pen_setting_kode t2 ON t2.idKode = t1.idKode WHERE t1.idSetting = '$par[idSetting]'");
list($namaSetting, $pelaksanaanMulai, $pelaksanaanSelesai, $kodePrespektif) = explode("~", $infoSetting);

$arrTipe = arrayQuery("SELECT CONCAT(kodeTipe, 'tabsplit\t', namaTipe) FROM pen_tipe WHERE statusTipe = 't' ORDER BY urutanTipe");
$arrPrespektif = arrayQuery("SELECT CONCAT(idPrespektif, 'tabsplit\t', namaPrespektif) FROM pen_setting_prespektif WHERE kodePrespektif = '$kodePrespektif'"); 
?>
...
function generateFirstChart(){
	global $par, $s, $arrTitle, $arrTipe, $arrPrespektif;
	...
	foreach($arrPrespektif as $objPrespektif){
		list($idPrespektif, $namaPrespektif) = explode("tabsplit\t", $objPrespektif);
		$tmplChart .= "<dataset seriesname=\"$namaPrespektif\">";
		foreach($arrTipe as $objTipe){
			list($kodeTipe, $namaTipe) = explode("tabsplit\t", $objTipe);
			$tempNilai = getField("SELECT IFNULL(AVG(t1.bobotPilihan), 0.00) FROM pen_penilaian_detail t1 JOIN pen_penilaian t2 ON t2.idPenilaian = t1.idPenilaian JOIN pen_pegawai t3 ON t3.idPegawai = t2.idPegawai JOIN pen_sasaran t4 ON t4.idSasaran = t1.idSasaran WHERE t2.idSetting = '$par[idSetting]'AND t3.tipePenilaian = '$kodeTipe' AND t4.idPrespektif = '$idPrespektif'");
...
/* End of file prespektif_view.php */
/* Location: .//var/www/html/bdp-outsourcing/intranet/sources/penilaian/laporan/prespektif/prespektif_view.php */
