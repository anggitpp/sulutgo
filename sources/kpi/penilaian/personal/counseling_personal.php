...
function lData(){
  ...
  $kodeCounseling = getField("SELECT kodeData FROM mst_data WHERE kodeMaster = 'ECO'");
  $sWhere = " WHERE a.id is not null and e.idPegawai ='$cIdPegawai'  and kategori = '$kodeCounseling'";
 ...
 join app_user as e on(a.atasan = e.idPegawai OR a.peserta = e.idPegawai)
 $sWhere order by $orderBy $sLimit";

 $res=db($sql);
 $json = array(
   "iTotalRecords" => mysql_num_rows($res),
   "iTotalDisplayRecords" => getField("select count(id) from pen_cmc where kategori = $kodeCounseling"),
   "aaData" => array(),
   );
      ...
      <div style=\"position:absolute; right:20px; top:14px;\">
        ...
        <input type=\"button\" class=\"cancel radius2\" style=\"float:right\" value=\"Batal\" onclick=\"window.location='index.php?".getPar($par,"mode")."';\"/>
			      ...
			      <label class=\"l-input-small\">Peserta</label>
				      <div class=\"field\">
				     	 ".comboData("SELECT b.idPegawai, a.name,a.id,b.atasan_langsung from emp as a 
							...
							order by a.name asc","id","name","inp[peserta]","Piih Peserta","$r[peserta]","","250px","chosen-select","")."
				      </div>
		      </p>
		      ...
		      <p>
			      <label class=\"l-input-small\">Kategori</label>
				      <div class=\"field\">
				     	".comboData("SELECT kodeData, namaData from mst_data WHERE kodeInduk='989' AND kodeCategory ='CMC2' ORDER BY namaData ASC","kodeData","namaData","inp[id_kategori]","Pilih Kategori","$r[id_kategori]","","210px","chosen-select","")."
				      </div>
		      </p>
		      <p>
		      ...
		      <p>
			      ...
			      Will <br>
			      <font size='1'>(Tindakan keputusan yang diambil)</font></label>
				      <div class=\"field\">
