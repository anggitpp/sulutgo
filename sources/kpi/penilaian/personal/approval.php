...
function getContent($par)
{
	...
	{   
        ...
        case "viewData":
            $text = empty($_submit) ? viewData() : simpanAprroval(); 
		break;
...
function simpanAprroval()
{
    global $s,$inp,$par,$arrTitle,$arrParameter,$menuAccess,$dirKtp,$dirUsaha,$cID;
    if($inp[approve] == "Approve") $status = "t";
    if($inp[approve] == "Reject") $status = "f";
    
    db("delete from pen_realisasi_individu_approval where id_pegawai = $par[idPegawai] and kode_penilaian = $par[kodePenilaian] and id_tahun = $par[tahunPenilaian] and id_bulan = $par[bulanPenilaian]");
    
    $id = getLastId('pen_realisasi_individu_approval', 'id_approval');
    db("insert into pen_realisasi_individu_approval (
                                                    id_approval, 
                                                    id_pegawai, 
                                                    kode_penilaian, 
                                                    id_tahun, 
            ...
            <form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
                        ...
                        <tr>
                            ...
                            <td style=\"width: 50%;\">
                                <p>
                        			...
                        			<span class=\"field\">".getField("select namaData from mst_data where kodeData = $par[bulanPenilaian]")."</span>
                        		</p>
                            </td>
                        <tr>
                    </table>
                ...
                <div class=\"widgetbox\" style=\"position: relative;\">
                    ...
                    <div style=\"float:right; margin-top:-20px;\">
                        <input type=\"submit\" class=\"submit radius2\" onclick=\"return confirm('Ada yakin ingin approve?')\" name=\"inp[approve]\" value=\"Approve\" style=\"background-color:#46ab46 !important;\"/>
        	            <input type=\"submit\" class=\"submit radius2\" onclick=\"return confirm('Ada yakin ingin reject?')\" name=\"inp[approve]\" value=\"Reject\" style=\"background-color:#ff0000 !important;\" />
                    </div>
                </div>
                <hr>
...
function lihat()
{
	global $s,$inp,$par,$arrTitle,$menuAccess,$cID,$cTipeAkses,$cKodeAkses, $getBulan;
    		...
    		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\" >
    			<thead>
    				<tr>
                        ...
                        <th style=\"vertical-align:middle; min-width:100px;\" rowspan=\"2\">foto</th>
...
function lData()
{
	global $s,$par,$fFile,$menuAccess,$cUsername,$sUser,$sGroup,$arrTitle,$arrParam,$m,$cID,$cTipeAkses,$cKodeAkses, $getBulan, $cIdPegawai;
	...
	$sWhere = " WHERE b.tipePenilaian != 0 AND b.kodePenilaian IS NOT NULL and a.id in (select idPegawai from pen_pegawai where atasan_langsung = $cIdPegawai or atasan_dari_atasan = $cIdPegawai)";
	...
	$json = array(
		...
		"iTotalDisplayRecords" => getField("select count(a.id) from dta_pegawai AS a
                                            JOIN pen_pegawai AS b ON (b.idPegawai = a.id)
                                	        $sWhere "),
		"aaData" => array()
		);
	...
	while($r = mysql_fetch_array($res)){
        ...
        foreach($getBulan as $bln)
        {
            ...
            if(!empty($hasil))
            {
                $cek = getField("select status from pen_realisasi_individu_approval where id_pegawai = $r[idPegawai] and kode_penilaian = $r[kodePenilaian] and id_tahun = $par[idPeriode] and id_bulan = $bln[kodeData]");
                ...
                $data2[$no_arr] = "<div align=\"center\" style=\"background-color:".$warna.";\"><a href=\"#\" onclick=\"openBox('popup.php?par[mode]=viewData&par[idPenPegawai]=$r[id]&par[idPegawai]=$r[idPegawai]&par[tipePenilaian]=$r[tipePenilaian]&par[kodePenilaian]=$r[kodePenilaian]&par[tahunPenilaian]=$par[idPeriode]&par[bulanPenilaian]=$bln[kodeData]".getPar($par,"mode")."', 1000, 500 )\"><strong>$tombol<strong></a></div>";
            }
            else
            {
...
function detail()
{
    ...
    $par[bulanPenilaian] = empty($par[bulanPenilaian]) ? getField("SELECT kodeData FROM mst_data WHERE kodeCategory = 'PRDB' AND kodeInduk = $par[idPeriode] limit 1") : $par[bulanPenilaian];
