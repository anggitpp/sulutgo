...
function lihat(){
	...
	function lData(){
		...
		$sWhere = " WHERE a.id is not null and a.kategori = $kodeCouching AND d.idPegawai ='$cIdPegawai'";
		...
		$sql="select a.id, a.tanggal,a.judul as bahasan, b.name as peserta, c.name as atasan,d.idPegawai
		from pen_cmc as a
		...
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					...
					<div class=\"field\">";	
					    if($par[mode]=="add"){
					    	$text.="".comboData("SELECT b.idPegawai, a.name,a.id,b.atasan_langsung from emp as a 
							join pen_pegawai as b on a.id = b.idPegawai 
							WHERE  b.atasan_langsung = '$cIdPegawai' 
							order by a.name asc","id","name","inp[peserta]","","$r[peserta]","","250px","chosen-select","")."
								";
					    }else{
						    	$text.="".comboData("SELECT b.idPegawai, a.name,a.id,b.atasan_langsung from emp as a 
							join pen_pegawai as b on a.id = b.idPegawai 
			...
			<fieldset>
				...
				<p>
					<label class=\"l-input-small\">Kategori</label>
					<div class=\"field\">
						".comboData("SELECT kodeData, namaData from mst_data WHERE kodeInduk='987' AND kodeCategory ='CMC2' ORDER BY namaData ASC","kodeData","namaData","inp[id_kategori]","Pilih Kategori","$r[id_kategori]","","210px","chosen-select","")."
					</div>
				</p>
