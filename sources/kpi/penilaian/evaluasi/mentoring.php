...
function lData(){
  ...
  $kodeMentoring = getField("SELECT kodeData FROM mst_data WHERE kodeMaster = 'CMC2'");

  $sWhere = " WHERE a.id is not null and kategori = '$kodeMentoring'";
 ...
 $sql="SELECT a.id, a.tanggal, b.name AS atasan, c.namaData AS kategori 
 FROM pen_cmc AS a 
 JOIN emp AS b ON(a.atasan = b.id) JOIN mst_data AS c ON(a.id_kategori = c.kodeData)
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
  <form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
    ...
    <p>
      ...
      <div class=\"field\">
       ".comboData("SELECT id, name from emp WHERE status ='535' order by name asc","id","name","inp[peserta]","Pilih Peserta","$r[id_pegawai]","","250px","chosen-select","")."
     </div>
   </p>
 </form>
</div>
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
  <form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
    <fieldset>
      ...
      <p>
        ...
        <div class=\"field\">
         ".comboData("SELECT id, name from emp WHERE status ='535' order by name asc","id","name","inp[atasan]","Pilih Mentor","$r[atasan]","","250px","chosen-select","")."
       </div>
     </p>
     <p>
  ...
  <br>
  <h4 style=\"float:left; margin-top:10px;\">Peserta</h4>
  <a href=\"#\" $action2 style=\"float:right; margin-bottom:5px;\" class=\"btn btn1 btn_document\"><span>Tambah</span></a>
  <hr style='clear:both;'>
  <table class=\"stdtable stdtablequick\">
    <thead>
      <tr>
  ...
  <br>
  <h4 style=\"float:left; margin-top:10px;\">Dokumentasi</h4>
  <a href=\"#\" $action style=\"float:right; margin-bottom:5px;\" class=\"btn btn1 btn_document\"><span>Tambah</span></a>
  <hr style='clear:both;'>
