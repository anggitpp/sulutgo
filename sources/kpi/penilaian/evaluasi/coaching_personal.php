...
function id4() {
  global $s, $id, $inp, $par, $arrParameter;
  $data = arrayQuery("SELECT CONCAT(a.id, '\t', a.name) FROM emp AS a
JOIN emp_phist AS b ON a.id = b.parent_id
WHERE b.top_id='$par[atasan]' and b.top_id != 0");
  return implode("\n", $data);
}
...
function lihat(){
	...
	function lData(){
		global $s,$par,$fFile,$menuAccess,$cUsername,$sUser,$sGroup,$arrTitle,$arrParam,$folder_upload,$cID;	
		if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1')
			$sLimit="limit ".intval($_GET['iDisplayStart']).", ".intval($_GET['iDisplayLength']);

		$kodeCouching = getField("SELECT kodeData FROM mst_data WHERE kodeMaster = 'ECH'");

		$sWhere = " WHERE a.id is not null and a.kategori = $kodeCouching AND d.idPegawai ='$cID'";
		...
		$sql="select a.id, a.tanggal, b.name as peserta, c.name as atasan,d.idPegawai
		from pen_cmc as a
		join emp as b on(a.peserta = b.id)
		join emp as c on(a.atasan = c.id)
		join app_user as d on(a.atasan = d.idPegawai OR a.peserta = d.idPegawai)
		$sWhere order by $orderBy $sLimit";
		/*echo $sql;*/
		$res=db($sql);
		$json = array(
			"iTotalRecords" => mysql_num_rows($res),
			"iTotalDisplayRecords" => getField("select count(id) from pen_cmc WHERE kategori = $kodeCouching"),
			"aaData" => array(),
			);
	...
	function detail(){
		...
		if(empty($r[kategori])){
			...
			if($r[kategori] == '1088'){
				$stylef = "";
				$stylev = "display:none";
			}elseif($r[kategori] == '1089'){
				$stylef = "";
				$stylev = "display:none";
			}elseif($r[kategori] == '1090'){
				$stylef = "display:none";
				$stylev = "";
			}
		}
	...
	function form(){
		...
		if(!empty($r[atasan])){
			$filterAtasan = "and b.top_id = '$r[atasan]'";
		}else{
			$filterAtasan = "and b.top_id = 'sss'";
		}
		...
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					...
					<div class=\"field\">
						".comboData("SELECT DISTINCT(top_id), (select name from emp where id = top_id) as name from emp_phist WHERE status ='1' and top_id != '0' order by top_id asc","top_id","name","inp[atasan]","Pilih Atasan","$r[atasan]","onchange=\"getAtasan('" . getPar($par, "mode") . "');\"","250px","chosen-select","")."
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Pegawai</label>
					<div class=\"field\">
						".comboData("SELECT a.id, a.name from emp as a 
							join emp_phist as b on a.id = b.parent_id 
							WHERE a.status ='981' $filterAtasan  
			...
			<fieldset>
				...
				<p>
					<label class=\"l-input-small\">Kategori</label>
					<div class=\"field\">
						".comboData("SELECT kodeData, namaData from mst_data WHERE kodeCategory ='CMC2' ORDER BY namaData ASC","kodeData","namaData","inp[id_kategori]","Pilih Kategori","$r[id_kategori]","","210px","chosen-select","")."
					</div>
				</p>
			...
			<div class=\"subcontent\" id=\"foto\" style=\"border-radius:0; display: block;\">
				<ul class=\"listfile\">";
					$sql = db("SELECT * FROM pen_cmc_dokumentasi WHERE kategori = '1088' and id_cmc = $par[id]");
					while($r = mysql_fetch_assoc($sql)){
						...
						<li style=\"border:1px solid #eee;\">
							...
							<div style=\"text-align:center;border-top:1px solid #eee;\">
								...
								<a onclick=\"openBox('popup.php?par[mode]=detailFile&par[idf]=$r[id]".getPar($par,"mode")."',600,350)\" href=\"#\" style=\"padding: 5px 15px; text-align: center; display: inline-block; border:none; border-right:1px solid #eee; cursor: pointer;\"><img src=\"images/menu/edit.png\" height=\"20px\"></a>
			...
			<div class=\"subcontent\" id=\"file\" style=\"border-radius:0; display: none;\">
				<table class=\"stdtable stdtablequick\">
					...
					<tbody>";
						$sql = db("SELECT * FROM pen_cmc_dokumentasi WHERE kategori = '1089' and id_cmc = $par[id]");
						while($r = mysql_fetch_assoc($sql)){
							@$no++;
							$text .= "
							<tr>
			...
			<div class=\"subcontent\" id=\"video\" style=\"border-radius:0; display: none;\">";
				$sql = db("SELECT * FROM pen_cmc_dokumentasi WHERE kategori = '1090' and id_cmc = $par[id]");
				while($r = mysql_fetch_assoc($sql)){
