...
$par['idPeriode'] = empty($par['idPeriode']) ? getField("SELECT kodeData FROM mst_data WHERE kodeCategory='PRDT' and statusData='t' order by kodeData DESC LIMIT 1") : $par[idPeriode];
...
function lData()
{
    ...
    while ($r = mysql_fetch_array($res)) {
        ...
        $data1 = array(
            ...
            "<div align=\"center\"><a href=\"?par[mode]=detail&par[idPenPegawai]=$r[id]&par[idPegawai]=$r[idPegawai]&par[tipePenilaian]=$r[tipePenilaian]&par[kodePenilaian]=$r[kodePenilaian]&par[tahunPenilaian]=$par[idPeriode]&par[bulanPenilaian]=$blnPenilaian" . getPar($par, "mode,idPenPegawai,idPegawai,tipePenilaian,kodePenilaian,tahunPenilaian,bulanPenilaian,idPeriode") . "\" title=\"Realisasi\" class=\"detail\"><span>Realisasi</span></a></div>",
        ...
        $blnPenilaian = getField("SELEC kodeData FROM mst_data WHERE kodeCategory='PRDB' AND kodeInduk = $par[idPeriode] LIMIT 1");
...
function viewData()
{
    ...
    $text .= "
            ...
            <div class=\"widgetbox\" style=\"position: relative;\">
                ...
                <div id=\"pos_r\" style=\"float:right; margin-top:-8px;\">
    ...
    <script>
    	jQuery(\"#btnExport\").live('click', function(e){
    		e.preventDefault();
    		window.location.href=\"?par[mode]=viewData&par[idPenPegawai]=$par[idPenPegawai]&par[idPegawai]=$par[idPegawai]&par[tipePenilaian]=$par[tipePenilaian]&par[kodePenilaian]=$par[kodePenilaian]&par[tahunPenilaian]=$par[tahunPenilaian]&par[bulanPenilaian]=$par[bulanPenilaian]&par[idPeriode]=$par[idPeriode]\"+\"" . getPar($par, "mode") . "\"+\"&export=xls\"
    	});
    </script>
    ";
    return $text;
...
function detail()
{
    ...
    <script>
    	jQuery(\"#btnExport\").live('click', function(e){
    		e.preventDefault();
    		window.location.href=\"?par[mode]=detail&par[idPenPegawai]=$par[idPenPegawai]&par[idPegawai]=$par[idPegawai]&par[tipePenilaian]=$par[tipePenilaian]&par[kodePenilaian]=$par[kodePenilaian]&par[tahunPenilaian]=$par[tahunPenilaian]&par[bulanPenilaian]=$par[bulanPenilaian]&par[idPeriode]=$par[idPeriode]\"+\"" . getPar($par, "mode") . "\"+\"&export=xls\"
    	});
    </script>
    ";
    return $text;
