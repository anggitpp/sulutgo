...
if($json==1){
	...
	$filter = "WHERE idAspek IS NOT NULL";
    
	if(!empty($par[idPeriode])) $filter .= " AND idPeriode = '$par[idPeriode]'";

	$sql = "
	SELECT 
	*
	FROM pen_setting_aspek 
	$filter
	ORDER BY urutanAspek";
	$ret = array();
	$res = db($sql);
	...
	<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="datatable">
		<thead>
			<tr>
				...
				<th width="250" style="vertical-align: middle">ASPEK</th>
				<th style="vertical-align: middle">PENILAIAN</th>
                <th width="80" style="vertical-align: middle">BOBOT</th>
				<th width="80" style="vertical-align: middle">TARGET</th>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		ot = jQuery('#datatable').dataTable({
			...
			{"mData": "namaAspek", "bSortable": true},
			{"mData": "penilaianAspek", "bSortable": false},
            {"mData": "bobotAspek" , "sClass": "alignCenter",  "bSortable": false},
			{"mData": "targetAspek", "sClass": "alignCenter", "bSortable": false, "fnRender": function(o){
				var ret = "<a href=\"#target\" onclick=\"targetBox('" + o.aData['idAspek'] + "');\">"; 
				ret += "<span style=\"color: #02a808\"><b>" + o.aData['targetAspek'] + "</b></span>";
				ret += "</a>";
				return ret;
			}},
            {"mData": "statusAspek", "sClass": "alignCenter", "bSortable": false, "fnRender": function(o){
				var ret = "";
				if(o.aData['statusAspek'] == "t")
			...
			if(isset($menuAccess[$s]['edit']) || isset($menuAccess[$s]['delete'])){
				...
				{"mData": null, "sClass": "alignCenter", "bSortable": false, "fnRender": function(o){
					...
					if(isset($menuAccess[$s]['edit'])){
						?>
						ret += "<a href=\"#edit\" onclick=\"editBox('" + o.aData['idAspek'] + "');\" class=\"edit\"><span>Edit Data</span></a>";
						<?php 
					}
					?>
					...
					if(isset($menuAccess[$s]['delete'])){
						?>
						ret += "<a href=\"#del\" onclick=\"del('" + o.aData['idAspek'] + "');\" class=\"delete\" title=\"Delete Data\"><span>Delete Data</span></a>";
						<?php 
					}
					?>
					return ret;
		...
		jQuery("#btnAdd").live("click", function(e){
			e.preventDefault();
			openBox('popup.php?par[mode]=add&par[idPeriode]=' + jQuery("#par\\[idPeriode\\]").val() + '<?= getPar($par, "mode,idPeriode") ?>', 800,500);
		});
...
function editBox(idAspek){
	openBox('popup.php?par[mode]=edit&par[kodeAspek]=' + kodeAspek + '&par[idAspek]=' + idAspek + '<?= getPar($par, "mode,kodeAspek") ?>', 800, 500);
}

function targetBox(idAspek){
	openBox('popup.php?par[mode]=dlgTarget&par[kodeAspek]=' + kodeAspek + '&par[idAspek]=' + idAspek + '<?= getPar($par, "mode,kodeAspek") ?>', 800, 500);
}

function del(idAspek){
	if(confirm('are you sure to delete data ?'))
		window.location='?par[mode]=del&par[kodeAspek]=' + kodeAspek + '&par[idAspek]=' + idAspek + '<?= getPar($par, "mode,kodeAspek") ?>';
}
