...
$fRencana = "files/dokumentasi/sdm/";
...
function uploadDok($id) {
	...
	if (($fileUpload != "") and ( $fileUpload != "none")) {
		...
		$foto_file = "sdm_dok-" . time() . "." . getExtension($fileUpload_name);
		fileRename($fRencana, $fileUpload_name, $foto_file);
	}
	if (empty($foto_file))
		$foto_file = getField("select file from doc_sdm where id ='$id'");
...
function tambah(){
	...
	$id = getField("select id from doc_sdm order by id desc limit 1")+1;
	$inp[file] = uploadDok($id);
	$sql = "insert into doc_sdm (id, id_kategori, nama_file, file, keterangan, created_date, created_by) values ('$id', '$inp[id_kategori]','$inp[nama_file]','$inp[file]','$inp[keterangan]','".date('Y-m-d H:i:s')."','$cUsername')";
	// echo $sql;
	 //die();
	db($sql);
...
function ubah(){
	...
	$sql = "update doc_sdm set nama_file = '$inp[nama_file]', file = '$inp[file]', id_kategori = '$inp[id_kategori]',keterangan = '$inp[keterangan]', updated_by = '$cUsername', updated_date = '".date('Y-m-d H:i:s')."' where id = '$par[id]'";
	db($sql);
	...
	<form action=\"\" method=\"post\" id = \"form\" class=\"stdform\" onsubmit=\"return false;\">
		...
		<div id=\"pos_l\" style=\"float:left;\">
			...
			<p>					
				...
				".comboData("select * from mst_data where kodeCategory = 'KDS' order by urutanData","kodeData","namaData","bSearch","ALL KATEGORI",$bSearch,"","210px;","chosen-select")."
			</p>
...
function lData(){
	...
	if (!empty($_GET['fSearch']))
		$filters.= " and (				
	lower(nama_file) like '%".mysql_real_escape_string(strtolower($_GET['fSearch']))."%'				
	)";
	...
	if(!empty($_GET[bSearch])){
		$filters.=" and id_kategori = '$_GET[bSearch]'";
	}


	$orderBy = $arrOrder["".$_GET[iSortCol_0].""]." ".$_GET[sSortDir_0];

	$sql = "SELECT *,date(created_date) as tanggal FROM doc_sdm $filters order by $orderBy $sLimit ";
		// echo $sql;
	...
	$json = array(
		...
		"iTotalDisplayRecords" => getField("SELECT COUNT(*) FROM doc_sdm $filters"),
		"aaData" => array(),
		);
	...
	while($r=mysql_fetch_array($res)){
		...
		if(empty($r[file])){
			...
			$r[download] = "<a href=\"download.php?d=fileDocSDM&f=$r[id]\"><img src=\"".getIcon($r[file])."\" align=\"center\" style=\"padding-right:5px; padding-bottom:5px; max-width:20px; max-height:20px;\"></a>";
			$r[view] = "<a href=\"#\" onclick=\"openBox('view.php?doc=fileSDM&par[idDoc]=$r[id]".getPar($par,"mode")."',725,500);\" class=\"detail\"><span>Detail</span></a>";
		...
		$data=array(
			...
			"<div align=\"left\">$r[nama_file]</div>",
			...
			"<div align=\"center\">$r[created_by]</div>",	
...
function xls(){		
	...
	$rows=5;
	$sql = "SELECT *,date(created_date) as tanggal from doc_sdm $filters";
	...
	while($r=mysql_fetch_array($res)){			
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$rows, $r[nama_file]);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, namaMaster($r[id_kategori]));
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $r[keterangan]);
...
function hapus(){
	...
	$sql="delete from doc_sdm where id='$par[id]'";
	db($sql);
...
function form(){
	...
	$sql="SELECT * FROM doc_sdm t1 WHERE id='$par[id]'";
	// echo $sql;
	$res=db($sql);
	$r=mysql_fetch_array($res);	
	$arrNama = arrayQuery("select username, nickName from app_user");
	$r[created_by] = empty($r[created_by]) ? $cNickname : $arrNama[$r[created_by]];
	$r[updated_by] = empty($r[updated_by]) ? $cNickname : $arrNama[$r[updated_by]];
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
				...
				<table style=\"width: 100%\">
					<tr>
						<td width=\"50%\">
							...
							<span class=\"field\">$r[created_date]&nbsp;</span>
						</td>
						<td width=\"50%\">
							<label class=\"l-input-small\" style=\"width:50%\">Input By</label>
							<span class=\"field\">$r[created_by]&nbsp;</span>		
						</td>
					</tr>
				</table>
				...
				<table style=\"width: 100%\">
					<tr>
						<td width=\"50%\">
							...
							<span class=\"field\">$r[updated_date]&nbsp;</span>
						</td>
						<td width=\"50%\">
							<label class=\"l-input-small\" style=\"width:50%\">Update By</label>
							<span class=\"field\">".$arrNama[$r[updated_by]]."&nbsp;</span>		
						</td>
					</tr>
				</table>
		...
		<fieldset>
			...
			<p>
				...
				<div class=\"field\">
					".comboData("select * from mst_data where statusData='t' and kodeCategory='KDS' order by urutanData","kodeData","namaData","inp[id_kategori]"," ",$r[id_kategori],"onchange=\"getAktifitas('" . getPar($par, "mode,id_kategori") . "');\"", "310px")."
				</div>
			</p>
		</fieldset>
		<fieldset>
			...
			<p>
				...
				<div class=\"field\">								
					<input type=\"text\" id=\"inp[nama_file]\" name=\"inp[nama_file]\"  value=\"$r[nama_file]\" class=\"mediuminput\" />
				</div>
			</p>
			<p>
