...
/**
 * Description of class.emp_asset.inc.php
 *
 * @author mazte
 */
class EmpAsset extends DAL {
...
:pStatus, 
    ...
    try {
      ...
      $this->astDate = setTanggal($this->astDate);
      $this->astDateStart = setTanggal($this->astDateStart);
      $this->astDateEnd = setTanggal($this->astDateEnd);
      $stmt = $this->db->prepare($sql);
      $stmt->bindParam(':pParentId', $this->parentId, PDO::PARAM_STR);
      $stmt->bindParam(':pAstName', $this->astName, PDO::PARAM_STR);
      $stmt->bindParam(':pAstNo', $this->astNo, PDO::PARAM_STR);
      $stmt->bindParam(':pAstUsage', $this->astUsage, PDO::PARAM_STR);
      $stmt->bindParam(':pAstDate', $this->astDate, PDO::PARAM_STR);
      $stmt->bindParam(':pAstCat', $this->astCat, PDO::PARAM_STR);
      $stmt->bindParam(':pAstType', $this->astType, PDO::PARAM_STR);
      $stmt->bindParam(':pAstDateStart', $this->astDateStart, PDO::PARAM_STR);
      $stmt->bindParam(':pAstDateEnd', $this->astDateEnd, PDO::PARAM_STR);
      $stmt->bindParam(':pAstJenis', $this->astJenis, PDO::PARAM_STR);
      $stmt->bindParam(':pAstFilename', $this->astFilename, PDO::PARAM_STR);
...
ast_cat=:pAstCat,
ast_type=:pAstType,
ast_date_start=:pAstDateStart,
ast_date_end=:pAstDateEnd,
ast_jenis=:pAstJenis,
ast_filename=:pAstFilename,
remark=:pRemark,
status=:pStatus,
...
status,
    ...
    try {
      ...
      foreach ($result as $row) {
        ...
        $this->astName = $row["ast_name"];
        $this->astNo = $row["ast_no"];
        $this->astUsage = $row["ast_usage"];
        $this->astDate = $row["ast_date"];
        $this->astCat = $row["ast_cat"];
        $this->astType = $row["ast_type"];
        $this->astDateStart = $row["ast_date_start"];
        $this->astDateEnd = $row["ast_date_end"];
        $this->astJenis = $row["ast_jenis"];
        $this->astFilename = $row["ast_filename"];
...
status,
    ...
    try {
      ...
      foreach ($result as $row) {
        ...
        $ret0->astName = $row["ast_name"];
        $ret0->astNo = $row["ast_no"];
        $ret0->astUsage = $row["ast_usage"];
        $ret0->astDate = $row["ast_date"];
        $ret0->astCat = $row["ast_cat"];
        $ret0->astType = $row["ast_type"];
        $ret0->astDateStart = $row["ast_date_start"];
        $ret0->astDateEnd = $row["ast_date_end"];
        $ret0->astJenis = $row["ast_jenis"];
        $ret0->astFilename = $row["ast_filename"];
...
DATE_FORMAT(t1.ast_date, '%d/%m/%Y') astDate,
DATE_FORMAT(t1.ast_date_start, '%d/%m/%Y') astDateStart,
DATE_FORMAT(t1.ast_date_end, '%d/%m/%Y') astDateEnd,
t1.ast_cat astCat,
t1.ast_type astType,
