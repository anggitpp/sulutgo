...
/**
 * Description of class.rec_applicant_edu.inc.php
 *
 * @author mazte
 */
class RecApplicantEdu extends DAL {
...
:pEduIpk, 
    ...
    try {
      ...
      $stmt->bindParam(':pEduIpk', $this->eduIpk, PDO::PARAM_STR);
  ...
  function update() {
    $sql = " UPDATE rec_applicant_edu SET
...
edu_ipk=:pEduIpk,
    ...
    try {
      ...
      $stmt->bindParam(':pEduFilename', $this->eduFilename, PDO::PARAM_STR);
      $stmt->bindParam(':pRemark', $this->remark, PDO::PARAM_STR);
      $stmt->bindParam(':pEduIpk', $this->remark, PDO::PARAM_STR);
  ...
  function destroy() {
    $sql = " DELETE FROM rec_applicant_edu
				 WHERE id=:pId
				 ";
    try {
      $stmt = $this->db->prepare($sql);
...
edu_ipk,
  ...
  function getEduByParentId($parentId) {
    $sql = " SELECT
...
t1.edu_ipk eduIpk,
  ...
  function getEmpEduByParentId($parentId) {
    $sql = " SELECT
...
//    $this->eduName = strtoupper($this->eduName);
    ...
    if ($fileKtpTmp != "" && $fileKtpTmp != "none") {
      $ktpPath = HOME_DIR . DS . "files" . DS . "recruit" . DS . "edu" . DS;
      if (!file_exists($ktpPath)) {
        mkdir($ktpPath, "0777", true);
      }
      $fktpName = uniqid("edu_") . "." . pathinfo($fileKtpName, PATHINFO_EXTENSION);
