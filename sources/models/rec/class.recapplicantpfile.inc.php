 ...
 * @author mazte
 */
class RecApplicantPfile extends DAL {
  ...
  function persist() {
    $sql = "INSERT INTO rec_applicant_pfile (
...
function update() {
  $sql = " UPDATE rec_applicant_pfile SET
...
function destroy() {
  $sql = " DELETE FROM rec_applicant_pfile
  WHERE id=:pId
  ";
  try {
    $stmt = $this->db->prepare($sql);
...
function getPfileByParentId($parentId) {
  $sql = " SELECT
...
function getEmpPfileByParentId($parentId) {
  $sql = " SELECT
