...
LEFT JOIN mst_data t8 ON t8.kodeData=t2.marital
  ...
  function loadTable($applCat, $cVac, $cYear) {
    $sql = " SELECT
t1.id id,
(SELECT COUNT(sel_status) from rec_selection_appl where appl_id = t1.applicant_id AND sel_status !='0') as countseleksi,
(SELECT COUNT(id) from rec_selection_appl where appl_id = t1.applicant_id AND sel_status ='601' AND phase_id='602') as ptrue,
(SELECT COUNT(id) from rec_applicant_ver where applicant_id = t1.applicant_id AND sel_status is not null) as sel,
(SELECT COUNT(id) from rec_selection_placement where appl_id = t1.applicant_id) as placeId,
(SELECT COUNT(id) from rec_selection_appl where appl_id = t1.applicant_id AND sel_status ='600' AND phase_id='602') as pfalse,
(SELECT COUNT(id) from rec_selection_appl where appl_id = t1.applicant_id AND sel_status ='601' AND phase_id = '603') as psikotestt,
(SELECT COUNT(id) from rec_selection_appl where appl_id = t1.applicant_id AND sel_status ='600' AND phase_id = '603') as psikotestf,
t1.plan_id planId,
t1.vac_id vacId,
t1.applicant_id applicantId,
t1.recommendation recommendation,
...
CASE WHEN t4.cat = '3' THEN t4.pos_available ELSE t4.subject END posAvailable,
t2.gender gender,
