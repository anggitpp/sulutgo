...
/**
 * Description of class.rec_applicant_char.inc.php
 *
 * @author mazte
 */
class RecApplicantChar extends DAL {
  ...
  function persist() {
    $sql = "INSERT INTO rec_applicant_char (
    ...
    try {
      ...
      $stmt->bindParam(':pCharacteristic', $this->characteristic, PDO::PARAM_STR);
      $stmt->bindParam(':pHobby', $this->hobby, PDO::PARAM_STR);
      $stmt->bindParam(':pAbilities', $this->abilities, PDO::PARAM_STR);
      $stmt->bindParam(':pOrganization', $this->organization, PDO::PARAM_STR);
  ...
  function update() {
    $sql = " UPDATE rec_applicant_char SET
  ...
  function destroy() {
    $sql = " DELETE FROM rec_applicant_char
				 WHERE id=:pId
				 ";
    try {
      $stmt = $this->db->prepare($sql);
