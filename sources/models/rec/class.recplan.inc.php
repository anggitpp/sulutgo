...
class RecPlan extends DAL {
  ...
  function insert(){
    ...
    $tanggal = date('Y-m-d');
    // $sql = "insert into rec_plan (div_id) values ('898')";
    // db($sql);
    


    $id_post = getField("select id from rec_post order by id desc limit 1") + 1;
    $id_vac = getField("select id from rec_vacancy order by id desc limit 1") + 1;
    $plan_id = getField("select id from rec_plan order by id desc limit 1");
    
    $sql = "insert into rec_vacancy (id, plan_id, status, appr1_sta,appr1_by,appr1_date,cre_by,cre_date) values ('$id_vac','$plan_id','3','3','$cUsername','$sekarang','$cUsername','$sekarang')";
    db($sql);

    $sql = "insert into rec_post (id, plan_id,vac_id, cre_by,cre_date) values ('$id_post','$plan_id','$id_vac','$cUsername','$sekarang')";
    db($sql);

    $id_detail = getField("select id from rec_post_detail order by id desc limit 1") + 1;
    $sql = "insert into rec_post_detail (id, parent_id,post_type_id, post_sort,post_value,cre_by,cre_date) values ('$id_detail','$id_post','608','1','$tanggal','$cUsername','$sekarang')"; #INTERNAL
    db($sql);
    $id_detail = getField("select id from rec_post_detail order by id desc limit 1") + 1;
    $sql = "insert into rec_post_detail (id, parent_id,post_type_id, post_sort,post_value,cre_by,cre_date) values ('$id_detail','$id_post','609','2','$tanggal','$cUsername','$sekarang')"; #INTERNET
    db($sql);
    $id_detail = getField("select id from rec_post_detail order by id desc limit 1") + 1;
    $sql = "insert into rec_post_detail (id, parent_id,post_type_id, post_sort,post_value,cre_by,cre_date) values ('$id_detail','$id_post','610','3','$tanggal','$cUsername','$sekarang')"; #MEDIA
    db($sql);
    $id_detail = getField("select id from rec_post_detail order by id desc limit 1") + 1;
    $sql = "insert into rec_post_detail (id, parent_id,post_type_id, post_sort,post_value,cre_by,cre_date) values ('$id_detail','$id_post','611','4','$tanggal','$cUsername','$sekarang')"; #KONSULTAN
    db($sql);



 
    $id_selection = getField("select id from rec_selection order by id desc limit 1") + 1;  

    $sql = "insert into rec_selection (id, plan_id,vac_id, cre_by,cre_date) values ('$id_selection','$plan_id','$id_vac','$cUsername','$sekarang')";
    db($sql);

    // $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    //  $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','602','1','$cUsername','$sekarang')"; #PANGGILAN
    //  db($sql);
    //  $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    // $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','603','2','$cUsername','$sekarang')"; #PSIKOTEST
    // db($sql);
    // $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    // $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','604','3','$cUsername','$sekarang')"; #WAWANCARA HR
    // db($sql);
    // $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    // $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','605','4','$cUsername','$sekarang')"; #WAWANCARA USER
    // db($sql);
    // $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    // $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','606','5','$cUsername','$sekarang')"; #MCU
    // db($sql);
    // $id_phase = getField("select id from rec_selection_phase order by id desc limit 1") + 1;
    // $sql = "insert into rec_selection_phase (id, parent_id,plan_id, vac_id,phase_id,phase_sort,cre_by,cre_date) values ('$id_phase','$id_selection','$plan_id','$id_vac','607','6','$cUsername','$sekarang')"; #HASIL
    // db($sql);
...
//           )";
    ...
    try {
      global $db,$cUsername,$planType;
      date_default_timezone_set('Asia/Jakarta');
      $this->creBy = $cUsername;
      $this->creDate = date('Y-m-d H:i:s');
      if($planType=="3"){ $this->divId = 898; $this->proposeDate = date('Y-m-d'); $this->apprDivSta = 3; $this->stat = 3; $this->apprSdmSta = 3; }
      if(empty($this->empId)) $this->empId = 0;
      ...
      $stmt->bindParam(':pEduId2', $this->eduId2, PDO::PARAM_STR);
      ...
      $stmt->bindParam(':pCharacter', $this->characters, PDO::PARAM_STR);
      ...
      $stmt->bindParam(':pStat', $this->stat, PDO::PARAM_STR);
  ...
  function loadTable($cat, $cDivisi, $cYear) {
    ...
    LEFT JOIN emp t5 on t1.emp_id=t5.id 
    WHERE t1.cat=:pCat
  ...
  function loadTableReport($cat, $cDivisi, $cYear, $stDate, $edDate) {
    ...
    t1.div_id divId,
    t1.loc_id locId,
    CASE WHEN t1.cat = '3' THEN upper(t1.pos_available) ELSE
    (select upper(pos_available) from rec_plan where id = t1.pos_available) END posAvailable,
    t1.person_needed personNeeded,
    t1.edu_id eduId,
    t1.edu_id2 eduId2,
    t1.edu_fac_id eduFacId,
...
//    echo $sql;
  ...
  function exportToExcelReport($cat, $cDivisi, $cYear, $stDate, $edDate) {
    ...
    t1.characters `Karakteristik`, t1.job_desk `Uraian Tugas Utama`,
