...
class RecApplicant extends DAL {
  ...
  function insert(){
    ...
    // $sql = "insert into rec_selection (id,plan_id,vac_id,cre_by,cre_date) VALUES ('$lastid','$plan_id','" . $_POST['vacId'] . "', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    // db($sql); #insert parent

    $lastid = getField("select id from rec_selection where vac_id = '" . $_POST['vacId'] . "' ");

    $plan_id = getField("select plan_id from rec_vacancy where id = '" . $_POST['vacId'] . "'");

    $id = getField("select id from rec_applicant order by id desc");
    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','602','1', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);

    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','603','2', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);

    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','604','3', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);

    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','605','4', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);

    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','606','5', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);

    $lastida = getField("select id from rec_selection_appl order by id desc")+1;
    $sql = "insert into rec_selection_appl (id,parent_id,plan_id,vac_id,appl_id,phase_id,phase_sort,cre_by,cre_date) VALUES ('$lastida','$lastid','$plan_id','" . $_POST['vacId'] . "','$id','607','6', '$cUsername','" . date('Y-m-d H:i:s') . "')";
    db($sql);
  }
  ...
  function processForm() {
    ...
    if ($filePicTmp != "" && $filePicTmp != "none") {
      ...
      if (move_uploaded_file($filePicTmp, $picPath . $fpicName)) {
       $this->picFilename = resizeImage($picPath,$fpicName,1280,720);
      }
    }
    if ($this->id == "") {
      $this->persist();
    ...
    if (count($trainIds) > 0) {
      ...
      foreach ($trainIds as $dTrainId) {
        ...
        $applTrain->name = $_POST["pTrainName"][$dno];
        $applTrain->training = $_POST["pTrainTraining"][$dno];
        $applTrain->position = $_POST["pTrainPosition"][$dno];
        $applTrain->location = $_POST["pTrainLocation"][$dno];
        $applTrain->start = $_POST["pTrainStart"][$dno];
        $applTrain->ends = $_POST["pTrainEnds"][$dno];
    ...
    //PROCESS APPL FILE
    $fileIds = $_POST["pFileId"];
    $dno = 0;
    if (count($fileIds) > 0) {
      $usedId = array();
      foreach ($fileIds as $dFileId) {
        $applFile = new RecApplicantPfile();
        $applFile->id = $dFileId;
        $applFile->parentId = $this->id;
        $applFile->name = $_POST["pFileName"][$dno];
        $applFile->filename = $_POST["pFileFilename"][$dno];
        $applFile->remark = $_POST["pFileRemark"][$dno];
        if ($applFile->id == "") {
          $applFile = $applFile->persist();
        } else {
          $applFile->update();
        }
        $usedId[] = $applFile->id;
      ...
      $usedDtlIds = implode(",", $usedId);
      $cutil->execute("DELETE FROM rec_applicant_pfile WHERE parent_id='$this->id' AND id NOT IN ($usedDtlIds)");
    } else {
      $cutil->execute("DELETE FROM rec_applicant_pfile WHERE parent_id='$this->id'");
    }
