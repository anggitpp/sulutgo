...
function addEmp(){
  global $s,$inp,$par,$arrTitle,$menuAccess,$arrParam, $par;
  $text .="
  <style>
        #inp_kodeRekening__chosen{
    min-width:250px;
...
<div class=\"centercontent contentpopup\">
  ...
  <div id=\"contentwrapper\" class=\"contentwrapper\">
    ...
    <input type=\"text\" name=\"par[filterData]\" value =\"$par[filterData]\" class=\"smallinput\" style=\"width:200px;\">
    <input type=\"submit\" value=\"GO\">
    <input type=\"hidden\" name=\"par[mode]\" value=\"$par[mode]\">
    <input type=\"hidden\" name=\"par[id_perubahan]\" value=\"$par[id_perubahan]\">
    </form>
      <table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" style=\"margin-top:10px;\" id=\"dyntable\">
        ...
        <tbody>";
          ...
          $added = implode(",", $added);
          $filter= empty($added) ? "" : "AND `id` NOT IN ($added)";

          if(!empty($par[filterData]))
            $filter .= " AND (lower(name) LIKE '%".strtolower($par[filterData])."%' OR lower(reg_no) LIKE '%".strtolower($par[filterData])."%')";

          $sql = db("SELECT * FROM `emp` WHERE `status` = '535' $filter");
...
function lData(){
 ...
 while($r=mysql_fetch_array($res)){
  ...
  $data=array(
    ...
    "<div align=\"center\"><a href='#' onclick=\"openBox('popup.php?par[mode]=app&par[id_perubahan]=$r[id_perubahan]&par[status_pegawai]=$r[kategori]".getPar($par,"mode, id_perubahan, status_pegawai")."',800,450)\">".$r[app_status]."</a></div>",
    "
    <div align=\"center\">
...
function form(){
  ...
  if(!empty($par[id_perubahan])){
    $script3 = "onclick=\"openBox('popup.php?par[mode]=addEmp".getPar($par,"mode")."', 960, 425)\"";
  }else{
...
<div id=\"contentwrapper\" class=\"contentwrapper\">
  <form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
    <fieldset>
      ...
      <p>
        <label class=\"l-input-small\">Perubahan Status $r[kategori]</label>
        <div class=\"field\">
         ".comboData("SELECT * from mst_data WHERE kodeCategory ='S05' and kodeData != '535'","kodeData","namaData","inp[kategori]","Pilih Perubahan Status","$r[kategori]","","250px","chosen-select","")."
       </div>
     </p>
     <p>
      <label class=\"l-input-small\">Keterangan</label>
    ...
    <div>
      ...
      if($r[app_status]!='1'){
      $text.="
      <a style='float:right' class=\"btn btn1 btn_document\" $script3><span>Tambah</span></a>";
      }
      $text.="
    </div>
    <br style='clear:both;'>
