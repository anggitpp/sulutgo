...
function update(){
	...
	if(is_array($det[keteranganAbsensi])){
		while(list($idPeserta, $keteranganAbsensi) = each($det[keteranganAbsensi])){
			$statusAbsensi = $det[statusAbsensi][$idPeserta] == "t" ? "t" : "f";
			$sql=getField("select idPeserta from plt_pelatihan_absensi where idPelatihan='$par[idPelatihan]' and idJadwal='$par[idJadwal]' and idPeserta='$idPeserta'")?
			"update plt_pelatihan_absensi set statusAbsensi='$statusAbsensi', keteranganAbsensi='$keteranganAbsensi', updateBy='$cUsername', updateTime='".date("Y-m-d H:i:s")."' where idPelatihan='$par[idPelatihan]' and idJadwal='$par[idJadwal]' and idPeserta='$idPeserta'":
			"insert into plt_pelatihan_absensi (idPelatihan, idJadwal, idPeserta, statusAbsensi, keteranganAbsensi, createBy, createTime) values ('$par[idPelatihan]', '$par[idJadwal]', '$idPeserta', '$statusAbsensi', '$keteranganAbsensi', '$cUsername', '".date("Y-m-d H:i:s")."')";
			db($sql);
		}
	}

	echo detail()."<script>
	alert('update daftar kehadiran success');
...
function detail(){
	global $db,$s,$inp,$par,$arrTitle,$arrParameter,$fileTemp,$fFile,$menuAccess;				
...
<div class=\"contentwrapper\">
	<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?".getPar($par)."\"  >	
		...
		<div id=\"general\" style=\"margin-top:20px;\">					
			...
			<fieldset id=\"fSet\" style=\"padding:10px; border-radius: 10px;\">
				<legend style=\"padding:10px; margin-left:20px;\"><h4>DAFTAR KEHADIRAN</h4></legend>
				".setPar($par)."";

				if(empty($par[idJadwal]))
					$par[idJadwal] = getField("select idJadwal from plt_pelatihan_jadwal where idPelatihan='".$par[idPelatihan]."' order by tanggalJadwal limit 1");

				if($par[idJadwal]){	
				// $text .= "(SELECT keteranganDetail FROM plt_pelatihan_detail WHERE idPelatihan = t1.idPelatihan AND t1.tanggalJadwal BETWEEN DATE(mulaiDetail) AND DATE(selesaiDetail) LIMIT 1), ' - ', ";		
					$text.="Jadwal Pelatihan : ".comboData("select t1.idJadwal id, CONCAT(t1.judulJadwal) description from plt_pelatihan_jadwal t1 where idPelatihan='$par[idPelatihan]' order by tanggalJadwal","id","description","par[idJadwal]","",$par[idJadwal],"onchange=\"document.getElementById('form').submit();\"","250px","chosen-select")."";
					$text.="&nbsp;&nbsp;&nbsp;Tanggal Pelatihan : <span id=\"displayTanggalJadwal\">".getTanggal(getField("select tanggalJadwal from plt_pelatihan_jadwal where idJadwal = '$par[idJadwal]' AND idPelatihan = '$par[idPelatihan]'"))."</span>";

					if (isset($menuAccess[$s]["edit"])){
						$text .= "<div style=\"float:right\">";
						$text.="<input type=\"checkbox\" id=\"checkAll\"/>Check All&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
						$text.="<input type=\"submit\" class=\"submit radius2\" name=\"_submit\" value=\"Update Data\"/>";
						$text .= "</div>";
					}

					$text.="<table id=\"dtPeserta\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" style=\"margin-top:15px;\">
					<thead>
						<tr>
							<th width=\"20\">No.</th>
							<th>Nama</th>
					...
					<tbody>";
						...
						$sql="select * from plt_pelatihan_absensi where idPelatihan='$par[idPelatihan]' and idJadwal='$par[idJadwal]'";
						$res=db($sql);					
						while($r=mysql_fetch_array($res)){								
							$det[statusAbsensi]["$r[idPeserta]"] = $r[statusAbsensi];
							$det[keteranganAbsensi]["$r[idPeserta]"] = $r[keteranganAbsensi];
						}
						...
						$no=1;
						while($r=mysql_fetch_array($res)){								
							$checked = $det[statusAbsensi]["$r[idPeserta]"] == "t" ? "checked=\"checked\"" : "";
							$disabled = isset($menuAccess[$s]["edit"]) ? "" : "disabled=\"disabled\"";
							$text.="<tr>
							<td>$no.</td>
							<td>".strtoupper($r[name])."</td>
							<td>$r[jabatanPeserta]</td>																
							<td align=\"center\">
								<input type=\"checkbox\" id=\"det[statusAbsensi][".$r[idPeserta]."]\" name=\"det[statusAbsensi][".$r[idPeserta]."]\" value=\"t\" $checked $disabled />
							</td>
							<td>";
								$text.= isset($menuAccess[$s]["edit"]) ? 
								"<input type=\"text\" id=\"det[keteranganAbsensi][".$r[idPeserta]."]\" name=\"det[keteranganAbsensi][".$r[idPeserta]."]\"  value=\"".$det[keteranganAbsensi]["$r[idPeserta]"]."\" class=\"mediuminput\" style=\"width:95%;\" maxlength=\"150\" />":
								$det[keteranganAbsensi]["$r[idPeserta]"];
								$text.="</td>
							</tr>";			
							$no++;
						}
				...
				$text.="<div align=\"center\">belum ada jadwal pelatihan</div>";
			}
			$text.="</fieldset>
		</div>				
	</form>";
	...
	if($par[mode] == "detXls"){			
		detXls();			
		$text.="<iframe src=\"download.php?d=exp&f=".ucwords(strtolower("Daftar Kehadiran")).".xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
	}	
...
function detXls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A11', 'ABSENSI KEHADIRAN');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('E12', 'HADIR');
	$objPHPExcel->getActiveSheet()->setCellValue('F12', 'KETERANGAN');
	...
	while($r=mysql_fetch_array($res)){				
		...
		$statusAbsensi = $det[statusAbsensi]["$r[idPeserta]"] == "t" ? "V" : "";
		...
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $statusAbsensi);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $det[keteranganAbsensi]["$r[idPeserta]"]);	
	...
	$objWriter->save($fFile.ucwords(strtolower("Daftar Kehadiran")).".xls");
}
