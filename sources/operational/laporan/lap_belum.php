...
function lData(){
	...
	list($kodePelatihan, $mulaiPelatihan, $jmlPelatihan, $jamPelatihan, $menitPelatihan) = explode("\t",getField("select concat(t1.kodePelatihan,'\t', t1.mulaiPelatihan, '\t', count(t2.idJadwal), '\t', sum(hour(timediff(t2.selesaiJadwal, t2.mulaiJadwal))), '\t', sum(minute(timediff(t2.selesaiJadwal, t2.mulaiJadwal)))) from plt_pelatihan t1 join plt_pelatihan_jadwal t2 on (t1.idPelatihan=t2.idPelatihan) $sWhere"));
	...
	$sql="select n1.*, n2.namaData as namaDepartemen from (
			select d1.*, d2.namaData as namaDivisi from (
				select * from (
					select t1.kodePelatihan, t1.mulaiPelatihan, t2.idPeserta, t2.idPegawai from plt_pelatihan t1 join plt_pelatihan_peserta t2 on (t1.idPelatihan=t2.idPelatihan) $sWhere
				) as p1 right join dta_pegawai p2 on (p1.idPegawai=p2.id) where p1.idPegawai is null
	...
	$json = array(
		...
		) as n1 left join mst_data n2 on (n1.dept_id=n2.kodeData)"),
		"aaData" => array(),
		);
	...
	while($r=mysql_fetch_array($res)){
		...
		$data=array(
			...
			"<div align=\"left\">".$kodePelatihan."</div>",			
			"<div align=\"center\">".getTanggal($mulaiPelatihan)."</div>",
...
function xls(){		
	...
	while($r=mysql_fetch_array($res)){	
		...
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $kodePelatihan);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, getTanggal($mulaiPelatihan));
