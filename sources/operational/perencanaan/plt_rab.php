...
function ubah()
{
    ...
    $sql = "update plt_pelatihan_rab set judulRab='$inp[judulRab]', jumlahRab='" . setAngka($inp[jumlahRab]) . "', satuanRab='$inp[satuanRab]', satuanHarga='$inp[satuanHarga]', satuanPengalih='$inp[satuanPengalih]', hargaRab='" . setAngka($inp[hargaRab]) . "', hargaPengalih='" . setAngka($inp[hargaPengalih]) . "', nilaiRab='" . setAngka($inp[nilaiRab]) . "', keteranganRab='$inp[keteranganRab]', updateBy='$cUsername', updateTime='" . date('Y-m-d H:i:s') . "' where idPelatihan='$par[idPelatihan]' and idRab='$par[idRab]'";
    db($sql);
...
function tambah()
{
    ...
    $sql = "insert into plt_pelatihan_rab (idPelatihan, idRab, judulRab, jumlahRab, satuanRab, satuanPengalih, satuanHarga, hargaPengalih, hargaRab, nilaiRab, keteranganRab, statusRab, createBy, createTime) values ('$par[idPelatihan]', '$idRab', '$inp[judulRab]', '" . setAngka($inp[jumlahRab]) . "', '$inp[satuanRab]', '$inp[satuanPengalih]', '$inp[satuanHarga]', '" . setAngka($inp[hargaPengalih]) . "', '" . setAngka($inp[hargaRab]) . "', '" . setAngka($inp[nilaiRab]) . "', '$inp[keteranganRab]', 't', '$cUsername', '" . date('Y-m-d H:i:s') . "')";
    db($sql);
...
function approval()
{
    ...
    $res = db($sql);
    $r = mysql_fetch_array($res);
    if (empty($r[approveBy])) $r[approveBy] = $cUsername;
    if (empty($r[approveTime]) || $r[approveTime] == "0000-00-00 00:00:00") $r[approveTime] = date("Y-m-d H:i:s");
    ...
    if ($r[statusPelatihan] == "f") $statusPelatihan = "Tolak";
    if ($r[statusPelatihan] == "p") $statusPelatihan = "Tunda";
      ...
      <div id=\"contentwrapper\" class=\"contentwrapper\">
       <form action=\"\" method=\"post\" id=\"form2\" class=\"stdform\" onsubmit=\"return false;\">
        ...
        <div class=\"widgetbox\">
          ...
          <p style=\"position: absolute; right: 30px; top: 250px;\">
            <input type=\"submit\" class=\"submit radius2\" name=\"btnSimpan\" value=\"Simpan\" style=\"float:right;\" />
          </p>
...
function getContent($par)
{
    ...
    switch ($par[mode]) {
        ...
        case "detail_peserta_biaya":
            $text = $_submit ? proses_akademik_biaya() : detail_peserta_biaya();
            break;
        case "app":
            $text = approval();
            break;
