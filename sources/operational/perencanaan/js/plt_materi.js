...
function getKodeSite(getPar){
    modul = document.getElementById('inp[id_modul_materi]');
    kategori_level = document.getElementById('inp[id_kategori_materi]');
        ...
        if(xmlHttp.readyState == 4 && xmlHttp.status==200){         
            ...
            if(xmlHttp.responseText){
                ...
                for(var i=0; i<arr.length; i++){                            
                    ...
                    jQuery("#inp\\[id_kategori_materi\\]").trigger("chosen:updated");
                }
            }
        }
    }
...
function getKodeMenu(getPar){
    kategori_level = document.getElementById('inp[id_kategori_materi]');
    menu = document.getElementById('inp[id_program_materi]');
    var xmlHttp = getXMLHttp();     
    xmlHttp.onreadystatechange = function(){    
        if(xmlHttp.readyState == 4 && xmlHttp.status==200){         
            ...
            if(xmlHttp.responseText){
                ...
                for(var i=0; i<arr.length; i++){                            
                    ...
                    jQuery("#inp\\[id_program_materi\\]").trigger("chosen:updated");
                }
            }
        }
    }
