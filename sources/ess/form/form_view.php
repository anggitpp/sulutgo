...
function lihat(){
	...
	if(empty($par[empType])) $par[empType] = trim(strtolower($arrParam[$s]));
	...
	if(in_array($par[empType], array('kg')))
	$text.="<div style=\"position:absolute; top:0; right:0; margin-right:20px; margin-top:10px;\">
			".comboData("select * from pay_jenis where statusJenis='t' order by urutanJenis","idJenis","namaJenis","par[idJenis]","ALL",$par[idJenis], "onchange=\"document.getElementById('form').submit();\"", "225px", "chosen-select")."
			</div>";
	$text.="<table style=\"width:100%\">
			...
			<td style=\"width:50%; text-align:left; vertical-align:top;\">
					...
					<div id=\"dFilter\" style=\"visibility:collapse;\">
						<p>
							<label class=\"l-input-small\" style=\"width:200px; text-align:left; padding-left:10px;\">PANGKAT</label>
						...
						<p>
							<label class=\"l-input-small\" style=\"width:200px; text-align:left; padding-left:10px;\">REGION</label>
							<div class=\"field\" style=\"margin-left:200px;\">
								".comboData("SELECT kodeData, namaData FROM mst_data WHERE statusData='t' AND kodeCategory = 'RG' ORDER BY urutanData", "kodeData", "namaData", "nSearch", "--REGION--", $_GET['nSearch'], "", "250px", "chosen-select")."
							</div>
						</p>
						<p>
      ...
      //                  $text .= $cutil->generateSelectChainedWithOption($sql, "id", "description", "kodeInduk", "aSearch", $_GET['aSearch'], "", "class='chosen-select' style=\"width: 250px\"");
						...
						<p>
							<label class=\"l-input-small\" style=\"width:200px; text-align:left; padding-left:10px;\">".strtoupper($arrParameter[57])."</label>
							<div class=\"field\" style=\"margin-left:200px;\">
			 					".comboData("SELECT kodeData, namaData FROM mst_data WHERE statusData='t' AND kodeCategory = 'X08' ORDER BY urutanData", "kodeData", "namaData", "lSearch", "--".strtoupper($arrParameter[57])."--", $_GET['lSearch'], "", "250px", "chosen-select")."
							</div>
						</p>
					</div>
					</fieldset>
			...
			jQuery(document).ready(function(){
				jQuery(\"#btnExpExcel\").click(function (e) {
					...
					var nSearch = jQuery(\"#nSearch\").val();
					...
					window.open(sajax + '&json=excel&params=' + fSearch + '~' + bSearch + '~' + tSearch + '~' + nSearch + '~' + dirSearch + '~' + pSearch + '~' + mSearch + '~' + gSearch + '~' + lSearch, '_blank');
				});
	...
	function lData(){
		...
		if(!empty($par[idJenis]))
			$sWhere.= " and payroll_id='".$par[idJenis]."'";
		...
		if (!empty($_GET['fSearch']))
			...
			$sql="select *, replace(
				...
				END,' 0 bln','') masaKerja, replace(
				case when coalesce(end_date,NULL) IS NULL or end_date='0000-00-00' or end_date='' THEN
				CONCAT(TIMESTAMPDIFF(YEAR,  start_date, CURRENT_DATE ),' thn ', TIMESTAMPDIFF(MONTH, start_date,  CURRENT_DATE ) % 12, ' bln')
				ELSE
				CONCAT(TIMESTAMPDIFF(YEAR,  start_date, end_date),' thn ', TIMESTAMPDIFF(MONTH, start_date,  end_date) % 12, ' bln')
				END,' 0 bln','') masaJabatan from dta_pegawai $sWhere order by $orderBy $sLimit";
				$res=db($sql);
				...
				while($r=mysql_fetch_array($res)){
					...
					$start_date=getField("select min(start_date) from emp_phist where parent_id='".$r[id]."' and grade='".$r[grade]."' and start_date!='0000-00-00' and start_date is not null");
