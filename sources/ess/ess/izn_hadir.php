...
function form()
{
    ...
    if (empty($r[idPengganti]) || empty($r[idAtasan])) {
        list($idPengganti, $idAtasan) = explode("\t", getField("select concat(replacement_id, '\t', leader_id) from emp_phist where parent_id='" . $r[idPegawai] . "' AND status = '1'"));
    ...
    <div class="contentwrapper">
        <form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>" onsubmit="return validation(document.form);" enctype="multipart/form-data">
            ...
            <input type="hidden" name="inp[idPegawai]" id="inp[idPegawai]" value="<?= $r[idPegawai] ?>">
            <br clear="all" />
            <div id="general">
                <table width="100%">
                    <tr>
                        <td width="50%">
                            <p><?= $ui->createField("Nomor", "inp[nomorHadir]", $r[nomorHadir], "", "t", "", "", "", "t") ?></p>
                            <p><?= $ui->createField("Nama", "inp[nikPegawai]", $r_[nikPegawai], "", "t", "", "", "", "t") ?></p>
                            <p><?= $ui->createField("NPP", "inp[namaPegawai]", $r_[namaPegawai], "", "t", "", "", "", "t") ?></p>
                        </td>
                        <td width="50%">
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            <tbody>
                <?php
                    ...
                    if (!empty($cID))
                        $filter .= " and t1.idPegawai='" . $cID . "'";
                    if (!empty($par[filter]))
                        $filter .= " and (lower(t1.nomorHadir) like '%" . strtolower($par[filter]) . "%')";
