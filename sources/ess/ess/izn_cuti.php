		...
		<form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>" onsubmit="return validation(document.form);" enctype="multipart/form-data">
			...
			<div id="general">
				<table width="100%">
					<tr>
						<td width="50%">
							<p><?= $ui->createField("Nomor", "inp[nomorCuti]", $r[nomorCuti]) ?></p>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
			...
			<tbody>
				<?php
					...
					if (!empty($par[filter]))
						$filter .= " and (lower(t1.nomorCuti) like '%" . strtolower($par[filter]) . "%')";

					$arrTipe = arrayQuery("select idCuti, namaCuti from dta_cuti order by idCuti");
					$sql = "select t1.* from att_cuti t1 left join emp t2 on (t1.idPegawai=t2.id) $filter order by t1.nomorCuti";
					$res = db($sql);
