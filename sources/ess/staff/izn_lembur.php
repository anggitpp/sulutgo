...
function form(){
    ...
    <div class=\"contentwrapper\">
        ...
        if(isset($menuAccess[$s]["add"])) 
            $text.="<a href=\"#\" class=\"btn btn1 btn_document\" style=\"position:absolute; top:7px; margin-top:5px; right:0; margin-right:15px; z-index:99;\" onclick=\"openBox('popup.php?par[mode]=peg".getPar($par,"mode")."',1150,550);\"><span>Tambah Data</span></a>";
            $text.="
        <style>
...
function lihat(){
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form method="post" class="stdform">
            ...
            <div id=\"pos_r\">";
                if(isset($menuAccess[$s]["add"])) 
                    $text.="<a href=\"?par[mode]=add".getPar($par,"mode,idLembur")."\" class=\"btn btn1 btn_document\"><span>Tambah Data</span></a>";
                $text.="
            </div>
        </form>
        <br clear=\"all\" />
            ...
            while($r=mysql_fetch_array($res)){			
                ...
                <tr>
                    ...
                    if(isset($menuAccess[$s]["edit"]) || isset($menuAccess[$s]["delete"])){
                        $text.="
                        <td align=\"center\">";
                        if(isset($menuAccess[$s]["edit"])&&$r[persetujuanLembur]!='t') $text.="<a href=\"?par[mode]=edit&par[idGroup]=$r[idGroup]".getPar($par,"mode,idGroup")."\" title=\"Edit Data\" class=\"edit\"><span>Edit</span></a>";				
                        if(isset($menuAccess[$s]["delete"])) $text.="<a href=\"?par[mode]=del&par[idGroup]=$r[idGroup]".getPar($par,"mode,idGroup")."\" onclick=\"return confirm('are you sure to delete data ?');\" title=\"Delete Data\" class=\"delete\"><span>Delete</span></a>";
                        $text.="
                        </td>";
                    }
                    $text.="
...
function pegawai(){
    global $db, $par, $cID, $inp;

    $arrChk = explode("\n", $inp[dtaPegawai]);
    if(is_array($arrChk)){
        foreach ($arrChk as $key => $value) {
            list($idPegawai, $nikPegawai, $namaPegawai) = explode("\t", trim($value));
            $arrPeg[$idPegawai] = $nikPegawai;
        }
    }
    ...
    <div class=\"centercontent contentpopup\">
        ...
        <form id=\"form\" action=\"\" method=\"post\" class=\"stdform\">
            <textarea id=\"inp[dtaPegawai]\" name=\"inp[dtaPegawai]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"display:none;\" >$inp[dtaPegawai]</textarea>
            <div style=\"position:absolute; top:0; right:0; margin-right:15px; margin-top:10px; z-index:99;\">						
                <input type=\"text\" id=\"par[filterNama]\" placeholder=\"Nama atau NPP..\" name=\"par[filterNama]\" style=\"width:250px;\" value=\"$par[filterNama]\" class=\"mediuminput\" />
            </div>
            <div style=\"position:absolute; right: -10\">			
                ".setPar($par, "filterNama")."
                <input type=\"submit\" value=\"GO\" class=\"btn btn_search btn-small\"/>
                <a href=\"#\" class=\"btn btn1 btn_document\" onclick=\"setPegawai();\"><span>Tambahkan</span></a>
            </div>	
        </form>
        <br clear=\"all\">
        
        <table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dynscroll_y\" >
            <thead>
                <tr>
                    <th width=\"20\">No.</th>
                    <th width=\"100\">NPP</th>
                    ...
                    <th width=\"20\"><input id=\"chkAll\" name=\"chkAll\" type=\"checkbox\" onclick=\"getAll();\"></th>
                </tr>
            </thead>
            <tbody id=\"chkPersonil\">";
            if(!empty($par[filterNama]))		
                $filter.= " and (lower(t1.name) like '%".strtolower($par[filterNama])."%' or (t1.reg_no) like '%".strtolower($par[filterNama])."%')";    
            
            $sql="select *,t1.id as id, t2.id as ids from ".$db['hrms'].".emp t1 join ".$db['hrms'].".emp_phist t2 on t1.id = t2.parent_id where t2.status = '1' AND t1.status = '535' $filter order by t1.name";
            $row=0;	
            $res=db($sql);
            while($r=mysql_fetch_array($res)){	
                
                $checked = isset($arrPeg[$r[id]]) ? "checked=\"checked\"" : "";
                ...
                <tr>
                    ...
                    <input type=\"checkbox\" id=\"chk_".$row."\" name=\"inp[".$r[id]."]\" value=\"".$r[id]."\t".$r[reg_no]."\t".strtoupper($r[name])."\t".$r[pos_name]."\" onclick=\"getPegawai(this);\" />
                    </td>
                </tr>";
                $row++;
            }			
