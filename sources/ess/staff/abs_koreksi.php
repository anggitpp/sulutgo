...
function gPegawai()
{
    ...
    $sql = "select pos_name, location, div_id from emp_phist where parent_id='" . $par[idPegawai] . "' AND status = '1'";
    $res = db($sql);
...
function hapus()
{
    ...
    echo "<script>window.location='?" . getPar($par, "mode,idKoreksi, idPegawai") . "';</script>";
}
...
function approve()
{
    ...
    $sql = "UPDATE att_koreksi SET persetujuanKoreksi = '$inp[persetujuanKoreksi]', catatanKoreksi = '$inp[catatanKoreksi]', approveBy = '$inp[approveBy]', approveTime = '$inp[approveTime]' WHERE idKoreksi = '$par[idKoreksi]'";
    db($sql);

    echo "<script>alert('DATA BERHASIL DI APPROVE');window.location='?" . getPar($par, "mode,idKoreksi") . "';</script>";
}
...
function sdm()
{
    ...
    $sql = "UPDATE att_koreksi SET sdmKoreksi = '$inp[sdmKoreksi]', noteKoreksi = '$inp[noteKoreksi]', sdmBy = '$inp[sdmBy]', sdmTime = '$inp[sdmTime]' WHERE idKoreksi = '$par[idKoreksi]'";
    db($sql);
...
function form()
{
    ...
    setValidation("is_null", "inp[idPegawai]", "anda harus mengisi Pegawai");
    setValidation("is_null", "inp[tanggalKoreksi]", "anda harus mengisi Tanggal Koreksi");
    setValidation("is_null", "inp[mulaiKoreksi_tanggal]", "anda harus mengisi Tanggal Koreksi");
    echo getValidation();
        ...
        <form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>" onsubmit="return validation(document.form);" enctype="multipart/form-data">
            ...
            <div id="general">
                <table width="100%">
                    <tr>
                        <td width="50%">
                            ...
                            <p><?= $ui->createComboData("NPP - Nama", $queryPegawai, "id", "description", "inp[idPegawai]", $r[idPegawai], "onchange=\"getPegawai('" . getPar($par, "mode") . "');\"", "305px", "t", "t", "t") ?></p>
                            <p><?= $ui->createField("Lokasi Kerja", "inp[namaLokasi]", $r_[namaLokasi], "", "t", "", "", "", "t") ?></p>
                        </td>
                        <td width="50%">
                ...
                <?php if ($par[mode] == "app") { ?>
                    ...
                    <p><?= $ui->createRadio("Status", "inp[persetujuanKoreksi]", array("t" => "Disetujui", "f" => "Ditolak"), $r[persetujuanKoreksi]) ?></p>
                    <p><?= $ui->createTextArea("Keterangan", "inp[catatanKoreksi]", $r[catatanKoreksi]) ?></p>
                <?php }
                    if ($par[mode] == "sdm") { ?>
                    <div class="widgetbox">
                        <div class="title">
                    ...
                    <p><?= $ui->createRadio("Status", "inp[sdmKoreksi]", array("t" => "Disetujui", "f" => "Ditolak"), $r[sdmKoreksi]) ?></p>
                    <p><?= $ui->createTextArea("Keterangan", "inp[noteKoreksi]", $r[noteKoreksi]) ?></p>
                <?php } ?>
            </div>
        </form>
    </div>
...
function lihat()
{
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form method="post" class="stdform">
            ...
            <div id="pos_r">
                <?php if (isset($menuAccess[$s]["add"])) ?><a href="?par[mode]=add<?= getPar($par, 'mode,idKoreksi, tahunKoreksi') ?>" class="btn btn1 btn_document"><span>Tambah Data</span></a>
            </div>
        </form>
        <br clear="all" />
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            <thead>
                <tr>
                    ...
                    <th style="vertical-align:middle" rowspan="2" style="*">Nama</th>
                    <th style="vertical-align:middle" rowspan="2" style="100">NPP</th>
                    <th style="vertical-align:middle" rowspan="2" style="75">Nomor</th>
                    <th style="vertical-align:middle" rowspan="2" width="75">Tanggal</th>
                    <th colspan="3" width="175">Koreksi</th>
                    <th colspan="2" width="100">Approval</th>
                    <th style="vertical-align:middle" rowspan="2" width="50">Dokumen</th>
                    <th style="vertical-align:middle" rowspan="2" width="50">Print</th>
                    <th style="vertical-align:middle" rowspan="2" width="50">Kontrol</th>
                </tr>
                <tr>
            ...
            <tbody>
                <?php
                    ...
                    if (!empty($par[filter]))
                        $filter .= " and (lower(t1.nomorKoreksi) like '%" . strtolower($par[filter]) . "%' OR lower(t2.reg_no) like '%" . strtolower($par[filter]) . "%' OR lower(t2.name) like '%" . strtolower($par[filter]) . "%')";

                    $sql = "select t1.*, t2.name, t2.reg_no from att_koreksi t1 left join emp t2 on (t1.idPegawai=t2.id) join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t1.nomorKoreksi";
                    $res = db($sql);
                    $no = 0;
                    while ($r = mysql_fetch_array($res)) {
                        ...
                        $control = "<a href=\"?par[mode]=det&par[idKoreksi]=$r[idKoreksi]" . getPar($par, "mode,idKoreksi") . "\" title=\"Detail Data\" class=\"detail\"><span>Detail</span></a>";

                        $download = empty($r[fileKoreksi]) ? "" : "<a href=\"download.php?d=essKoreksi&f=$r[idKoreksi]\"><img src=\"" . getIcon($r[fileKoreksi]) . "\" align=\"center\" style=\"padding-right:5px; padding-bottom:5px; max-width:50px; max-height:50px;\"></a>";

                        $linkAtasan = "<a href=\"?par[mode]=app&par[idKoreksi]=$r[idKoreksi]" . getPar($par, "mode,idKoreksi") . "\">$persetujuanKoreksi</a>";
                        $linkSdm = isset($menuAccess[$s]["apprlv2"]) ? "<a href=\"?par[mode]=sdm&par[idKoreksi]=$r[idKoreksi]" . getPar($par, "mode,idKoreksi") . "\">$sdmKoreksi</a>" : $sdmKoreksi;
                    ...
                    <tr>
                        ...
                        <td align="center">
                            <a href="#" title="Print Data" class="print" onclick="openBox('ajax.php?par[mode]=print&par[id]=<?= $r[idKoreksi] . getPar($par, 'mode') ?>',1200,600);"><span>Print</span></a>
                        </td>
