<?php
	...
	function lihat(){
		...
		$text.="<div class=\"pageheader\">
			...
			<thead>
				<tr>
					...
					<th width=\"75\">Tanggal Selesai</th>
		...
		$sql="select * from sup_tiket t1 join app_user t2 join mst_data t3 join sup_status t4 on (t1.createBy=t2.username and t1.idPrioritas=t3.kodeData and t1.idTiket=t4.idTiket) $filter order by t1.tanggalTiket";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			...
			$text.="<tr>
					...
					<td>$r[namaUser]</td>
					<td>$r[namaData]</td>
					<td align=\"center\">".getTanggal($r[selesaiTiket])."</td>
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'REKAP TROUBLE TICKET');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('G4', 'TANGGAL SELESAI');
		...
		while($r=mysql_fetch_array($res)){
			...
			$objPHPExcel->getActiveSheet()->getStyle('H'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB($tesStatus);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[namaData]);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, getTanggal($r[selesaiTiket]));			
