<?php
	...
	function ubah(){
		...
		if(!getField("select disetujuiBy from sup_status where idTiket='$par[idTiket]'")) $disetujuiBy = ", disetujuiBy='$cUsername', disetujuiTime='".date('Y-m-d H:i:s')."'";
					
		$sql="update sup_status set disetujuiTanggal='".setTanggal($inp[disetujuiTanggal])."', disetujuiStatus='$inp[disetujuiStatus]', disetujuiKeterangan='$inp[disetujuiKeterangan]' ".$disetujuiBy.", updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where idTiket='$par[idTiket]'";
		db($sql);
	...
	function form(){
		...
		$sql="select t1.*, t2.namaUser from sup_status t1 left join app_user t2 on (t1.disetujuiBy=t2.username) where idTiket='$par[idTiket]'";
		$res=db($sql);
		$r=mysql_fetch_array($res);
								
		if(empty($r[namaUser])) $r[namaUser]=getField("select namaUser from app_user where username='".$cUsername."'");		
		if(empty($r[disetujuiTanggal]) || $r[disetujuiTanggal]=="0000-00-00") $r[disetujuiTanggal]=date('Y-m-d');
		
		$diperiksaStatus = $r[diperiksaStatus] == "f" ? "Tidak Setuju" : "Setuju";
		
		$selesai =  $r[disetujuiStatus] == "f" ? "checked=\"checked\"" : "";		
		$dikerjakan = empty($selesai) ? "checked=\"checked\"" : "";			
				
		setValidation("is_null","disetujuiTanggal","anda harus mengisi tanggal");				
		setValidation("is_null","inp[disetujuiKeterangan]","anda harus mengisi keterangan");		
		$text = getValidation();
		...
		$text.="<div class=\"pageheader\">
					...
					<ul class=\"hornav\">
						<li class=\"current\"><a href=\"#disetujui\">Disetujui</a></li>
		...
		#TAB DISETUJUI
		$text.="<div id=\"disetujui\" class=\"subcontent\">
					<p>
						<label class=\"l-input-small\">Tanggal</label>
						<div class=\"field\">
							<input type=\"text\" id=\"disetujuiTanggal\" name=\"inp[disetujuiTanggal]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[disetujuiTanggal])."\" class=\"vsmallinput hasDatePicker\"/>	
						</div>
					</p>
					<p>
					...
					<p>
						...
						<div class=\"fradio\">
							<input type=\"radio\" id=\"dikerjakan\" name=\"inp[disetujuiStatus]\" value=\"t\" ".$dikerjakan." /> <span class=\"sradio\">Dikerjakan</span>
							<input type=\"radio\" id=\"selesai\" name=\"inp[disetujuiStatus]\" value=\"f\" ".$selesai." /> <span class=\"sradio\">Selesai</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
						<div class=\"field\">
							<textarea id=\"inp[disetujuiKeterangan]\" name=\"inp[disetujuiKeterangan]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:75px; width:360px;\">$r[disetujuiKeterangan]</textarea>
						</div>
					</p>
				</div>";
	...
	function lihat(){
		...
		$filter = "where t1.tanggalTiket between '".setTanggal($par[mulaiTiket])."' and '".setTanggal($par[selesaiTiket])."' and t5.diperiksaStatus='t'";
		...
		while($r=mysql_fetch_array($res)){
			...
			$disetujuiTanggal = (empty($r[disetujuiTanggal]) || $r[disetujuiTanggal]=="0000-00-00") ? "<img src=\"styles/images/f.png\">" : getTanggal($r[disetujuiTanggal]);
			
			$disetujuiStatus = "<img src=\"styles/images/p.png\" title=\"Belum Diproses\">";
			if($r[disetujuiStatus] == "t") $disetujuiStatus = "<img src=\"styles/images/t.png\" title=\"Setuju\">";
			if($r[disetujuiStatus] == "f") $disetujuiStatus = "<img src=\"styles/images/f.png\" title=\"Tidak Setuju\">";
			...
			$text.="<tr>
					...
					<td align=\"center\">";				
			$text.= isset($menuAccess[$s]["edit"]) ? "<a href=\"?par[mode]=edit&par[idTiket]=$r[idTiket]".getPar($par,"mode,idTiket")."\" title=\"Persetujuan Tiket\" >".$disetujuiTanggal."</a>" : $disetujuiTanggal;
			$text.="</td>
