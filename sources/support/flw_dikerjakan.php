<?php
	...
	function ubah(){
		...
		if(!getField("select dikerjakanBy from sup_status where idTiket='$par[idTiket]'")) $dikerjakanBy = ", dikerjakanBy='$cUsername', dikerjakanTime='".date('Y-m-d H:i:s')."'";
		$dikerjakanTanggal = $inp[dikerjakanStatus] == "t" ? "0000-00-00" : date('Y-m-d');
			
		$sql="update sup_status set dikerjakanTanggal='".$dikerjakanTanggal."', dikerjakanMulai='".setTanggal($inp[dikerjakanMulai])."', dikerjakanSelesai='".setTanggal($inp[dikerjakanSelesai])."', dikerjakanStatus='$inp[dikerjakanStatus]', dikerjakanKeterangan='$inp[dikerjakanKeterangan]' ".$dikerjakanBy.", updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where idTiket='$par[idTiket]'";
		db($sql);
	...
	function form(){
		...
		$sql="select t1.*, t2.namaUser from sup_status t1 left join app_user t2 on (t1.dikerjakanBy=t2.username) where idTiket='$par[idTiket]'";
		$res=db($sql);
		$r=mysql_fetch_array($res);
		
		if(empty($r[namaUser])) $r[namaUser]=getField("select namaUser from app_user where username='".$cUsername."'");		
		if(empty($r[dikerjakanMulai]) || $r[dikerjakanMulai]=="0000-00-00") $r[dikerjakanMulai]=date('Y-m-d');
		if(empty($r[dikerjakanSelesai]) || $r[dikerjakanSelesai]=="0000-00-00") $r[dikerjakanSelesai]=date('Y-m-d');
		
		$diperiksaStatus = $r[diperiksaStatus] == "f" ? "Tidak Setuju" : "Setuju";
		$disetujuiStatus = $r[diperiksaStatus] == "f" ? "Selesai" : "Dikerjakan";
		
		$selesai =  $r[dikerjakanStatus] == "f" ? "checked=\"checked\"" : "";		
		$proses = empty($selesai) ? "checked=\"checked\"" : "";			
				
		setValidation("is_null","dikerjakanMulai","anda harus mengisi tanggal mulai");
		setValidation("is_null","dikerjakanSelesai","anda harus mengisi perkiraan selesai");
		setValidation("is_null","inp[dikerjakanKeterangan]","anda harus mengisi keterangan");		
		$text = getValidation();
		...
		$text.="<div class=\"pageheader\">
					...
					<ul class=\"hornav\">
						<li class=\"current\"><a href=\"#dikerjakan\">Dikerjakan</a></li>
		...
		#TAB DIKERJAKAN
		$text.="<div id=\"dikerjakan\" class=\"subcontent\">
					<p>
						<label class=\"l-input-small\">Tanggal Mulai</label>
						<div class=\"field\">
							<input type=\"text\" id=\"dikerjakanMulai\" name=\"inp[dikerjakanMulai]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[dikerjakanMulai])."\" class=\"vsmallinput hasDatePicker\"/>	
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Perkiraan Selesai</label>
						<div class=\"field\">
							<input type=\"text\" id=\"dikerjakanSelesai\" name=\"inp[dikerjakanSelesai]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[dikerjakanSelesai])."\" class=\"vsmallinput hasDatePicker\"/>	
						</div>
					</p>
					<p>
					...
					<p>
						...
						<div class=\"fradio\">
							<input type=\"radio\" id=\"proses\" name=\"inp[dikerjakanStatus]\" value=\"t\" ".$proses." /> <span class=\"sradio\">Proses</span>
							<input type=\"radio\" id=\"selesai\" name=\"inp[dikerjakanStatus]\" value=\"f\" ".$selesai." /> <span class=\"sradio\">Selesai</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
						<div class=\"field\">
							<textarea id=\"inp[dikerjakanKeterangan]\" name=\"inp[dikerjakanKeterangan]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:75px; width:360px;\">$r[dikerjakanKeterangan]</textarea>
						</div>
					</p>
				</div>";
	...
	function lihat(){
		...
		$filter = "where t1.tanggalTiket between '".setTanggal($par[mulaiTiket])."' and '".setTanggal($par[selesaiTiket])."' and t4.disetujuiStatus='t'";
		...
		while($r=mysql_fetch_array($res)){
			...
			$dikerjakanMulai = (empty($r[dikerjakanMulai]) || $r[dikerjakanMulai]=="0000-00-00") ? "<img src=\"styles/images/f.png\">" : getTanggal($r[dikerjakanMulai]);
			
			$dikerjakanStatus = "<img src=\"styles/images/f.png\" title=\"Belum Dikerjakan\">";
			if($r[dikerjakanStatus] == "t") $dikerjakanStatus = "<img src=\"styles/images/p.png\" title=\"Proses\">";
			if($r[dikerjakanStatus] == "f") $dikerjakanStatus = "<img src=\"styles/images/t.png\" title=\"Selesai\">";
			...
			$text.="<tr>
					...
					<td align=\"center\">";				
			$text.= isset($menuAccess[$s]["edit"]) ? "<a href=\"?par[mode]=edit&par[idTiket]=$r[idTiket]".getPar($par,"mode,idTiket")."\" title=\"Pengerjaan Tiket\" >".$dikerjakanMulai."</a>" : $dikerjakanMulai;
			$text.="</td>
