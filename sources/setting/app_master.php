<?php
	...
	function ubahDet(){
		...
		$sql="update mst_data set kodeInduk='$inp[kodeInduk]', namaData='$inp[namaData]', keteranganData='$inp[keteranganData]', urutanData='".setAngka($inp[urutanData])."', statusData='$inp[statusData]', updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where kodeData='$par[kodeData]'";
		db($sql);
	...
	function tambah(){
		...
		$sql="insert into app_menu (kodeMenu, kodeSite, kodeInduk, namaMenu, targetMenu, aksesMenu, iconMenu, urutanMenu, statusMenu, levelMenu, createBy, createTime) values ('$kodeMenu', '$par[kodeSite]', '$r[kodeMenu]', '$inp[namaCategory]', '$r[targetMenu]', '$r[aksesMenu]', '$iconMenu', '".setAngka($inp[urutanCategory])."', '$inp[statusCategory]', '$levelMenu', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
		
		$sql="insert into mst_category (kodeCategory, kodeSite, kodeInduk, kodeMenu, namaCategory, keteranganCategory, urutanCategory, statusCategory, createBy, createTime) values ('$inp[kodeCategory]', '$par[kodeSite]', '$inp[kodeInduk]', '$kodeMenu', '$inp[namaCategory]', '$inp[keteranganCategory]', '".setAngka($inp[urutanCategory])."', '$inp[statusCategory]', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
		echo "<script>closeBox();reloadPage();</script>";
	}
	...
	function form(){
		...
		if(empty($r[urutanCategory])) $r[urutanCategory] = getField("select urutanCategory from mst_category where kodeSite='".$par[kodeSite]."' order by urutanCategory desc limit 1") + 1;
		...
		$text.="<div class=\"centercontent contentpopup\">
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<p>
						...
						<div class=\"field\">
							".comboData("select * from mst_category where kodeSite='".$par[kodeSite]."' and kodeCategory!='$par[kodeCategory]' and statusCategory='t' order by urutanCategory","kodeCategory","namaCategory","inp[kodeInduk]"," ",$r[kodeInduk],"", "360px")."
						</div>
					</p>
					<p>
	...
	function lihat(){
		...
		if(empty($par[kodeSite])) $par[kodeSite] = $c == 1 ? 1 : $c;
		$text.="<div class=\"pageheader\">
				<h1 class=\"pagetitle\">".$arrTitle[$s]."</h1>			
				".getBread()."
			...
			<div style=\"position:absolute; right:0px; margin-right:20px; margin-top:-65px;\">";
		
		if($c == 1)
		$text.="<div style=\"float:left; margin-right:5px;\">
			Modul : ".comboData("select * from app_site where statusSite='t' order by urutanSite","kodeSite","namaSite","par[kodeSite]","",$par[kodeSite],"onchange=\"document.getElementById('form').submit();\"")."
		...
		if(isset($menuAccess[$s]["add"]) && $par[kodeSite] == $c) $text.="<a href=\"#Add\" class=\"btn btn1 btn_document\" onclick=\"openBox('popup.php?par[mode]=add".getPar($par,"mode,kodeCategory")."',825,500);\" style=\"margin-top:0px;\"><span>New Master</span></a>";
		$text.="</div>
			<div id=\"pos_l\" style=\"float:left;\">
			<p>
				<span>Search : </span>
	...
	function icon(){
		...
		$filter ="where t1.kodeSite='".$par[kodeSite]."'";
