...
function hapus(){
	...
	$sql="delete from app_group_menu t1 join app_menu t2 on t2.kodeMenu = t1.kodeMenu where t2.kodeModul='$par[kodeModul]'";
	db($sql);
...
function ubah(){
	...
	$sql="update app_modul set namaModul='$inp[namaModul]', folderModul='$inp[folderModul]', iconModul='$iconModul', urutanModul='".setAngka($inp[urutanModul])."', statusModul='$inp[statusModul]', updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where kodeModul='$par[kodeModul]'";
	db($sql);
...
function tambah(){
	...
	$sql="insert into app_modul (kodeModul, namaModul, folderModul, iconModul, urutanModul, statusModul, createBy, createTime) values ('$kodeModul', '$inp[namaModul]', '$inp[folderModul]', '$iconModul', '".setAngka($inp[urutanModul])."', '$inp[statusModul]', '$cUsername', '".date('Y-m-d H:i:s')."')";
	db($sql);
	...
	foreach($arrSubSettingMenu as $objSubSettingMenu){
		...
		foreach($objSubSettingMenu[childMenu] as $objChildMenu){
			...
			$sql = "INSERT INTO app_menu (kodeMenu, kodeModul, kodeSite, kodeInduk, namaMenu, targetMenu, parameterMenu, aksesMenu, iconMenu, urutanMenu, statusMenu, levelMenu, createBy, createTime) VALUES ('$kodeMenu2', '$kodeModul', '$kodeSite', '$kodeMenu', '$objChildMenu[namaMenu]', '$objChildMenu[targetMenu]', '', '$objChildMenu[aksesMenu]', '$objChildMenu[iconMenu]', '$objChildMenu[urutanMenu]', 't', '2', '$cUsername', '".date("Y-m-d H:i:s")."')";
			// echo $sql."<br><br>";
			db($sql);
...
function form(){
	...
	if(empty($r[urutanModul])) $r[urutanModul] = getField("select urutanModul from app_modul order by urutanModul desc limit 1") + 1;
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" action=\"\" method=\"post\" class=\"stdform\">
			...
			<div id=\"pos_r\">";
				if(isset($menuAccess[$s]["add"])) $text.="<a href=\"#Add\" class=\"btn btn1 btn_document\" onclick=\"openBox('popup.php?par[mode]=add".getPar($par,"mode,kodeModul")."',825,450);\"><span>Add Data</span></a>";
				$text.="</div>
			</form>
			<br clear=\"all\" />
					...
					<tbody>";
						...
						while($r=mysql_fetch_array($res)){			
							...
							if(isset($menuAccess[$s]["edit"]) || isset($menuAccess[$s]["delete"])){
								...
								if(isset($menuAccess[$s]["edit"])) $text.="<a href=\"#Edit\" title=\"Edit Data\" class=\"edit\"  onclick=\"openBox('popup.php?par[mode]=edit&par[kodeModul]=$r[kodeModul]".getPar($par,"mode,kodeModul")."',825,450);\"><span>Edit</span></a>";
								if(isset($menuAccess[$s]["delete"])) $text.="<a href=\"#Delete\" onclick=\"del('$r[kodeModul]','".getPar($par,"mode,kodeModul")."')\" title=\"Delete Data\" class=\"delete\"><span>Delete</span></a>";
								$text.="</td>";
							}
							$text.="</tr>";
						}	
