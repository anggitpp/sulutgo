...
if($cUsername == $sUser){
	$arrSite = arrayQuery("select kodeSite, namaSite from app_site where statusSite='t' and kodeModul='$kodeModul' order by urutanSite");
...
function ubah() {
  ...
  $arrMenu = arrayQuery("select t1.kodeMenu, t2.kodeMenu from app_menu t1 join app_group_menu t2 on (t1.kodeMenu=t2.kodeMenu) where t1.kodeModul='$kodeModul' and t2.kodeGroup='$cGroup'");
  if (is_array($arrMenu)) {    
...
function sinkronisasi(){
	global $s,$db,$inp,$par,$kodeModul,$arrTitle,$menuAccess;	
