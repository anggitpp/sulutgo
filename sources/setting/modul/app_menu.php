	...
	function lihat(){
		...
		if(is_array($arrMenu[0])){
			while(list($kodeMenu,$r)=each($arrMenu[0])){
				$no++;
				$fileMenu = empty($r[fileMenu]) ? " - " : "<a href=\"download.php?d=fileMenu&f=$r[kodeMenu]".getPar($par, "mode")."\"><img src=\"".getIcon($r[fileMenu])."\" height=\"20\"></a>";
				...
				if(empty($r[iconMenu])){
					$r[iconMenu] = "blank.png";
				}
				$text.="<tr>
						<td>$no.</td>
						<td align=\"center\"><img style='width:30px;' src=\"images/menu/".$r[iconMenu]."\"></td>
						<td style=\"padding-left:$paddingMenu;\">$r[namaMenu]</td>
						<td align=\"center\">$fileMenu</td>
