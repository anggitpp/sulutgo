<?php
	...
	function ubahDet(){
		...
		$sql="update mst_data set kodeMaster='$inp[kodeMaster]',kodeInduk='$inp[kodeInduk]', namaData='$inp[namaData]', keteranganData='$inp[keteranganData]', urutanData='".setAngka($inp[urutanData])."', statusData='$inp[statusData]', updateBy='$cUsername', updateTime='".date('Y-m-d H:i:s')."' where kodeData='$par[kodeData]'";
		db($sql);
	...
	function tambahDet(){
		...
		repField();
		$sql="insert into mst_data (kodeMaster, kodeData, kodeInduk, kodeMenu, kodeReport, kodeCategory, namaData, keteranganData, urutanData, statusData, createBy, createTime) values ('$inp[kodeMaster]','$kodeData', '$inp[kodeInduk]', '$kodeMenu', '$kodeReport', '$par[kodeCategory]', '$inp[namaData]', '$inp[keteranganData]', '".setAngka($inp[urutanData])."', '$inp[statusData]', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
		echo "<script>closeBox();reloadPage();</script>";
	}
	...
	function formDet(){
		...
		$text.="<div class=\"centercontent contentpopup\">
				...
				$text.="<p>
					...
					<p>
						<label class=\"l-input-small\">Kode Master</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[kodeMaster]\" name=\"inp[kodeMaster]\"  value=\"$r[kodeMaster]\" class=\"mediuminput\" style=\"width:50px; text-align:right;\" />
						</div>
					</p>
					<p>
	...
	function lihat(){
		...
		$text.="</div>
			<div id=\"pos_l\" style=\"float:left;\">
			<p>
				<input type=\"text\" id=\"par[filter]\" placeholder=\"Search..\" name=\"par[filter]\" size=\"50\" value=\"$par[filter]\" class=\"mediuminput\" />
