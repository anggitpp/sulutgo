function getNomor(getPar){
	tanggalGanti=document.getElementById("tanggalGanti");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorGanti]").value = xmlHttp.responseText;
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalGanti=" + tanggalGanti.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
