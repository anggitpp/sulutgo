...
function getJumlah(getPar){		
	...
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			...
			if(response){
				jatahCuti.value = response;
				pengambilanCuti = dateDiff(mulaiCuti.value, selesaiCuti.value, 'days') + 1;
	...
	xmlHttp.open("GET", "ajax.php?par[mode]=cut&idTipe=" + idTipe.value + "&tanggalCuti=" + tanggalCuti.value + "&nikPegawai=" + nikPegawai.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
