function getNomor(getPar){
	tanggalEkstra=document.getElementById("tanggalEkstra");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorEkstra]").value = xmlHttp.responseText;
			}
			getJumlah(getPar);
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalEkstra=" + tanggalEkstra.value + getPar, true);
	xmlHttp.send(null);
	return false;
}

function setPegawai(nikPegawai, getPar, getUrl){	
	parent.document.getElementById("inp[nikPegawai]").value = nikPegawai;	
	parent.getPegawai(getPar, getUrl);
	closeBox();
}

function getPegawai(getPar, getUrl){
	nikPegawai = document.getElementById("inp[nikPegawai]").value;
	tanggalEkstra = document.getElementById("tanggalEkstra").value;
	...
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			...
			if(response){				
				...
				window.parent.location = "?par[idPegawai]=" + data["idPegawai"] + getUrl;
			}			
		}
	}
	
	xmlHttp.open("GET", "ajax.php?par[mode]=get&par[nikPegawai]=" + nikPegawai + "&par[tanggalEkstra]=" + tanggalEkstra + getPar, true);
	xmlHttp.send(null);
	return false;
}
