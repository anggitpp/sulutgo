function getNomor(getPar){
	tanggalPindah=document.getElementById("tanggalPindah");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorPindah]").value = xmlHttp.responseText;
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalPindah=" + tanggalPindah.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
...
function getShift(getPar){
	idPegawai = document.getElementById("inp[idPegawai]").value;
	tanggalJadwal = document.getElementById("shiftPindah").value;
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			response = xmlHttp.responseText.trim();	
			if(response){				
				var data = JSON.parse(response);
				document.getElementById("inp[awalPindah]").value = data["idShift"] == undefined ? "" : data["idShift"];
				document.getElementById("awalPindah").value = data["namaShift"] == undefined ? "" : data["namaShift"];
			}
		}
	}
	
	xmlHttp.open("GET", "ajax.php?par[mode]=shift&par[idPegawai]=" + idPegawai + "&par[tanggalJadwal]=" + tanggalJadwal + getPar, true);
	xmlHttp.send(null);
	return false;
}