<?php
	...
	function lihat(){
		...
		".comboData("select t1.kodeData id, t1.namaData description, t1.kodeInduk from mst_data t1
			...
			<thead>
				<tr>
					...
					<th width=\"75\">NPP</th>												
					<th style=\"width:100px;\">Total Alpha</th>
				</tr>
			</thead>
			<tbody>";
		...
		/*
		$sql="select * from (
				select * from dta_jadwal t1 join dta_pegawai t2 on (t1.idPegawai = t2.id) where t1.tanggalJadwal<='".date('Y-m-d')."' and t2.location IN ( $areaCheck ) ".$filter." and t1.tanggalJadwal between '".setTanggal($par[tanggalAwal])."' and '".setTanggal($par[tanggalAkhir])."'
			) as d1 left join dta_absen d2 on (d1.tanggalJadwal between date(d2.mulaiAbsen) and date(d2.selesaiAbsen)  and d1.idPegawai=d2.idPegawai) where d1.idShift not in (select idShift from dta_shift where mulaiShift='00:00:00' and selesaiShift='00:00:00') and d2.idAbsen is null order by d1.tanggalJadwal";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			$arrAlpha["".$r[id].""]++;
			$arrData["".$r[id].""] = $r[reg_no]."\t".$r[name];
		}
		*/
		...
		if (is_array($arrAlpha)){
			arsort($arrAlpha);
			reset($arrAlpha);
			while(list($idPegawai, $cntAlpha) = each($arrAlpha)){		
				list($nikPegawai, $namaPegawai) = explode("\t", $arrData[$idPegawai]);
				$arrPegawai[] = $idPegawai;
				$text.="<tr>
					<td>$no.</td>
					<td>".$namaPegawai."</td>
					<td>".$nikPegawai."</td>									
					<td align=\"right\">".getAngka($cntAlpha)." Hari</td>
					</tr>";	
					$no++;
			}
		}

		$listPegawai = implode(", ", $arrPegawai);
		...
		$sql = "select * from dta_pegawai where id NOT IN($listPegawai)";
		$res = db($sql);
		while($r=mysql_fetch_array($res)){	
		...
		$res = db($sql);
		while($r=mysql_fetch_array($res)){	
			if(!empty($cntPegawai[$r[id]])){
					$no++;
					$text.="<tr>
					<td>$no.</td>
					<td>".strtoupper($r[name])."</td>
					<td>$r[reg_no]</td>
					<td align=\"right\">".$cntPegawai[$r[id]]." Hari</td>
					</tr>";
				}
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'REKAP ALPHA');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('D4', 'TOTAL ALPHA');
		...
		*/
		...
		if (is_array($arrAlpha)){
			...
			while(list($idPegawai, $cntAlpha) = each($arrAlpha)){		
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, getAngka($arrAlpha[$idPegawai])." Hari");				
		...
		while($r=mysql_fetch_array($res)){	
			if(!empty($cntPegawai[$r[id]])){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $cntPegawai[$r[id]]." Hari");				
