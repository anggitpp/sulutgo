...
function lihat()
{
    ...
    $queryKantor = "SELECT kodeData id, namaData description from mst_data where kodeCategory='X03' order by kodeInduk, urutanData";
    $queryUnit = "SELECT t1.kodeData id, t1.namaData description, t1.kodeInduk from mst_data t1 JOIN mst_data t2 ON t2.kodeData=t1.kodeInduk where t2.kodeCategory='X03' and t1.kodeInduk = '$par[idKantor]' order by t1.urutanData";
    ?>
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form id="form" action="" method="post" class="stdform">
            <div id="pos_l">
                <?= comboData($queryKantor, "id", "description", "par[idKantor]", "- Pilih Kantor -", $par[idKantor], "onchange=\"document.getElementById('form').submit();\"") ?>
                <?= comboData($queryUnit, "id", "description", "par[idUnit]", "- Pilih Unit/Cabang -", $par[idUnit], "onchange=\"document.getElementById('form').submit();\"") ?>
            </div>
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            $filter = !empty($par[idKantor]) ? " AND t1.kodeInduk = '$par[idKantor]'" : "";
            $filter.= !empty($par[idUnit]) ? " AND t1.kodeData = '$par[idUnit]'" : "";
            $arrUnitKerja = arrayQuery("SELECT t1.kodeData id, t1.namaData description from mst_data t1 JOIN mst_data t2 ON t2.kodeData=t1.kodeInduk where t2.kodeCategory='X03' $filter order by t1.urutanData");
...
//            debugVar($arrUnitKerja);

            if (is_array($arrUnitKerja)) {
                reset($arrUnitKerja);
                while (list($idUnitKerja, $namaUnitKerja) = each($arrUnitKerja)) {
                    $no++;
                    ?>
                    <tr>
                        <td><?= $no ?>.</td>
                        ...
                        if (is_array($arrStatus)) {
                            reset($arrStatus);
                            while (list($i, $statusAbsen) = each($arrStatus)) {
                                echo "<td align=\"center\">" . getAngka($arrAbsen[$arrMaster[$idUnitKerja]][$statusAbsen], 0, 5) . "</td>";
                            }
                        }
                        $persenAbsen = $arrShift[$idUnitKerja] > 0 ? $arrAbsen[$idUnitKerja][""] / $arrShift[$idUnitKerja] * 100 : 0;
                        ?>
                        <td align="center"><?= getAngka($persenAbsen) ?>%</td>
                        <td align="center"><a title="Detail Data" class="detail"
                                              href="?par[mode]=det&par[idDivisi]=<?= $idUnitKerja . getPar($par, "mode,idDivisi") ?>"><span>Detail</span></a>
                        </td>
                    </tr>
                    <?php
                }
