<?php
	...
	function lihat(){
		...
		$filter = "where nomorKoreksi is not null and t2.location in ($areaCheck)";		
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F4', 'KOREKSI');		
		$objPHPExcel->getActiveSheet()->setCellValue('I4', 'APPROVAL');
									
		$objPHPExcel->getActiveSheet()->setCellValue('F5', 'TANGGAL');
		$objPHPExcel->getActiveSheet()->setCellValue('G5', 'DATANG');
		...
		while($r=mysql_fetch_array($res)){
			...
			if($r[persetujuanKoreksi] == "t") $objPHPExcel->getActiveSheet()->getStyle('I'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB("11199000");
			else if($r[persetujuanKoreksi] == "f") $objPHPExcel->getActiveSheet()->getStyle('I'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB("fff00000");
			...
			if($r[sdmKoreksi] == "t") $objPHPExcel->getActiveSheet()->getStyle('J'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB("11199000");
			else if($r[sdmKoreksi] == "t") $objPHPExcel->getActiveSheet()->getStyle('J'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB("fff00000");
			else $objPHPExcel->getActiveSheet()->getStyle('J'.$rows)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB("fffff000");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $r[nomorKoreksi]);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, getTanggal($r[tanggalKoreksi]));
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, getTanggal($mulaiKoreksi));
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, substr($r[masukKoreksi],0,5));
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, substr($r[pulangKoreksi],0,5));
