...
if ($_GET["json"] == 'excel') {
  list($cLocId, $cDivId, $cDeptId, $cUnitId) = explode("~", $_GET["params"]);
  $res = $emp->exportToExcelReportKat($cLocId, $cDivId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Kategori",
    ...
    ), "laporan_jumlah_pegawai_per_kategori_" . uniqid() . ".xlsx");
}
...
if ($_GET["json"] == 1) {
  ...
  echo $emp->loadTableReportKat($locId, $divId, $deptId, $unitId);
  exit();
}
?>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="datatable">
    <thead>
      <tr>
        ...
        <th rowspan="2"  style="vertical-align:middle;width: 100px;">Total</th>
      </tr>
      <tr>
...
<script>
  jQuery(document).ready(function () {
    ot = jQuery('#datatable').dataTable({
      ...
      "bFilter": true,
      "iDisplayStart": 0,
      "sAjaxSource": sajax + "&json=1&clocid=<?= $cLocId ?>&cdivid=<?= $cDivId ?>&cdeptid=<?= $cDeptId ?>&cunitid=<?= $cUnitId ?>",
      "aoColumns": [
      {"mData": "deptId", "bSortable": false, "sClass": "alignRight"},
      {"mData": "deptName"},
    ...
    jQuery("#btnExpExcel").click(function (e) {
      e.preventDefault();
      var sMonth = jQuery("#cLocId").val() + "~" + jQuery("#cDivId").val() + "~" +jQuery("#cDeptId").val() + "~" +jQuery("#cUnitId").val();
