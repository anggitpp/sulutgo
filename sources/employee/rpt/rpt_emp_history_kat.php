...
if ($_GET["json"] == 'excel') {
  list($cmonth, $cyear, $cLocId, $cDivId, $cDeptId, $cUnitId,$cKatId) = explode("~", $_GET["params"]);
  $res = $emp->exportToExcelReportHistoryKat($cyear, $cmonth, $cLocId, $cDivId, $cDeptId, $cUnitId,$cKatId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Rekap History Pegawai Kategori",
   ...
   "Bulan : " . (empty($cmonth) ? "ALL" : getBulan($cmonth)) . "   Tahun : " . (empty($cmonth) ? "ALL" : $cyear),	  
   ), "laporan_rekap_pegawai_kategori_" . uniqid() . ".xlsx");
}
$cyear = isset($_POST["cyear"]) ? $_POST["cyear"] : date("Y");
$cmonth = isset($_POST["cmonth"]) ? $_POST["cmonth"] : date("m");
$cLocId = isset($_POST["cLocId"]) ? $_POST["cLocId"] : $cLocId;
$cKatId = isset($_POST["cKatId"]) ? $_POST["cKatId"] : $cKatId;
...
if ($_GET["json"] == 1) {
  ...
  $katId = isset($cKatId) ? $cKatId : $_GET["ckatid"];
  ...
  echo $emp->loadTableReportHistoryKat($cyear, $cmonth, $locId, $divId, $deptId, $unitId,$katId);
  exit();
}
?>
...
<div id="contentwrapper" class="contentwrapper">
  <form action="?<?php echo getPar($par, "mode") ?>" id="form" method="post" class="stdform">
          ...
          <table>
            <tr>
              <td style="vertical-align:top;">
                ...
                <p>
                  <label class="l-input-small" style="width:200px; text-align:left; padding-left:10px;">KATEGORI</label>
                  <div class="field" style="margin-left:240px;">
                    <?php echo comboData("SELECT kodeData, namaData FROM mst_data WHERE kodeCategory='KT' ORDER BY urutanData", "kodeData", "namaData", "cKatId", "--KATEGORI--", $cKatId, "onchange=\"document.getElementById('form').submit();\"", "250px", "chosen-select") ?>
                  </div>
                </p>
              </td>
    ...
    <tr>
      <td style="width:50%; text-align:left; vertical-align:top;">
        <p>
        <label class="l-input-small" style="width:200px; text-align:left; padding-left:10px;">Bulan</label>
          <span class="fieldB">
            <?php echo comboMonth("cmonth", $cmonth, "", "", "ALL") ?>
            &nbsp;&nbsp;&nbsp;&nbsp;<b>Tahun  : </b>&nbsp;&nbsp;
            <?php echo comboYear("cyear", $cyear, "", "", "", "ALL"); ?>
          </span>
          &nbsp;&nbsp;&nbsp;
          <input type="submit" class="submit radius2 btn_save" id="btnGo" name="btnGo" value="Cari"/>&nbsp;
        </p>
      </td>
    </tr>
  </table>
...
<script>
  jQuery(document).ready(function () {
    ot = jQuery('#datatable').dataTable({
      ...
      "bFilter": true,
      "iDisplayStart": 0,
      "sAjaxSource": sajax + "&json=1&cyear=<?= $cyear ?>&cmonth=<?= $cmonth ?>&clocid=<?= $cLocId ?>&cdivid=<?= $cDivId ?>&cdeptid=<?= $cDeptId ?>&cunitid=<?= $cUnitId ?>&ckatid=<?= $cKatId ?>",
      "aoColumns": [
      {"mData": null, "bSortable": false, "sClass": "alignRight"},
      {"mData": "name"},
      {"mData": "nik"},
     
      {"mData": "deptName", "sClass": "alignCenter"},
       {"mData": "jabatan"},
       {"mData": "kategori"},
      {"mData": "tanggalSelesai", "sClass": "alignCenter"},
      ],
      "aaSorting": [[1, "desc"]],
    ...
    jQuery("#btnExpExcel").click(function (e) {
      e.preventDefault();
      var sMonth = jQuery("#cmonth").val() + "~" + jQuery("#cyear").val() + "~" + jQuery("#cLocId").val() + "~" + jQuery("#cDivId").val() + "~" +jQuery("#cDeptId").val() + "~" +jQuery("#cUnitId").val()+ "~" +jQuery("#cKatId").val();
