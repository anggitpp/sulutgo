...
if ($_GET["json"] == 'excel') {
  ...
  $res = $emp->exportToExcelReportLoc($cLocId, $cDivId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Lokasi Kerja",
    ...
    ), "laporan_jumlah_pegawai_per_lokasi_" . date('Y-m-d H:i') . ".xlsx");
}
...
if ($_GET["json"] == 1) {
  ...
  echo $emp->loadTableReportLoc($locId, $divId, $deptId, $unitId);
  exit();
}
?>
...
<script>
  jQuery(document).ready(function () {
    ot = jQuery('#datatable').dataTable({
      ...
      "aoColumns": [
      {"mData": "locId", "bSortable": false, "sClass": "alignRight"},
      {"mData": "locName"},
