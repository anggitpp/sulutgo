...
if ($_GET["json"] == 'excel') {
  list($cDivId, $cLocId, $cDeptId, $cUnitId) = explode("~", $_GET["params"]);
  $res = $emp->exportToExcelReportCat($cDivId, $cLocId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Status",
   ...
   ), "laporan_jumlah_pegawai_per_status_" . date('Y-m-d H:i') . ".xlsx");
}
...
if ($_GET["json"] == 1) {
  ...
  echo $emp->loadTableReportCat($divId, $locId, $deptId, $unitId);
  exit();
}
?>
...
"sDom": "<'top'f>rt<'bottom'lip><'clear'>",
    ...
    jQuery("#btnExpExcel").click(function (e) {
      e.preventDefault();
      var sMonth = jQuery("#cdivid").val()+ "~" + jQuery("#clocid").val() + "~" +jQuery("#cDeptId").val() + "~" +jQuery("#cUnitId").val();
