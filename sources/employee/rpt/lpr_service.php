<?php
	...
	function gData(){
		...
		$bulanMulai = intval($_GET['mSearch']) == 1 ? 12 : $_GET['mSearch'] - 1;
		$tahunMulai = intval($_GET['mSearch']) == 1 ? $_GET['tSearch']-1 : $_GET['tSearch'];
		$tanggalMulai = $tahunMulai."-".str_pad($bulanMulai,2,"0",STR_PAD_LEFT)."-21";
		$tanggalSelesai = $_GET['tSearch']."-".$_GET['mSearch']."-20";	
		$totalHari = selisihHari($tanggalMulai, $tanggalSelesai) + 1;
		
		//$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
		//$sWhere= " where t1.id is not null and t1.status='".$status."' and t1.location in ( $areaCheck )";
		$sWhere= " where t1.id is not null and (t1.leave_date>'".$tanggalSelesai."' or t1.leave_date is null or t1.leave_date='0000-00-00') and t1.location in ( $areaCheck )";
		...
		//JADWAL
		$arrJadwal = arrayQuery("select idPegawai, tanggalJadwal, idShift from dta_jadwal where tanggalJadwal between '".$tanggalMulai."' and '".$tanggalSelesai."'");		
				
		//EKSTRA		
		$sql="select * from att_ekstra where persetujuanEkstra='t' and sdmEkstra='t' and penggantiEkstra between '".$tanggalMulai."' and '".$tanggalSelesai."'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			$tglEkstra["".$r[idPegawai].""]["".$r[penggantiEkstra].""] = $r[penggantiEkstra];
			$arrEkstra["".$r[idPegawai].""]++;
		}
		
		//CUTI
		$sql="select * from att_cuti where persetujuanCuti='t' and sdmCuti='t' and (mulaiCuti between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiCuti between '".$tanggalMulai."' and '".$tanggalSelesai."')";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			list($tanggalCuti) = explode(" ", $r[mulaiCuti]);
			list($selesaiCuti) = explode(" ", $r[selesaiCuti]);
			while($tanggalCuti <= $selesaiCuti){				
				list($Y,$m,$d) = explode("-", $tanggalCuti);				
				if($tanggalCuti >= $tanggalMulai && $tanggalCuti <= $tanggalSelesai){
					$tglCuti["".$r[idPegawai].""][$tanggalCuti] = $tanggalCuti;
					$arrCuti["".$r[idPegawai].""]++;
				}
				$tanggalCuti=date("Y-m-d",dateAdd("d", 1, mktime(0,0,0,$m,$d,$Y)));
			}
		}
		
		//ABSEN
		$sql="select * from att_absen where tanggalAbsen between '".$tanggalMulai."' and '".$tanggalSelesai."'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			if(!isset($tglEkstra["".$r[idPegawai].""]["".$r[tanggalAbsen].""])){
				$idShift = $arrJadwal["".$r[idPegawai].""]["".$r[tanggalAbsen].""];
				if(
					(!in_array($idShift, array(0)) || strlen($idShift) == 0) &&
					!isset($tglCuti["".$r[idPegawai].""]["".$r[tanggalAbsen].""])
				){
					$tglAbsen["".$r[idPegawai].""]["".$r[tanggalAbsen].""] = $r[tanggalAbsen];
					$arrAbsen["".$r[idPegawai].""]++;
				}
			}
		}
		
		//DINAS & PULANG CEPAT
		$sql="select * from att_hadir where persetujuanHadir='t' and sdmHadir='t' and idKategori in (575, 577) and (mulaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."')";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			list($tanggalDinas) = explode(" ", $r[mulaiHadir]);
			list($selesaiHadir) = explode(" ", $r[selesaiHadir]);
			while($tanggalDinas <= $selesaiHadir){				
				list($Y,$m,$d) = explode("-", $tanggalDinas);				
				if($tanggalDinas >= $tanggalMulai && $tanggalDinas <= $tanggalSelesai){
					if(!isset($tglAbsen["".$r[idPegawai].""][$tanggalDinas]) && !isset($tglEkstra["".$r[idPegawai].""][$tanggalDinas])) $arrDinas["".$r[idPegawai].""]++;
				}
				$tanggalDinas=date("Y-m-d",dateAdd("d", 1, mktime(0,0,0,$m,$d,$Y)));
			}
		}
		
		//KOREKSI
		$sql="select * from att_koreksi where persetujuanKoreksi='t' and sdmKoreksi='t' and (date(mulaiKoreksi) between '".$tanggalMulai."' and '".$tanggalSelesai."' or date(selesaiKoreksi) between '".$tanggalMulai."' and '".$tanggalSelesai."')";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			list($tanggalKoreksi) = explode(" ", $r[mulaiKoreksi]);
			list($selesaiKoreksi) = explode(" ", $r[selesaiKoreksi]);
			while($tanggalKoreksi <= $selesaiKoreksi){				
				list($Y,$m,$d) = explode("-", $tanggalKoreksi);				
				if($tanggalKoreksi >= $tanggalMulai && $tanggalKoreksi <= $tanggalSelesai){
					if(!isset($tglAbsen["".$r[idPegawai].""][$tanggalKoreksi]) && !isset($tglEkstra["".$r[idPegawai].""][$tanggalKoreksi])) $arrKoreksi["".$r[idPegawai].""]++;
				}
				$tanggalKoreksi=date("Y-m-d",dateAdd("d", 1, mktime(0,0,0,$m,$d,$Y)));
			}
		}		
		
		$sql="select * from dta_absen where date(mulaiAbsen) between '".$tanggalMulai."' and '".$tanggalSelesai."' or date(selesaiAbsen) between '".$tanggalMulai."' and '".$tanggalSelesai."'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			...
			while($tanggalAbsen <= $selesaiAbsen){				
				...
				if($tanggalAbsen >= $tanggalMulai && $tanggalAbsen <= $tanggalSelesai){
		...
		$sql="select * from dta_jadwal where tanggalJadwal between '".$tanggalMulai."' and '".$tanggalSelesai."' and idShift not in (select idShift from dta_shift where mulaiShift='00:00:00' and selesaiShift='00:00:00') and idShift!='0'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){		
			$arrAlpha["".$r[idPegawai].""]+= isset($arrMasuk["".$r[idPegawai].""]["".$r[tanggalJadwal].""]) ? 0 : 1;
		}			
		
		$arrIjin = arrayQuery("select idPegawai, sum(datediff(selesaiHadir,mulaiHadir) + 1) from att_hadir where persetujuanHadir='t' and sdmHadir='t' and idKategori in (579) and idTipe not in (962, 963) and (mulaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."') group by 1");
		$arrKhusus = arrayQuery("select idPegawai, sum(datediff(selesaiHadir,mulaiHadir) + 1) from att_hadir where persetujuanHadir='t' and sdmHadir='t' and idKategori in (579) and idTipe in (962, 963) and (mulaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."') group by 1");		
		$arrSakit = arrayQuery("select idPegawai, sum(datediff(selesaiHadir,mulaiHadir) + 1) from att_hadir where persetujuanHadir='t' and sdmHadir='t' and idKategori in (576) and (mulaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiHadir between '".$tanggalMulai."' and '".$tanggalSelesai."') group by 1");
		$arrEkstra = arrayQuery("select idPegawai, count(*) from att_ekstra where persetujuanEkstra='t' and sdmEkstra='t' and penggantiEkstra between '".$tanggalMulai."' and '".$tanggalSelesai."' group by 1");		
		
		$sql="select t2.* from dta_pegawai t1 join pay_service t2 on (t1.id=t2.idPegawai) ".$sWhere." and t2.tahunService='".$_GET['tSearch']."' and t2.bulanService='".$_GET['mSearch']."'";
		$res=db($sql);		
		while($r=mysql_fetch_array($res)){
		...
		$res=db($sql);						
		while($r=mysql_fetch_array($res)){						
			if(isset($arrService["".$r[id].""]) && isset($menuAccess[$s]["add"])){
				$s=$arrService["".$r[id].""];
				...
				$kerjaHari = $arrAbsen["".$r[id].""] + $arrAlpha["".$r[id].""] + $arrSakit["".$r[id].""] + $arrIjin["".$r[id].""] + $arrEkstra["".$r[id].""] + $arrCuti["".$r[id].""] + $arrKhusus["".$r[id].""] + $arrDinas["".$r[id].""] + $arrKoreksi["".$r[id].""];
				$liburHari = $jumlahHari-$kerjaHari;
				$alphaHari = $arrAlpha["".$r[id].""];
				$sakitHari = $arrSakit["".$r[id].""];
				$ijinHari = $arrIjin["".$r[id].""];
				$eoHari = $arrEkstra["".$r[id].""];
				$cutiHari = $arrCuti["".$r[id].""] + $arrKhusus["".$r[id].""];
				$serviceHari = $kerjaHari - $alphaHari - $sakitHari - $ijinHari;		
			}
		...
		return getAngka($tHari)."\t".getAngka($tLibur)."\t".getAngka($tKerja)."\t".getAngka($tAlpha)."\t".getAngka($tSakit)."\t".getAngka($tIjin)."\t".getAngka($tEo)."\t".getAngka($tCuti)."\t".getAngka($tService)."\t".getTanggal($tanggalMulai,"t")." s.d ".getTanggal($tanggalSelesai,"t");
	}
	...
	function lData(){
		...
		$sql="select * from dta_pegawai t1 $sWhere order by $orderBy $sLimit";		
		$res=db($sql);		
		$json = array(
	...
	function lihat(){
		...
		/*
		$tanggalMulai = '2017-01-21';
		$tanggalSelesai = '2017-02-20';
					
		//CUTI
		$sql="select * from att_cuti where persetujuanCuti='t' and sdmCuti='t' and (mulaiCuti between '".$tanggalMulai."' and '".$tanggalSelesai."' or selesaiCuti between '".$tanggalMulai."' and '".$tanggalSelesai."') and idPegawai=129";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			list($tanggalCuti) = explode(" ", $r[mulaiCuti]);
			list($selesaiCuti) = explode(" ", $r[selesaiCuti]);
		...
		//ABSEN
		$arrJadwal = arrayQuery("select idPegawai, tanggalJadwal, idShift from dta_jadwal where tanggalJadwal between '".$tanggalMulai."' and '".$tanggalSelesai."' and idPegawai=129");		
		$sql="select * from att_absen where tanggalAbsen between '".$tanggalMulai."' and '".$tanggalSelesai."' and idPegawai=129";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			$idShift = $arrJadwal["".$r[idPegawai].""]["".$r[tanggalAbsen].""];
			if((!in_array($idShift, array(0)) || strlen($idShift) == 0) && !isset($tglCuti["".$r[idPegawai].""]["".$r[tanggalAbsen].""])){
				$tglAbsen["".$r[idPegawai].""]["".$r[tanggalAbsen].""] = $r[tanggalAbsen];
				$arrAbsen["".$r[idPegawai].""]++;
			}
		}
		...
		*/
		...
		$text.="<div class=\"pageheader\">
			...
			<form id=\"form\" action=\"\" method=\"post\" class=\"stdform\">
				...
				<table>
					...
					".comboArray("pSearch", array("All", "NAMA", "NO ID"), $par[paramData])."</td>
		...
		$text.="<td><a href=\"#\" onclick=\"window.location='?par[mode]=xls&mSearch=' + document.getElementById('mSearch').value + '&tSearch=' + document.getElementById('tSearch').value + '".getPar($par,"mode")."';\" class=\"btn btn1 btn_inboxi\"><span>Export Data</span></a>
				</td>
				</tr>
				</table>
				</div>	
		...
		if($par[mode] == "xls"){
			xls();			
			$text.="<iframe src=\"download.php?d=exp&f=Rekap Tunjangan Service.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
		}	
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', getTanggal($tanggalMulai,"t")." s.d ".getTanggal($tanggalSelesai,"t"));
		...
		$jumlahHari = $liburHari = $kerjaHari = $alphaHari = $sakitHari = $ijinHari = $eoHari = $cutiHari = $serviceHari = 0;		
		$sql="select * from dta_pegawai t1 $sWhere order by t1.name";		
		$res=db($sql);						
		while($r=mysql_fetch_array($res)){						
			$no++;
		...
		$objWriter->save($fFile."Rekap Tunjangan Service.xls");
	}
