...
if ($_GET["json"] == 'excel') {
  ...
  $res = $emp->exportToExcelReportDep($cLocId, $cDivId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Departemen",
    ...
    ), "laporan_jumlah_pegawai_per_departemen_" . date('Y-m-d H:i') . ".xlsx");
}
...
if ($_GET["json"] == 1) {
  ...
  echo $emp->loadTableReportDep($locId, $divId, $deptId, $unitId);
  exit();
}
?>
