...
if ($_GET["json"] == 'excel') {
  ...
  $res = $emp->exportToExcelReportAge($cDivId, $cLocId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Usia",
   ...
   ), "laporan_jumlah_pegawai_per_usia_" . date('Y-m-d H:i') . ".xlsx");
}
...
if ($_GET["json"] == 1) {
 ...
 echo $emp->loadTableReportAge($divId, $locId, $deptId, $unitId);
 exit();
}
?>
