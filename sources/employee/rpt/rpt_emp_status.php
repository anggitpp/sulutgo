...
if ($_GET["json"] == 'excel') {
  list($cLocId, $cStsId, $cJabId) = explode("~", $_GET["params"]);
  $res = $emp->exportToExcelReportStatus($cLocId, $cStsId, $cJabId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Pegawai per Status & Jabatan",
      "Lokasi : " . (empty($cLocId) ? "ALL" : $cutil->getMstDataDesc($cLocId)),
	  "Status : " . (empty($cStsId) ? "ALL" : $cutil->getMstDataDesc($cStsId)),
	  "Jabatan : " . (empty($cJabId) ? "ALL" : $cJabId),
          ), "laporan_pegawai_per_status_jabatan_" . uniqid() . ".xlsx");
}
$cLocId = isset($_POST["clocid"]) ? $_POST["clocid"] : "";
$cStsId = isset($_POST["cstsid"]) ? $_POST["cstsid"] : "";
$cJabId = isset($_POST["cjabid"]) ? $_POST["cjabid"] : "";
if ($_GET["json"] == 1) {
  $locId = isset($_GET["clocid"]) ? $_GET["clocid"] : "";
  $stsId = isset($_GET["cstsid"]) ? $_GET["cstsid"] : "";
  $jabId = isset($_GET["cjabid"]) ? $_GET["cjabid"] : "";
  header("Content-type: application/json");
  echo $emp->loadTableReportStatus($locId, $stsId, $jabId);
  exit();
}
?>
...
<div id="contentwrapper" class="contentwrapper">
  <form class="stdform" action="<?= $loc ?>" method="post" style="width: 100%">
	...
	<p><label class="l-input-small">Jabatan</label>
      <span class="fieldB">
        <?php echo $cutil->generateSelectWithEmptyOption("SELECT pos_name id, pos_name description FROM emp_phist WHERE status=1 GROUP BY pos_name ORDER BY pos_name", "id", "description", "cjabid", $cJabId, "<option value = \"\">All</option>", "onchange=\"this.form.submit();\"") ?>
      </span>
    </p>
  </form>
  <br class="clear"/>
...
<script>
  jQuery(document).ready(function () {
    ot = jQuery('#datatable').dataTable({
      ...
      "bFilter": true,
      "iDisplayStart": 0,
      "sAjaxSource": sajax + "&json=1&clocid=<?= $cLocId ?>&cstsid=<?= $cStsId ?>&cjabid=<?= $cJabId ?>",
      "aoColumns": [
        ...
        {"mData": "deptName"},
        {"mData": "posName"},
        {"mData": "birth_date", "sClass": "alignCenter"},
    ...
    jQuery("#btnExpExcel").click(function (e) {
      e.preventDefault();
      var sMonth = jQuery("#clocid").val() + "~" + jQuery("#cstsid").val() + "~" + jQuery("#cjabid").val();
