...
if ($_GET["json"] == 'excel') {
  list($cmonth, $cyear, $cLocId, $cDivId, $cDeptId, $cUnitId) = explode("~", $_GET["params"]);
  $res = $emp->exportToExcelReportContractExpired($cyear, $cmonth, $cLocId, $cDivId, $cDeptId, $cUnitId);
  $uie = new UiHelperExtra();
  $uie->exportXLS($res, array("Laporan Jumlah Pegawai per Jabatan",
   ...
   ), "laporan_jumlah_pegawai_telah_habis_kontrak_" . date('Y-m-d H:i') . ".xlsx");
}
...
if ($_GET["json"] == 1) {
  ...
  echo $emp->loadTableReportContractExpired($cyear, $cmonth, $locId, $divId, $deptId, $unitId);
  exit();
}
?>
...
<div id="contentwrapper" class="contentwrapper">
  <form action="?<?php echo getPar($par, "mode") ?>" id="form" method="post" class="stdform">
    ...
    <tr>
      <td style="width:50%; text-align:left; vertical-align:top;">
        <p>
        <label class="l-input-small2">Bulan</label>
          <span class="fieldB">
...
<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="datatable">
  <thead>
    ...
    <tr>
      <th style="text-align: center;width: 70px;">Mulai</th>
...
<script>
  jQuery(document).ready(function () {
    ot = jQuery('#datatable').dataTable({
      ...
      "bFilter": true,
      "iDisplayStart": 0,
      "sAjaxSource": sajax + "&json=1&cyear=<?= $cyear ?>&cmonth=<?= $cmonth ?>&clocid=<?= $cLocId ?>&cdivid=<?= $cDivId ?>&cdeptid=<?= $cDeptId ?>&cunitid=<?= $cUnitId ?>",
      "aoColumns": [
      {"mData": "jabId", "bSortable": false, "sClass": "alignRight"},
      {"mData": "name"},
      {"mData": "deptName"},
      {"mData": "posName"},
      {"mData": "startDate", "sClass": "alignCenter"},
      {"mData": "expireDate", "sClass": "alignCenter"}
      ],
      "aaSorting": [[1, "desc"]],
    ...
    jQuery("#btnExpExcel").click(function (e) {
      e.preventDefault();
      var sMonth = jQuery("#cmonth").val() + "~" + jQuery("#cyear").val() + "~" + jQuery("#cLocId").val() + "~" + jQuery("#cDivId").val() + "~" +jQuery("#cDeptId").val() + "~" +jQuery("#cUnitId").val();
