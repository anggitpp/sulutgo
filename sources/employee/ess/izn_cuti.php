...
function gCuti(){
	...
	$jatahCuti = $r[masaCuti] != 0 && $r[masaCuti] < $getMasaKerja ? $jatahCuti : 0;
...
function tambah(){
	...
	if($cek){
		...
		$sql="insert into att_cuti (fileCuti, idCuti, idTipe, idPegawai, idPengganti, idAtasan, nomorCuti, tanggalCuti, mulaiCuti, selesaiCuti, jatahCuti, jumlahCuti, sisaCuti, keteranganCuti, persetujuanCuti, sdmCuti, createBy, createTime) values ('$fileCuti','$idCuti', '$inp[idTipe]', '$inp[idPegawai]', '$inp[idPengganti]', '$inp[idAtasan]', '$inp[nomorCuti]', '".setTanggal($inp[tanggalCuti])."', '".setTanggal($inp[mulaiCuti])."', '".setTanggal($inp[selesaiCuti])."', '".setAngka($inp[jatahCuti])."', '".setAngka($inp[jumlahCuti])."', '".setAngka($inp[sisaCuti])."', '$inp[keteranganCuti]', 'p', 'p', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
			...
			<table width=\"100%\">
				...
				<td width=\"45%\">
					...
					<p>
						...
						<div class=\"field\">
							<input type=\"text\" id=\"inp[jatahCuti]\" name=\"inp[jatahCuti]\"  value=\"".getAngka($r[jatahCuti])."\" class=\"mediuminput\" style=\"width:50px; text-align:right; float:left\" readonly=\"readonly\"/> 
							<span style=\"float:left; margin-left:2px; margin-top:5px; margin-right:30px;\">hari</span>
				...
				<td width=\"55%\">";
					...
					<p>
						...
						<div class=\"field\">";
							...
							"<a href=\"download.php?d=cuti&f=$r[idCuti]\"><img src=\"".getIcon($r[fileCuti])."\" align=\"left\" style=\"padding-right:5px; padding-bottom:5px; max-width:50px; max-height:50px;\"></a>
...
function lihat(){
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		...
		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dyntable\">
			...
			$sql="select * from att_cuti t1 left join emp t2 on (t1.idPegawai=t2.id) $filter order by t1.nomorCuti";
			$res=db($sql);
