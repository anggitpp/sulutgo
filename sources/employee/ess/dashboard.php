<?php
	...
	function lihat(){
		...
		$arrHadir = arrayQuery("select idKategori, idPegawai, idPegawai from att_hadir where year(mulaiHadir)='".$par[tahunAbsen]."'  and idPegawai='".$par[idPegawai]."' group by 1, 2");
		...
		$arrAbsen = arrayQuery("select date(mulaiAbsen), idAbsen from dta_absen where idPegawai = '$par[idPegawai]' AND year(mulaiAbsen) = '$par[tahunAbsen]'");
		$arrMaster = arrayQuery("select kodeData, namaData from mst_data");
		
	if($par[tahunAbsen] != date('Y')){
		for($bulan=1;$bulan<=12;$bulan++){
			$d=cal_days_in_month(CAL_GREGORIAN,$bulan,$par[tahunAbsen]);
			for ($i=1; $i <= $d; $i++) { 
				if($i<10){
				$i = "0".$i;
				}
				...
				$tanggal = $par[tahunAbsen]."-".$bulan."-".$i;			
				$weekDay = date('w', strtotime($tanggal));
				if($weekDay < 6 && $weekDay !=0){
					if($par[tahunAbsen]<=date('Y')){
						if(empty($arrAbsen[$tanggal])){
							$cntAlpha++;
							$cntAlphaBulanan[intval($bulan)][$tanggal] = 1;
							$cntAlphaBulananLibur[$tanggal] = 1;
							// echo $tanggal;
						}
					}
				}
	...
	$sqlLibur = "select mulaiLibur from dta_libur";
	$resLibur = db($sqlLibur);
	while ($b = mysql_fetch_array($resLibur)) {
		$holidays[] = $b[mulaiLibur];
	}

	foreach ($cntAlphaBulananLibur as $key => $value) {
		if(!in_array($key, $holidays))
			{
				$cntAlphaBulananS[$key] = 1;
			}
		}

	for($i = 1; $i<=12; $i++){
	foreach ($cntAlphaBulanan[$i] as $key => $value) {
		if(!in_array($key, $holidays))
			{
				$cntAlphaBulananFix[$i][$key] = 1;
			}
		}
	}

	$cntAlpha = array_sum($cntAlphaBulananS);
		...
		$arrJadwal=arrayQuery("select t1.tanggalJadwal, t1.idPegawai, t1.mulaiJadwal from dta_jadwal t1 join dta_shift t2 on (t1.idShift=t2.idShift) where year(t1.tanggalJadwal)='".$par[tahunAbsen]."' and t1.idPegawai='".$par[idPegawai]."' and lower(trim(t2.namaShift)) not in ('off', 'cuti')");
		
		$sql="select * from dta_absen where year(mulaiAbsen)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."' group by date(mulaiAbsen), idPegawai order by mulaiAbsen";
		$res=db($sql);		
       		...
       		$dt2 = date("N", $dt1);
  	  		if($dt2 < 6){
					...
					if($jamAbsen.$menitAbsen > $jamJadwal.$menitJadwal && !empty($mulaiAbsen) && $mulaiAbsen != "00:00:00"){
						...
						$arrTerlambat[intval($bulanAbsen)][$tanggalAbsen] = 1;					
					}												
				}
				// }			
				
				$arrAbsen[$tanggalAbsen]=$mulaiAbsen;
				$arrAbsenBulanan[intval($bulanAbsen)][$tanggalAbsen]=1;
				// if(isset($arrJadwal[$tanggalAbsen]) && $mulaiAbsen > $arrJadwal[$tanggalAbsen]) $arrTerlambat++;
				
				// if(!isset($arrJadwal[$tanggalAbsen]) || $mulaiAbsen <= $arrJadwal[$tanggalAbsen])  $arrHadir++;			
		}					
		...
		$sql__="select
			*
		from emp_phist where parent_id='".$cID."' and status = '1'";
		$res__=db($sql__);
		$r__=mysql_fetch_array($res__);

		

		$r[rawatSkala] = getField("select rawatSkala from pay_skala where idGolongan = '".$arrMaster[$r__[grade]]."'");
		$r[pengali] = getField("select nilai from rawatjalan_plafon where idJenis='$r_[marital]'");
		$r[angkaPengali] = str_replace(",", ".", $r[pengali]);

		$r[totalUpah] = $r[rawatSkala] * $r[angkaPengali];

		$getPokok = getField("select nilaiPokok from pay_pokok where idPegawai = '$r_[parent_id]'");

		$rawatJalan = getField("SELECT SUM(pengambilan) FROM rawatjalan_klaim WHERE idPegawai = '$r_[parent_id]' AND YEAR(tanggalKlaim) = '$par[tahunAbsen]' AND apprKlaim = 't' ");

		$getNilai = getField("select nilai from rawatjalan_plafon where idJenis = '$r_[marital]' AND tahun = '".date('Y')."'");

		$cntRawatJalan = $r[totalUpah] - $rawatJalan;

		$maxRawatJalan = $r[totalUpah];
		...
		$cntCuti = getField("select count(*) from att_cuti where year(mulaiCuti)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."'");
		$appCuti = getField("select count(*) from att_cuti where year(mulaiCuti)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."' and persetujuanCuti='t' and sdmCuti='t'");
		...
		$cntLembur = getField("select count(*) from att_lembur where year(mulaiLembur)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."'");
		$appLembur = getField("select count(*) from att_lembur where year(mulaiLembur)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."' and persetujuanLembur='t'");		
		
		$maxLembur = $cntLembur > 0 ? ceilAngka($cntLembur, pow(10,strlen($cntLembur))) : 10;
		$downLembur = 0.5 * $maxLembur;
		$upLembur = 0.8 * $maxLembur;

		$cntKetidakhadiran = getField("select count(*) from att_hadir where year(tanggalHadir)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."'");
		$appKetidakhadiran = getField("select count(*) from att_hadir where year(tanggalHadir)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."' and persetujuanHadir='t'");

		$cntDinas = getField("select count(*) from ess_dinas where year(tanggalDinas)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."'");
		$appDinas = getField("select count(*) from ess_dinas where year(tanggalDinas)='".$par[tahunAbsen]."' and idPegawai='".$par[idPegawai]."' and persetujuanDinas='t'");		
		...
		$text.="<div class=\"pageheader\">
			...
			Periode : ".comboYear("par[tahunAbsen]", $par[tahunAbsen], "", "onchange=\"document.getElementById('form').submit();\"")."
			</div>				
			</form>
				...
				<td style=\"width:50%; vertical-align:top;\">
					...
					if(is_array($arrIzin)){						
						...
						while(list($idIzin ,$namaIzin) = each($arrIzin)){							
							$text.="<set label=\"".$namaIzin." \" value=\"".getField("select count(id".ucwords($idIzin).") from att_$idIzin where idPegawai = '$par[idPegawai]' AND year(tanggal".ucwords($idIzin).") = '$par[tahunAbsen]'")."\"/> ";
			...
			for($i=1; $i<=12; $i++){
				$valAlpha = array_sum($cntAlphaBulananFix[$i]) > 0 ? array_sum($cntAlphaBulananFix[$i]) : 0;
				$text.="<set value=\"".$valAlpha."\" toolText=\"Alpha, ".getBulan($i)." ".getBulan($par[bulanAbsen]).", ".getAngka($valAlpha)." Kali\"/>";
			}
			$text.="</dataset>";
			
			$text.="<dataset seriesName=\"Telat\" color=\"015887\">";
			for($i=1; $i<=12; $i++){
				$valTerlambat = array_sum($arrTerlambat[$i]) > 0 ? array_sum($arrTerlambat[$i]) : 0;
				$text.="<set value=\"".$valTerlambat."\" toolText=\"Telat, ".getBulan($i)." ".getBulan($par[bulanAbsen]).", ".getAngka($valTerlambat)." Kali\"/>";
			}
			$text.="</dataset>";
			
			$text.="<dataset seriesName=\"Hadir\" color=\"78AE1C\">";
			for($i=1; $i<=12; $i++){
				$valHadir = array_sum($arrAbsenBulanan[$i]) > 0 ? array_sum($arrAbsenBulanan[$i]) : 0;
		...
		$text.="<table style=\"width:100%; margin-top:30px; margin-bottom:10px; margin-left:-15px;\">
			<tr>				
				...
				<td style=\"width:25%; vertical-align:top; padding-left:15px; padding-right:15px;\">
					...
					<script type=\"text/javascript\">
					var klaimChart ='<chart manageResize=\"1\" origW=\"300\" origH=\"150\" palette=\"2\" bgColor=\"999999,EEEEEE\" lowerLimit=\"0\" upperLimit=\"".$maxRawatJalan."\" showBorder=\"1\" borderColor=\"888888\" basefontColor=\"000000\" chartBottomMargin=\"50\" chartRightMargin=\"10\" gaugeStartAngle=\"220\" gaugeEndAngle=\"-40\"  pivotRadius=\"8\" pivotFillMix=\"{CCCCCC},{000000}\" pivotBorderColor=\"000000\" >";
					
					$text.="<colorRange>";
					$text.="<color minValue=\"0\" maxValue=\"".$downTerlambat."\"/>";
					$text.="<color minValue=\"".$downTerlambat."\" maxValue=\"".$upTerlambat."\"/>";
					$text.="<color minValue=\"".$upTerlambat."\" maxValue=\"".$maxRawatJalan."\"/>";
					$text.="</colorRange>";
			...
			<tr>				
				<td style=\"width:25%; vertical-align:top; padding-left:15px; padding-right:15px;\">
					...
					<div id=\"divAlpha\" align=\"center\" onclick=\"openBox('popup.php?par[mode]=detAlpha".getPar($par,"mode")."',875,450);\"></div>
					<script type=\"text/javascript\">
				...
				<td style=\"width:25%; vertical-align:top; padding-left:15px; padding-right:15px;\">
					...
					<div id=\"divSPPD\" align=\"center\" onclick=\"openBox('popup.php?par[mode]=detSPPD".getPar($par,"mode")."',1075,450);\"></div>
					<script type=\"text/javascript\">
				...
				if(getField("select id from emp_plafon where idCuti = '7' AND parent_id = '$par[idPegawai]' ")){
					$text.="
				<td style=\"width:25%; vertical-align:top; padding-left:15px; padding-right:15px;\">
					<div class=\"widgetbox\">
						<div class=\"title\" style=\"margin-bottom:0px;\"><h3>Cuti Besar</h3></div>
					</div>
					<div id=\"divBesar\" align=\"center\" onclick=\"openBox('popup.php?par[mode]=detCutiBesar".getPar($par,"mode")."',875,450);\"></div>
					<script type=\"text/javascript\">
		...
		// echo "</pre>";
		// echo "ini obat".$obat;
		$idJenis = getField("select kodeData from mst_data where kodeMaster = 'MRL1'");
		$nilaiMaksimal = getField("select nilai from rawatinap_plafon where idGolongan = '$obat' AND idJenis = '$idJenis'");
		$jumlahAll = getField("SELECT SUM(total) FROM rawatinap_klaim t1 join rawatinap_klaim_detail t2 on t1.id = t2.idKlaim WHERE t1.idPegawai = '$par[idPegawai]' AND year(t1.tanggal) = '$par[tahunAbsen]' AND t1.apprKlaim = 't'");
		$sisaMaksimal = $nilaiMaksimal - $jumlahAll;
		

		// echo "select nilai from rawatinap_plafon where idGolongan = '$obat' AND idJenis = 'MRL1'";

		$text.="<div class=\"widgetbox\">
				<div class=\"title\" style=\"margin-bottom:0px;\"><h3>PERAWATAN KESEHATAN <p style=\"right:10px;position:absolute;margin-top:-15px;\">NILAI MAKSIMAL : ".getAngka($sisaMaksimal)."</p></h3></div>
			</div>
			
			<div id=\"divPangkat\" align=\"center\" onclick=\"openBox('popup.php?par[mode]=detPerawatan".getPar($par,"mode")."',875,450);\"></div>
			<script type=\"text/javascript\">
			...
			if(is_array($arrInap)){
				...
				while(list($idInap, $valInap) = each($arrInap)){
					...
					$jumlahPakai = getField("SELECT SUM(total) FROM rawatinap_klaim t1 join rawatinap_klaim_detail t2 on t1.id = t2.idKlaim WHERE t2.idKategori = '$idInap' AND year(t1.tanggal) = '$par[tahunAbsen]' AND t1.idPegawai = '$par[idPegawai]'");
					// $sisaInap = $jumlahInap - $jumlahPakai;

					// $totalValue = getField("select ")
					 $text.="<set label=\"".$arrInap[$idInap]."\" value=\"".$jumlahPakai."\"/> ";					
				}
			}
	...
	function detIzin(){
		...
		$text.="<div class=\"pageheader\">
			...
			<div id=\"contentwrapper\" class=\"contentwrapper\">			
			<br clear=\"all\" />
			<form id=\"formIzin\" name=\"formIzin\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" >
			<div style=\"position:absolute;right:5px;top:10px;\">
			".comboMonth("par[bulanAbsen]", $par[bulanAbsen],"onchange=\"document.getElementById('formIzin').submit();\"","","All")."
			</div>
	...
	function detTerlambat(){
		...
		$arrJadwal=arrayQuery("select t1.tanggalJadwal, t1.idPegawai, t1.mulaiJadwal from dta_jadwal t1 join dta_shift t2 on (t1.idShift=t2.idShift) where year(t1.tanggalJadwal)='".$par[tahunAbsen]."'  not in ('off', 'cuti')");
		...
		if(!empty($par[bulanAbsen])){
			$filter.= "and month(t2.mulaiAbsen) = '$par[bulanAbsen]'";
		}
		
		$sql="select * from dta_pegawai t1 join dta_absen t2 on (t1.id=t2.idPegawai) where year(t2.mulaiAbsen)='".$par[tahunAbsen]."' and t1.id='".$par[idPegawai]."' ".$filter." group by date(t2.mulaiAbsen) order by t1.name, t2.mulaiAbsen ";
		$res=db($sql);		
		while($r=mysql_fetch_array($res)){
			list($tanggalAbsen, $mulaiAbsen) = explode(" ", $r[mulaiAbsen]);
			$dt1 = strtotime($tanggalAbsen);
		...
		<td><div align=\"right\">$no Hari</div></td>
		<td><div align=\"right\">$totalMenit Menit</div></td>
		</tr>
		</div>
		</tfoot>
			</table>
			...
			for ($i=1; $i <= $d; $i++) { 
				...
				if($weekDay < 6 && $weekDay !=0){
					if($par[tahunAbsen]<=date('Y')){
						if(empty($arrAbsen[$tanggal])){
							$cntAlpha++;
							$cntAlphaBulananFix[$tanggal] = $bulan;
							// echo $tanggal;
						}
					}
				}
	...
	foreach ($cntAlphaBulananFix as $key => $value) {
		if(!in_array($key, $holidays))
			{
				$cntAlphaBulanan[$key] = 1;
			}
		}

	$no=0;
	is_array($cntAlphaBulanan);
	reset($cntAlphaBulanan);
	while(list($tanggal ,$bulan) = each($cntAlphaBulanan)){
		if(!empty($par[bulanAbsen])){
			if($bulan == $par[bulanAbsen]){
				$no++;
				$text.=" <tr>
				<td>$no.</td>
				<td>$r[name]</td>
	...
	function detKlaim(){
		...
		$filter = "where t2.id='".$par[idPegawai]."' AND t1.apprKlaim = 't'";		
		if(!empty($par[bulanAbsen]))
			$filter.= " and month(t1.tanggal)='$par[bulanAbsen]'";
		if(!empty($par[tahunAbsen]))
			$filter.= " and year(t1.tanggal)='$par[tahunAbsen]'";				
		if(!empty($par[lokasi]))
		...
		$sql="select t1.*, t2.name, t2.reg_no from rawatjalan_klaim t1 left join dta_pegawai t2 on (t1.idPegawai=t2.id) $filter order by t1.nomor";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
			$no++;
		...
		<td><div align=\"right\">".getAngka($rmbTotal)."</div></td>	
		</tr>
		</div>
		</tfoot>
			</table>
	...
	function detPerawatan(){
		...
		$arrKategori = arrayQuery("select kodeData, namaData from mst_data");

		$sql="select *, t1.id as idK from rawatinap_klaim t1 left join dta_pegawai t2 on (t1.idPegawai=t2.id) $filter order by t1.nomor";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
			$no++;
			$r[nilai] = getField("select SUM(total) from rawatinap_klaim_detail where idKlaim = '$r[idK]' AND idKategori = '$r[idKategori]'");
			$text.="<tr>
					<td>$no.</td>
					...
					<td>$r[name]</td>					
					<td>".getField("select name from emp_family where id = '$r[idPasien]'")."</td>
		...
		<tr>
		<td colspan=\"5\"><div align=\"right\">Total</div></td>
		<td><div align=\"right\">".getAngka($nilaiTotal)."</div></td>	
		<td>&nbsp;</td>
		</tr>
		</div>
		</tfoot>
	...
	function detCutiBesar(){
		...
		$filter = "where nomorCuti is not null and idPegawai='".$par[idPegawai]."' AND idTipe IN (7,8)";		
		if(!empty($par[bulanAbsen]))
			$filter.= " and month(t1.tanggalCuti)='$par[bulanAbsen]'";
		if(!empty($par[tahunAbsen]))
			$filter.= " and year(t1.tanggalCuti)='$par[tahunAbsen]'";				
	...
	function detSPPD(){
		...
		if(!empty($par[bulanAbsen]))
			$filter.= " and month(t1.tanggalDinas)='$par[bulanAbsen]'";
		if(!empty($par[tahunAbsen]))
		...
		<td><div align=\"right\">".getAngka($totalDinas)."</div></td>	
		</tr>
		</div>
		</tfoot>
			</table>
