function getNomor(getPar){
	tanggalIzin=document.getElementById("tanggalIzin");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorIzin]").value = xmlHttp.responseText;
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalIzin=" + tanggalIzin.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
