function getNomor(getPar){
	tanggalPelatihan=document.getElementById("tanggalPelatihan");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorPelatihan]").value = xmlHttp.responseText;
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalPelatihan=" + tanggalPelatihan.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
