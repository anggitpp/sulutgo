<?php
	...
	function lihat(){
		...
		$filter = "where year(t1.tanggalIzin)='$par[tahunIzin]'";
		...
		$sql="select * from att_izin t1 left join emp t2 on (t1.idPegawai=t2.id) $filter order by t1.nomorIzin";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
			...
			$text.="<tr>
					...
					<td align=\"center\">$persetujuanIzin</td>";
			if(isset($menuAccess[$s]["edit"]) || isset($menuAccess[$s]["delete"])){
				$text.="<td align=\"center\">";
				
				if(in_array($r[persetujuanIzin], array("p","r")))
				...
				if(in_array($r[persetujuanIzin], array("p")))
