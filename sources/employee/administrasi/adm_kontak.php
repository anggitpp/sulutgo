...
function setData(){
	...
	if($par[rowData] <= $highestRow){  
		$rowData = $sheet->rangeToArray('A' . $par[rowData] . ':O' . $par[rowData], NULL, TRUE, TRUE);             
		$dta = $rowData[0];
		$tRow = 6;
		...
		if(!in_array(trim(strtolower($dta[1])), array("","NPP PEGAWAI"))){          
			...
			$idKontak = getField("SELECT id from emp_contact order by id desc limit 1")+1;
			$sql = "insert into emp_contact set id = '$idKontak', parent_id = '$parentId', sr_nama = '".$dta[3]."', sr_hub = '".$dta[4]."', sr_phone = '".$dta[5]."', sr_address = '".$dta[6]."', sr_prov = '".$arrMaster[trim(strtolower($dta[7]))]."', sr_city = '".$arrMaster[trim(strtolower($dta[8]))]."', br_nama = '".$dta[9]."', br_hub = '".$dta[10]."', br_phone = '".$dta[11]."', br_address = '".$dta[12]."', br_prov = '".$arrMaster[trim(strtolower($dta[13]))]."', br_city = '".$arrMaster[trim(strtolower($dta[14]))]."', cre_by = 'migrasi', cre_date = '".date('Y-m-d H:i:s')."'";
			db($sql);
...
function tambah(){
	...
	$id = getField("select id from emp_contact order by id desc limit 1") + 1;
	$sql = "insert into emp_contact (id, parent_id, sr_nama, sr_hub, sr_phone, sr_address, sr_prov, sr_city, br_nama, br_hub, br_phone, br_address, br_prov, br_city,  cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[sr_nama]', '$inp[sr_hub]', '$inp[sr_phone]', '$inp[sr_address]', '$inp[sr_prov]', '$inp[sr_city]','$inp[br_nama]', '$inp[br_hub]', '$inp[br_phone]', '$inp[br_address]', '$inp[br_prov]', '$inp[br_city]', '".date('Y-m-d H:i:s')."', '$cUsername')";
	db($sql);
...
function ubah(){
	...
	$sql = "update emp_contact set parent_id='$inp[parent_id]',sr_nama='$inp[sr_nama]',sr_hub='$inp[sr_hub]', sr_phone='$inp[sr_phone]', sr_address='$inp[sr_address]', sr_prov='$inp[sr_prov]',  sr_city='$inp[sr_city]', br_nama='$inp[br_nama]',br_hub='$inp[br_hub]', br_phone='$inp[br_phone]', br_address='$inp[br_address]', br_prov='$inp[br_prov]',  br_city='$inp[br_city]', upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);

	echo "<script>alert('UPDATE DATA BERHASIL');window.location='?par[mode]=edit&par[id]=$par[id]" . getPar($par, "mode") . "';</script>";	
}
function hapus(){
	global $par;
		...
		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\">
			<thead>
				<tr>
					...
					<th rowspan=\"2\" width=\"75\">NPP</th>
					<th colspan=\"2\" width=\"200\">Kontak</th>
					<th rowspan=\"2\" width=\"125\">Kontrol</th>
				</tr>
				<tr>
...
function lData(){
	...
	$sql="select t1.id, t1.sr_phone, t1.br_phone, t2.name, t2.reg_no from emp_contact t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_contact t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);
	...
	while($r=mysql_fetch_array($res)){
		...
		$data=array(
			...
			"<div align=\"left\">".$r[sr_phone]."</div>",
			"<div align=\"left\">".$r[br_phone]."</div>",
...
function form(){
	...
	$sql="select * from emp_contact where id='$par[id]'";
	$res=db($sql);
			...
			<table style=\"width:100%\">
				...
				<td style=\"width:50%\">			
					<p>
						...
						<div class=\"field\">
							<input type=\"text\" id=\"inp[sr_nama]\" name=\"inp[sr_nama]\" class=\"smallinput\" value=\"$r[sr_nama]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small2\">Hubungan</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[sr_hub]\" name=\"inp[sr_hub]\" class=\"smallinput\" value=\"$r[sr_hub]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small2\">No. Telp</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[sr_phone]\" name=\"inp[sr_phone]\" onkeyup=\"cekPhone(this);\" class=\"smallinput\" value=\"$r[sr_phone]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small2\">Alamat</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[sr_address]\" name=\"inp[sr_address]\" class=\"smallinput\" value=\"$r[sr_address]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small2\">Propinsi</label>
						<div class=\"field\">
							" . comboData("select * from mst_data where kodeCategory='S02' and statusData='t' order by namaData", "kodeData", "namaData", "inp[sr_prov]", " ", $r[sr_prov], "", "205px","chosen-select") . "
						</div>
					</p>
					<p>
						<label class=\"l-input-small2\">Kota</label>
						<div class=\"field\">
							" . comboData("select * from mst_data where kodeCategory='S03' and statusData='t' order by namaData", "kodeData", "namaData", "inp[sr_city]", " ", $r[sr_city], "", "205px","chosen-select") . "
						</div>
					</p>
				</td>
				<td style=\"width:50%\">
					<p>
						...
						<div class=\"field\">
							<input type=\"text\" id=\"inp[br_nama]\" name=\"inp[br_nama]\" class=\"smallinput\" value=\"$r[br_nama]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Hubungan</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[br_hub]\" name=\"inp[br_hub]\" class=\"smallinput\" value=\"$r[br_hub]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">No. Telp</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[br_phone]\" name=\"inp[br_phone]\" onkeyup=\"cekPhone(this);\" class=\"smallinput\" value=\"$r[br_phone]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Alamat</label>
						<div class=\"field\">
							<input type=\"text\" id=\"inp[br_address]\" name=\"inp[br_address]\" class=\"smallinput\" value=\"$r[br_address]\" />	
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Propinsi</label>
						<div class=\"field\">
							" . comboData("select * from mst_data where kodeCategory='S02' and statusData='t' order by namaData", "kodeData", "namaData", "inp[br_prov]", " ", $r[br_prov], "", "205px","chosen-select") . "
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Kota</label>
						<div class=\"field\">
							" . comboData("select * from mst_data where kodeCategory='S03' and statusData='t' order by namaData", "kodeData", "namaData", "inp[br_city]", " ", $r[br_city], "", "205px","chosen-select") . "
						</div>
					</p>
				</td>
				</tr>
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN DATA KONTAK');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A4:O5')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$objPHPExcel->getActiveSheet()->getStyle('D4:I4')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$objPHPExcel->getActiveSheet()->getStyle('J4:O4')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','SERUMAH');
	$objPHPExcel->getActiveSheet()->setCellValue('D4','BEDA RUMAH');
	
    $objPHPExcel->getActiveSheet()->setCellValue('D5','NAMA');
    $objPHPExcel->getActiveSheet()->setCellValue('E5','HUBUNGAN');
    $objPHPExcel->getActiveSheet()->setCellValue('F5','NO TELP');
	$objPHPExcel->getActiveSheet()->setCellValue('G5','ALAMAT');
	$objPHPExcel->getActiveSheet()->setCellValue('H5','PROPINSI');
	$objPHPExcel->getActiveSheet()->setCellValue('I5','KOTA');
	$objPHPExcel->getActiveSheet()->setCellValue('J5','NAMA');
    $objPHPExcel->getActiveSheet()->setCellValue('K5','HUBUNGAN');
    $objPHPExcel->getActiveSheet()->setCellValue('L5','NO TELP');
	$objPHPExcel->getActiveSheet()->setCellValue('M5','ALAMAT');
	$objPHPExcel->getActiveSheet()->setCellValue('N5','PROPINSI');
	$objPHPExcel->getActiveSheet()->setCellValue('O5','KOTA');
	...
	$sql="SELECT t1.*, t2.name, t2.reg_no from emp_contact t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t2.name";
    $res = db($sql);
    $no=0;
    $arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory IN ('S02', 'S03')");
    while ($r = mysql_fetch_array($res)) {
		$no++;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, strtoupper($r[sr_nama]));
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows,	$r[sr_hub]);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('F'.$rows, $r[sr_phone],PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $r[sr_address]);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, $arrMaster[$r[sr_prov]]);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$rows, $arrMaster[$r[sr_city]]);
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$rows, strtoupper($r[br_nama]));
		$objPHPExcel->getActiveSheet()->setCellValue('K'.$rows,	$r[br_hub]);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('L'.$rows, $r[br_phone],PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcel->getActiveSheet()->setCellValue('M'.$rows, $r[br_address]);
        $objPHPExcel->getActiveSheet()->setCellValue('N'.$rows, $arrMaster[$r[br_prov]]);
        $objPHPExcel->getActiveSheet()->setCellValue('O'.$rows, $arrMaster[$r[br_city]]);
