...
$dFile = "files/emp/adm/peringatan/";
$fFile = "files/export/";
$fLog = "files/logPeringatan.log";
...
function setData(){
	...
	$arrMaster = arrayQuery("select lower(trim(namaData)), kodeData from mst_data where kodeCategory = 'S20'");
	...
	if($par[rowData] <= $highestRow){  
		...
		if(!in_array(trim(strtolower($dta[1])), array("","NPP PEGAWAI"))){          
			...
			$idPeringatan = getField("SELECT id from emp_punish order by id desc limit 1")+1;
			$sql = "insert into emp_punish set id = '$idPeringatan', parent_id = '$parentId', pnh_no = '".$dta[3]."', pnh_subject = '".$dta[4]."', pnh_agency = '".$dta[5]."',  pnh_year = '".$dta[6]."', pnh_type = '".$arrMaster[strtolower(trim($dta[7]))]."', pnh_date_start = '".implode('-', array_reverse(explode('/', $dta[8])))."', pnh_date_end = '".implode('-', array_reverse(explode('/', $dta[9])))."', cre_by = 'migrasi', cre_date = '".date('Y-m-d H:i:s')."'";
			db($sql);
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">
			<form class=\"stdform\" enctype=\"multipart/form-data\">   
				<div id=\"formInput\" class=\"subcontent\">
					...
					<p>
						<div style=\"float:right; margin-top:10px; margin-right:150px;\"><a href=\"download.php?d=tmpPeringatan\" class=\"detil\">* download template.xls</a></div>                
					</p>
				</div>
				...
				<div id=\"progresEnd\" class=\"progress\" style=\"margin-top:30px; display:none;\">                
					<a href=\"download.php?d=logPeringatan\" class=\"btn btn1 btn_inboxi\"><span>Download Result</span></a>               
...
function uploadBukti($id) {
	...
	$fileUpload = $_FILES["pnh_filename"]["tmp_name"];
	$fileUpload_name = $_FILES["pnh_filename"]["name"];
	if (($fileUpload != "") and ( $fileUpload != "none")) {
		fileUpload($fileUpload, $fileUpload_name, $dFile);
		$foto_file = "punish-" . time() . "." . getExtension($fileUpload_name);
		fileRename($dFile, $fileUpload_name, $foto_file);
	}
	if (empty($foto_file))
		$foto_file = getField("select pnh_filename from emp_punish where id ='$id'");
...
function hapusFoto() {
	...
	$foto_file = getField("select pnh_filename from emp_punish where id='$par[id]'");
	if (file_exists($dFile . $foto_file) and $foto_file != "")
		unlink($dFile . $foto_file);

	$sql = "update emp_punish set pnh_filename='' where id='$par[id]'";
	db($sql);
...
function tambah(){
	...
	$id = getField("select id from emp_punish order by id desc limit 1") + 1;
	$inp[pnh_filename] = uploadBukti($id);
	$inp[pnh_date_start] = setTanggal($inp[pnh_date_start]);
	$inp[pnh_date_end] = setTanggal($inp[pnh_date_end]);

	$sql = "insert into emp_punish (id, parent_id, pnh_no, pnh_subject, pnh_filename, remark,pnh_agency,pnh_year,pnh_date_start,pnh_date_end,pnh_type,cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[pnh_no]', '$inp[pnh_subject]', '$inp[pnh_filename]', '$inp[remark]', '$inp[pnh_agency]','$inp[pnh_year]','$inp[pnh_date_start]','$inp[pnh_date_end]','$inp[pnh_type]', '".date('Y-m-d H:i:s')."', '$cUsername')";
	db($sql);
...
function ubah(){
	...
	$pnh_filename = uploadBukti($par[id]);
	$inp[pnh_date_start] = setTanggal($inp[pnh_date_start]);
	$inp[pnh_date_end] = setTanggal($inp[pnh_date_end]);
	$sql = "update emp_punish set pnh_filename='$pnh_filename', parent_id='$inp[parent_id]',pnh_no='$inp[pnh_no]',pnh_subject='$inp[pnh_subject]',  remark='$inp[remark]',pnh_agency='$inp[pnh_agency]',pnh_year='$inp[pnh_year]',pnh_date_start='$inp[pnh_date_start]',pnh_date_end='$inp[pnh_date_end]',pnh_type='$inp[pnh_type]',  upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);
		...
		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\">
			<thead>
				<tr>
					...
					<th>Pegawai</th>
					<th width=\"75\">NPP</th>
					<th width=\"200\">Nomor Peringatan</th>
...
function lData(){
	...
	$sql="select t1.id, t1.pnh_no, t1.pnh_subject, t1.pnh_agency, t1.pnh_type, t1.pnh_year, t1.pnh_filename, t2.name, t2.reg_no from emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);

	$arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory = 'S20'");
	...
	while($r=mysql_fetch_array($res)){
		...
		$download = empty($r[pnh_filename]) ? " - " : "<a href=\"download.php?d=emppnh&f=$r[id]\"><img src=\"".getIcon($r[pnh_filename])."\" align=\"center\" style=\"vertical-align:middle; margin-top: 7px;\" ></a>";
		...
		$data=array(
			...
			"<div align=\"left\">".$r[pnh_no]."</div>",
			"<div align=\"left\">".$r[pnh_subject]."</div>",
			"<div align=\"left\">".$r[pnh_agency]."</div>",
			"<div align=\"center\">".$arrMaster[$r[pnh_type]]."</div>",
			"<div align=\"center\">".$r[pnh_year]."</div>",
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					<label class=\"l-input-small\">Nomor Peringatan</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[pnh_no]\" name=\"inp[pnh_no]\" class=\"smallinput\" value=\"$r[pnh_no]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Perihal</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[pnh_subject]\" name=\"inp[pnh_subject]\" class=\"smallinput\" value=\"$r[pnh_subject]\" />	
					</div>
				</p>
				<table style=\"width:100%\">
					<tr>
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small2\">Penerbit</label>
							<div class=\"fieldB\">
								<input type=\"text\" id=\"inp[pnh_agency]\" name=\"inp[pnh_agency]\" class=\"smallinput\" value=\"$r[pnh_agency]\" />	
							</div>
						</p>
					</td>
					<td style=\"width:50%\">
						<p>
							...
							<div class=\"fieldB\">
								<input type=\"text\" id=\"inp[pnh_year]\" name=\"inp[pnh_year]\" class=\"smallinput\" value=\"$r[pnh_year]\" />	
							</div>
						</p>
					</td>
					</tr>
				...
				<p>
					...
					<div class=\"field\">
						" . comboData("select * from mst_data where kodeCategory='S20' and statusData='t' order by namaData", "kodeData", "namaData", "inp[pnh_type]", " ", $r[pnh_type], "", "225px","chosen-select") . "
				...
				<table style=\"width:100%\">
					...
					<td style=\"width:50%\">
						<p>
							...
							<div class=\"field\">
								<input type=\"text\" id=\"inp[pnh_date_start]\" name=\"inp[pnh_date_start]\"  value=\"".getTanggal($r[pnh_date_start])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
							</div>
						</p>
					</td>
					<td style=\"width:50%\">
						<p>
							...
							<div class=\"field\">
								<input type=\"text\" id=\"inp[pnh_date_end]\" name=\"inp[pnh_date_end]\"  value=\"".getTanggal($r[pnh_date_end])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
							</div>
						</p>
					</td>
					</tr>
				...
				<p>
					<label class=\"l-input-small\">File</label>
					<div class=\"field\">";
						$text.=empty($r[pnh_filename])?
						"<input type=\"text\" id=\"fotoTemp\" name=\"fotoTemp\" class=\"input\" style=\"width:295px;\" maxlength=\"100\" />
						<div class=\"fakeupload\">
							<input type=\"file\" id=\"pnh_filename\" name=\"pnh_filename\" class=\"realupload\" size=\"50\" onchange=\"this.form.fotoTemp.value = this.value;\" />
						</div>":
						"<img src=\"".getIcon($r[pnh_filename])."\" align=\"left\" height=\"25\" style=\"padding-right:5px; padding-bottom:5px;\">
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN DATA PERINGATAN');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','NOMOR PERINGATAN');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('H4','TIPE');
	$objPHPExcel->getActiveSheet()->setCellValue('I4','TGL. MULAI');
	$objPHPExcel->getActiveSheet()->setCellValue('J4','TGL. SELESAI');
	...
	$sql="SELECT t1.*, t2.name, t2.reg_no from emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t2.name";
    $res = db($sql);
    $no=0;
    $arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory = 'S20'");
    while ($r = mysql_fetch_array($res)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $r[pnh_no]);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $r[pnh_subject]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[pnh_agency]);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $r[pnh_year]);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, $arrMaster[$r[pnh_type]]);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$rows, getTanggal($r[pnh_date_start]));
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$rows, getTanggal($r[pnh_date_end]));
