...
$fLog = "files/log/logKesehatan.log";
...
function setData()
{
    ...
    if ($par[rowData] <= $highestRow) {
        ...
        if (!in_array(trim(strtolower($dta[1])), array("", "NIK PEGAWAI"))) {
            ...
            $idKesehatan = getLastId("emp_health", "id");
            $sql = "insert into emp_health set id = '$idKesehatan', parent_id = '$parentId', hlt_date = '" . implode('-', array_reverse(explode('/', $dta[3]))) . "', hlt_place = '" . $dta[4] . "', hlt_doctor = '" . $dta[5] . "', hlt_note = '" . $dta[6] . "', create_by = 'migrasi', create_date = '" . date('Y-m-d H:i:s') . "'";
            db($sql);
...
function tambah()
{
    ...
    $sql = "insert into emp_health set id = '$id', parent_id = '$inp[parent_id]', hlt_place = '$inp[hlt_place]', hlt_doctor = '$inp[hlt_doctor]', hlt_note = '$inp[hlt_note]', hlt_date = '$inp[hlt_date]', filename = '$inp[filename]', remark = '$inp[remark]', create_by = '$inp[create_by]', create_date = '$inp[create_date]'";
    db($sql);
...
function ubah()
{
    ...
    $sql = "update emp_health set parent_id = '$inp[parent_id]', hlt_place = '$inp[hlt_place]', hlt_doctor = '$inp[hlt_doctor]', hlt_note = '$inp[hlt_note]', hlt_date = '$inp[hlt_date]', filename = '$inp[filename]', remark = '$inp[remark]', update_by = '$inp[update_by]', update_date = '$inp[update_date]' where id = '$par[id]'";
    db($sql);
...
function form()
{
    ...
    $sql = "SELECT t1.*, t2.reg_no, t2.status, t2.join_date, t3.pos_name, t3.rank, t2.name as namaPegawai, t2.pic_filename FROM emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') WHERE t1.id = '$par[id]'";
    $res = db($sql);
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            <tbody>
                <?php
                    ...
                    $sql = "SELECT t1.id, t1.hlt_date, t1.hlt_place, t1.hlt_doctor, t1.hlt_note, t1.filename, t2.name, t2.reg_no FROM emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
                    $res = db($sql);
                    $no = 0;
...
function xls()
{
    ...
    $field = array("no", "pegawai", "nik", "tanggal", "nama tempat", "dokter", "hasil");
    ...
    if (!empty($par[idJabatan]))
        $filter .= " and t3.pos_name = '$par[idJabatan]'";
    $sql = "SELECT t1.*, t2.name, t2.reg_no FROM emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
    $res = db($sql);
    $no = 0;
    while ($r = mysql_fetch_assoc($res)) {
        $no++;
