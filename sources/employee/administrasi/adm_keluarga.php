...
function setData(){
	...
	if($par[rowData] <= $highestRow){  
		...
		if(!in_array(trim(strtolower($dta[1])), array("","NPP PEGAWAI"))){          
			...
			$sql = "insert into emp_family set id = '$idFamily', parent_id = '$parentId', name = '".$dta[3]."', rel = '".$arrMaster[trim(strtolower($dta[4]))]."', gender = '".$dta[5]."', birth_place = '".$arrMaster[trim(strtolower($dta[6]))]."', birth_date = '".implode('-', array_reverse(explode('/', $dta[7])))."', cre_by = 'migrasi', cre_date = '".date('Y-m-d H:i:s')."'";
			db($sql);
...
function uploadBukti($id) {
	...
	if (empty($foto_file))
		$foto_file = getField("select rel_filename from emp_family where id ='$id'");
...
function hapusFoto() {
	...
	$foto_file = getField("select rel_filename from emp_family where id='$par[id]'");
...
function tambah(){
	...
	repField();
	$id = getField("select id from emp_family order by id desc limit 1") + 1;
	$inp[rel_filename] = uploadBukti($id);
	$inp[birth_date] = setTanggal($inp[birth_date]);	

	$sql = "insert into emp_family (id, parent_id, name, rel, birth_place, birth_date, gender, rel_filename, remark, status, cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[name]', '$inp[rel]', '$inp[birth_place]', '$inp[birth_date]', '$inp[gender]', '$inp[rel_filename]', '$inp[remark]', '$inp[status]', '".date('Y-m-d H:i:s')."', '$cUsername')";
...
function ubah(){
	...
	$inp[birth_date] = setTanggal($inp[birth_date]);
	$rel_filename = uploadBukti($par[id]);
	$sql = "update emp_family set rel_filename='$rel_filename', parent_id='$inp[parent_id]',name='$inp[name]',rel='$inp[rel]', birth_place='$inp[birth_place]', gender='$inp[gender]', status='$inp[status]',  remark='$inp[remark]', birth_date='$inp[birth_date]', upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);

	echo "<script>alert('UPDATE DATA BERHASIL');window.location='?par[mode]=edit&par[id]=$id" . getPar($par, "mode") . "';</script>";
...
function lData(){
	...
	$sql="SELECT t1.id, t2.name as namaPegawai, t2.reg_no, t1.name as namaKeluarga, t1.rel, CONCAT(TIMESTAMPDIFF(YEAR, t1.birth_date, CURRENT_DATE ),' thn') as umur, t1.rel_filename from emp_family t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";
	// echo $sql;
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_family t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);
	...
	while($r=mysql_fetch_array($res)){
		...
		$download = empty($r[rel_filename]) ? " - " : "<a href=\"download.php?d=emprel&f=$r[id]\"><img src=\"".getIcon($r[rel_filename])."\" align=\"center\" style=\"vertical-align:middle; margin-top: 7px;\" ></a>";
		...
		$data=array(
			...
			"<div align=\"left\">".strtoupper($r[namaKeluarga])."</div>",
			"<div align=\"left\">".$arrMaster[$r[rel]]."</div>",
			"<div align=\"center\">".$r[umur]."</div>",
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					<label class=\"l-input-small\">Nama Lengkap</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[name]\" name=\"inp[name]\" class=\"smallinput\" value=\"$r[name]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Hubungan</label>
					<div class=\"field\">
						" . comboData("select * from mst_data where kodeCategory='S12' and statusData='t' order by namaData", "kodeData", "namaData", "inp[rel]", " ", $r[rel], "", "225px","chosen-select") . "
				...
				<p>
					...
					<div class=\"field\">
						" . comboData("select concat(t2.namaData,' - ', t1.namaData) as namaData, t1.kodeData from mst_data t1 join mst_data t2 on t1.kodeInduk = t2.kodeData where t1.kodeCategory='S03' and t1.statusData='t' order by namaData", "kodeData", "namaData", "inp[birth_place]", " ", $r[birth_place], "", "225px","chosen-select") . "
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Tanggal Lahir</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[birth_date]\" name=\"inp[birth_date]\"  value=\"".getTanggal($r[birth_date])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Bukti</label>
					<div class=\"field\">";
						$text.=empty($r[rel_filename])?
						"<input type=\"text\" id=\"fotoTemp\" name=\"fotoTemp\" class=\"input\" style=\"width:295px;\" maxlength=\"100\" />
						<div class=\"fakeupload\">
							<input type=\"file\" id=\"rel_filename\" name=\"rel_filename\" class=\"realupload\" size=\"50\" onchange=\"this.form.fotoTemp.value = this.value;\" />
						</div>":
						"<img src=\"".getIcon($dFile."".$r[rel_filename])."\" align=\"left\" height=\"20\" style=\"padding-right:5px; padding-bottom:5px;\">
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN DATA KELUARGA');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','NAMA KELUARGA');
    $objPHPExcel->getActiveSheet()->setCellValue('E4','HUBUNGAN');
    $objPHPExcel->getActiveSheet()->setCellValue('F4','JENIS KELAMIN');
    $objPHPExcel->getActiveSheet()->setCellValue('G4','TEMPAT LAHIR');
	$objPHPExcel->getActiveSheet()->setCellValue('H4','TANGGAL LAHIR');
	...
	$sql="SELECT t1.id, t2.name as namaPegawai, t2.reg_no, t1.name as namaKeluarga, t1.rel, t1.birth_place, t1.birth_date from emp_family t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t1.name";
    $res = db($sql);
    $no=0;
    $arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory IN ('S03', 'S12')");
    while ($r = mysql_fetch_array($res)) {
		$no++;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, strtoupper($r[namaKeluarga]));
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $arrMaster[$r[rel]]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[gender]);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $arrMaster[$r[birth_place]]);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, getTanggal($r[birth_date]));
