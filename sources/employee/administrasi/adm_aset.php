...
$dFile = "files/emp/adm/aset/";
$fFile = "files/export/";
$fLog = "files/logAset.log";
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">
			<form class=\"stdform\" enctype=\"multipart/form-data\">   
				<div id=\"formInput\" class=\"subcontent\">
					...
					<p>
						<div style=\"float:right; margin-top:10px; margin-right:150px;\"><a href=\"download.php?d=tmpAset\" class=\"detil\">* download template.xls</a></div>                
					</p>
				</div>
				...
				<div id=\"progresEnd\" class=\"progress\" style=\"margin-top:30px; display:none;\">                
					<a href=\"download.php?d=logAset\" class=\"btn btn1 btn_inboxi\"><span>Download Result</span></a>               
...
function uploadBukti($id) {
	...
	$fileUpload = $_FILES["ast_filename"]["tmp_name"];
	$fileUpload_name = $_FILES["ast_filename"]["name"];
	if (($fileUpload != "") and ( $fileUpload != "none")) {
		fileUpload($fileUpload, $fileUpload_name, $dFile);
		$foto_file = "aset-" . time() . "." . getExtension($fileUpload_name);
		fileRename($dFile, $fileUpload_name, $foto_file);
	}
	if (empty($foto_file))
		$foto_file = getField("select ast_filename from emp_asset where id ='$id'");
...
function hapusFoto() {
	...
	$foto_file = getField("select ast_filename from emp_asset where id='$par[id]'");
	if (file_exists($dFile . $foto_file) and $foto_file != "")
		unlink($dFile . $foto_file);

	$sql = "update emp_asset set ast_filename='' where id='$par[id]'";
	db($sql);
...
function tambah(){
	...
	repField();
	$id = getField("select id from emp_asset order by id desc limit 1") + 1;
	$inp[ast_filename] = uploadBukti($id);
	...
	$sql = "insert into emp_asset (id, parent_id, ast_name, ast_no, ast_date,ast_date_start, ast_date_end, ast_filename, remark,ast_usage,ast_cat,ast_type,ast_jenis, status, cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[ast_name]', '$inp[ast_no]', '$inp[ast_date]', '$inp[ast_date_start]', '$inp[ast_date_end]', '$inp[ast_filename]', '$inp[remark]', '$inp[ast_usage]','$inp[ast_cat]','$inp[ast_type]','$inp[ast_jenis]','$inp[status]', '".date('Y-m-d H:i:s')."', '$cUsername')";
	db($sql);
...
function ubah(){
	...
	$ast_filename = uploadBukti($par[id]);
	$inp[ast_date] = setTanggal($inp[ast_date]);	
	$inp[ast_date_start] = setTanggal($inp[ast_date_start]);	
	$inp[ast_date_end] = setTanggal($inp[ast_date_end]);
	$sql = "update emp_asset set ast_filename='$ast_filename', parent_id='$inp[parent_id]',ast_name='$inp[ast_name]',ast_no='$inp[ast_no]', ast_date='$inp[ast_date]',  ast_date_start='$inp[ast_date_start]', ast_date_end='$inp[ast_date_end]',ast_usage='$inp[ast_usage]', status='$inp[status]',  remark='$inp[remark]',ast_cat='$inp[ast_cat]',ast_type='$inp[ast_type]',ast_jenis='$inp[ast_jenis]', upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);
...
function lihat(){
	global $s, $par, $arrParameter, $arrTitle, $menuAccess;		
...
function lData(){
	...
	$sql="select t1.id, t1.ast_name, t1.ast_no, t1.ast_cat, t1.ast_type, t1.ast_date, t1.ast_filename, t2.name, t2.reg_no from emp_asset t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";	
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_asset t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);
	...
	while($r=mysql_fetch_array($res)){
		$no++;
		$controlEmp="";
		$download = empty($r[ast_filename]) ? " - " : "<a href=\"download.php?d=empast&f=$r[id]\"><img src=\"".getIcon($r[ast_filename])."\" align=\"center\" style=\"vertical-align:middle; margin-top: 7px;\" ></a>";
		...
		$data=array(
			...
			"<div align=\"left\">".$r[ast_name]."</div>",
			"<div align=\"left\">".$r[ast_no]."</div>",
			"<div align=\"left\">".$arrMaster[$r[ast_cat]]."</div>",
			"<div align=\"left\">".$arrMaster[$r[ast_type]]."</div>",
			"<div align=\"left\">".getTanggal($r[ast_date])."</div>",
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					<label class=\"l-input-small\">Nama Aset</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[ast_name]\" name=\"inp[ast_name]\" class=\"smallinput\" value=\"$r[ast_name]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Nomor Seri</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[ast_no]\" name=\"inp[ast_no]\" class=\"smallinput\" value=\"$r[ast_no]\" />	
					</div>
				</p>
				<table style=\"width:100%\">
					<tr>
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small2\">Peruntukan</label>
							<div class=\"field\">
								<input type=\"text\" id=\"inp[ast_usage]\" name=\"inp[ast_usage]\" class=\"smallinput\" value=\"$r[ast_usage]\" />	
							</div>
						</p>
					</td>
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small\">Diberikan Sejak</label>
							<div class=\"field\">
								<input type=\"text\" id=\"inp[ast_date]\" name=\"inp[ast_date]\"  value=\"".getTanggal($r[ast_date])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
							</div>
						</p>
					</td>
					</tr>
					...
					<tr>
						<td style=\"width:50%\">
							<p>
								...
								<div class=\"field\">
									" . comboData("select * from mst_data where kodeCategory='S21' and statusData='t' order by namaData", "kodeData", "namaData", "inp[ast_cat]", " ", $r[ast_cat], "", "225px","chosen-select") . "
								</div>
							</p>
						</td>
						<td style=\"width:50%\">
							<p>
								...
								<div class=\"field\">
									" . comboData("select * from mst_data where kodeCategory='S22' and statusData='t' order by namaData", "kodeData", "namaData", "inp[ast_type]", " ", $r[ast_type], "", "225px","chosen-select") . "
								</div>
							</p>
						</td>
					</tr>
					<tr>
						<td style=\"width:50%\">
							<p>
								...
								<div class=\"field\">
									<input type=\"text\" id=\"inp[ast_date_start]\" name=\"inp[ast_date_start]\"  value=\"".getTanggal($r[ast_date_start])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
								</div>
							</p>
						</td>
						<td style=\"width:50%\">
							<p>
								...
								<div class=\"field\">
									<input type=\"text\" id=\"inp[ast_date_end]\" name=\"inp[ast_date_end]\"  value=\"".getTanggal($r[ast_date_end])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
								</div>
							</p>
						</td>
					</tr>
				...
				<p>
					...
					<div class=\"field\">
						<input type=\"text\" id=\"inp[ast_jenis]\" name=\"inp[ast_jenis]\" class=\"smallinput\" value=\"$r[ast_jenis]\" />	
					</div>
				</p>
				<p>
				...
				<p>
					...
					<div class=\"field\">";
						$text.=empty($r[ast_filename])?
						"<input type=\"text\" id=\"fotoTemp\" name=\"fotoTemp\" class=\"input\" style=\"width:295px;\" maxlength=\"100\" />
						<div class=\"fakeupload\">
							<input type=\"file\" id=\"ast_filename\" name=\"ast_filename\" class=\"realupload\" size=\"50\" onchange=\"this.form.fotoTemp.value = this.value;\" />
						</div>":
						"<img src=\"".getIcon($r[ast_filename])."\" align=\"left\" height=\"20\" style=\"padding-right:5px; padding-bottom:5px;\">
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN DATA KESEHATAN');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','NAMA ASET');
    $objPHPExcel->getActiveSheet()->setCellValue('E4','NOMOR SERI');
    $objPHPExcel->getActiveSheet()->setCellValue('F4','PERUNTUKAN');
	$objPHPExcel->getActiveSheet()->setCellValue('G4','DIBERIKAN SEJAK');
	$objPHPExcel->getActiveSheet()->setCellValue('H4','KATEGORI');
	$objPHPExcel->getActiveSheet()->setCellValue('I4','TIPE');
	$objPHPExcel->getActiveSheet()->setCellValue('J4','TANGGAL BERLAKU');
	$objPHPExcel->getActiveSheet()->setCellValue('K4','TANGGAL BERAKHIR');
	$objPHPExcel->getActiveSheet()->setCellValue('L4','JENIS');
	...
	$sql="SELECT t1.*, t2.name, t2.reg_no from emp_asset t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t2.name";
    $res = db($sql);
    $no=0;
    while ($r = mysql_fetch_array($res)) {
		$no++;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $r[ast_name]);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $r[ast_no]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[ast_usage]);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, getTanggal($r[ast_date]));
        $objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, $arrMaster[$r[ast_cat]]);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$rows, $arrMaster[$r[ast_type]]);
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$rows, getTanggal($r[ast_date_start]));
		$objPHPExcel->getActiveSheet()->setCellValue('K'.$rows, getTanggal($r[ast_date_end]));
		$objPHPExcel->getActiveSheet()->setCellValue('L'.$rows, $r[ast_jenis]);
