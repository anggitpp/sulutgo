...
$dFile = "files/emp/adm/pendidikan/";
$fFile = "files/export/";
...
function setData(){
	...
	if($par[rowData] <= $highestRow){  
		$rowData = $sheet->rangeToArray('A' . $par[rowData] . ':J' . $par[rowData], NULL, TRUE, TRUE);             
		$dta = $rowData[0];
		$tRow = 6;
		...
		if(!in_array(trim(strtolower($dta[1])), array("","NPP PEGAWAI"))){          
			...
			$idPendidikan = getField("SELECT id from emp_edu order by id desc limit 1")+1;
			$sql = "insert into emp_edu set id = '$idPendidikan', parent_id = '$parentId', edu_type = '".$arrMaster[strtolower(trim($dta[3]))]."', edu_name = '".$dta[4]."', edu_city = '".$arrMaster[strtolower(trim($dta[5]))]."', edu_year = '".$dta[6]."', edu_fac = '".$arrMaster[strtolower(trim($dta[7]))]."', edu_dept = '".$arrMaster[strtolower(trim($dta[8]))]."',  edu_essay = '".$dta[9]."', cre_by = 'migrasi', cre_date = '".date('Y-m-d H:i:s')."'";
			db($sql);
...
function jurusan() {
	...
	$data = arrayQuery("select concat(kodeData, '\t', namaData) from mst_data where statusData='t' and kodeInduk='$par[kodeFakultas]' and kodeCategory='R13' order by namaData");
...
function uploadBukti($id) {
	...
	if (($fileUpload != "") and ( $fileUpload != "none")) {
		...
		$foto_file = "edu-" . time() . "." . getExtension($fileUpload_name);
		fileRename($dFile, $fileUpload_name, $foto_file);
	}
	if (empty($foto_file))
		$foto_file = getField("select edu_filename from emp_edu where id ='$id'");
...
function hapusFoto() {
	...
	$foto_file = getField("select edu_filename from emp_edu where id='$par[id]'");
	if (file_exists($dFile . $foto_file) and $foto_file != "")
		unlink($dFile . $foto_file);

	$sql = "update emp_edu set edu_filename='' where id='$par[id]'";
	db($sql);
...
function tambah(){
	...
	$inp[edu_filename] = uploadBukti($id);
	$sql = "insert into emp_edu (id, parent_id, edu_type, edu_name, edu_city, edu_year, edu_fac, edu_dept, edu_essay, edu_filename, remark, cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[edu_type]', '$inp[edu_name]', '$inp[edu_city]', '$inp[edu_year]', '$inp[edu_fac]','$inp[edu_dept]','$inp[edu_essay]','$inp[edu_filename]','$inp[remark]', '".date('Y-m-d H:i:s')."', '$cUsername')";
	db($sql);
...
function ubah(){
	...
	$edu_filename = uploadBukti($par[id]);
	$sql = "update emp_edu set edu_filename='$edu_filename', parent_id='$inp[parent_id]',edu_name='$inp[edu_name]',edu_type='$inp[edu_type]',  remark='$inp[remark]',edu_year='$inp[edu_year]',edu_fac='$inp[edu_fac]',edu_dept='$inp[edu_dept]',edu_essay='$inp[edu_essay]',edu_city='$inp[edu_city]',  upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);
...
function lihat(){
	global $s, $par, $arrParameter, $arrTitle, $menuAccess, $areaCheck;		
		...
		<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dataList\">
			<thead>
				<tr>
					...
					<th width=\"200\">Nama Lembaga</th>
					<th width=\"100\">Jurusna</th>
...
function lData(){
	...
	$sql="select t1.id, t1.edu_type, t1.edu_name, t1.edu_dept, t1.edu_city, t1.edu_year, t1.edu_filename, t2.name, t2.reg_no from emp_edu t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_edu t1 join emp t2 on t1.parent_id = t2.id join emp_phist on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);

	$arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory IN('R11', 'R12', 'R13', 'S03')");

	$no=intval($_GET['iDisplayStart']);
	while($r=mysql_fetch_array($res)){
		$no++;
		$controlEmp="";
		$download = empty($r[edu_filename]) ? " - " : "<a href=\"download.php?d=empedu&f=$r[id]\"><img src=\"".getIcon($r[edu_filename])."\" align=\"center\" style=\"vertical-align:middle; margin-top: 7px;\" ></a>";
		...
		$data=array(
			...
			"<div align=\"left\">".$arrMaster[$r[edu_type]]."</div>",
			"<div align=\"left\">".$r[edu_name]."</div>",
			"<div align=\"left\">".$arrMaster[$r[edu_dept]]."</div>",
			"<div align=\"left\">".$arrMaster[$r[edu_city]]."</div>",
			"<div align=\"center\">".$r[edu_year]."</div>",
		...
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					...
					<div class=\"field\">
						" . comboData("select * from mst_data where kodeCategory='R11' and statusData='t' order by statusData", "kodeData", "namaData", "inp[edu_type]", " ", $r[edu_type], "", "225px","chosen-select") . "
					</div>
				</p>	
				<p>
					<label class=\"l-input-small\">Nama Lembaga</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[edu_name]\" name=\"inp[edu_name]\" class=\"smallinput\" value=\"$r[edu_name]\" />	
					</div>
				</p>
				<table style=\"width:100%\">
					<tr>
					<td style=\"width:50%\">
						<p>
							...
							<div class=\"field\">
								" . comboData("select * from mst_data where kodeCategory='S03' and statusData='t' order by namaData", "kodeData", "namaData", "inp[edu_city]", " ", $r[edu_city], "", "225px","chosen-select") . "
							</div>
						</p>
					</td>
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small\">Tahun Lulus</label>
							<div class=\"field\">
									<input type=\"text\" id=\"inp[edu_year]\" name=\"inp[edu_year]\" class=\"smallinput\" value=\"$r[edu_year]\" />	
							</div>
						</p>
					</td>
					</tr>
				...
				<table style=\"width:100%\">
					...
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small2\">Fakultas</label>
							<div class=\"field\">
								" . comboData("select * from mst_data where kodeCategory='R12' and statusData='t' order by namaData", "kodeData", "namaData", "inp[edu_fac]", " ", $r[edu_fac], "onchange=\"getJurusan('" . getPar($par, "mode,edu_fac") . "');\"", "225px","chosen-select") . "
							</div>
						</p>
					</td>
					<td style=\"width:50%\">
						<p>
							<label class=\"l-input-small\">Jurusan</label>
							<div class=\"field\">
								" . comboData("select * from mst_data where kodeCategory='R13' and kodeInduk='$r[edu_fac]' and statusData='t' order by namaData", "kodeData", "namaData", "inp[edu_dept]", " ", $r[edu_dept], "", "225px","chosen-select") . "
							</div>
						</p>
					</td>
					</tr>
				...
				<p>
					<label class=\"l-input-small\">Judul Skripsi/</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[edu_essay]\" name=\"inp[edu_essay]\" class=\"smallinput\" value=\"$r[edu_essay]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">File</label>
					<div class=\"field\">";
						$text.=empty($r[edu_filename])?
						"<input type=\"text\" id=\"fotoTemp\" name=\"fotoTemp\" class=\"input\" style=\"width:295px;\" maxlength=\"100\" />
						<div class=\"fakeupload\">
							<input type=\"file\" id=\"edu_filename\" name=\"edu_filename\" class=\"realupload\" size=\"50\" onchange=\"this.form.fotoTemp.value = this.value;\" />
						</div>":
						"<img src=\"".getIcon($r[edu_filename])."\" align=\"left\" height=\"25\" style=\"padding-right:5px; padding-bottom:5px;\">
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN DATA PENDIDIKAN');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A4:K4')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','TIPE PENDIDIKAN');
    $objPHPExcel->getActiveSheet()->setCellValue('E4','NAMA LEMBAGA');
    $objPHPExcel->getActiveSheet()->setCellValue('F4','KOTA');
    $objPHPExcel->getActiveSheet()->setCellValue('G4','TAHUN LULUS');
	$objPHPExcel->getActiveSheet()->setCellValue('H4','FAKULTAS');
	$objPHPExcel->getActiveSheet()->setCellValue('I4','JURUSAN');
	$objPHPExcel->getActiveSheet()->setCellValue('J4','JUDUL SKRIPSI');
	$objPHPExcel->getActiveSheet()->setCellValue('K4','KETERANGAN');
	...
	$sql="SELECT t1.*, t2.name, t2.reg_no from emp_edu t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t2.name";
    $res = db($sql);
    $no=0;
    $arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory IN('R11', 'R12', 'R13', 'S03')");
    while ($r = mysql_fetch_array($res)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, $arrMaster[$r[edu_type]]);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $r[edu_name]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $arrMaster[$r[edu_city]]);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $r[edu_year]);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, $arrMaster[$r[edu_fac]]);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$rows, $arrMaster[$r[edu_dept]]);
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$rows, $r[edu_essay]);
        $objPHPExcel->getActiveSheet()->setCellValue('K'.$rows, $r[remark]);
