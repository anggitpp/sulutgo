...
$dFile = "files/emp/adm/kesehatan/";
$fFile = "files/export/";
$fLog = "files/logKesehatan.log";
...
function setData(){
	...
	if($par[rowData] <= $highestRow){  
		...
		if(!in_array(trim(strtolower($dta[1])), array("","NPP PEGAWAI"))){          
			...
			$idKesehatan = getField("SELECT id from emp_health order by id desc limit 1")+1;
			$sql = "insert into emp_health set id = '$idKesehatan', parent_id = '$parentId', hlt_date = '".implode('-', array_reverse(explode('/', $dta[3])))."', hlt_place = '".$dta[4]."', hlt_doctor = '".$dta[5]."', hlt_note = '".$dta[6]."', cre_by = 'migrasi', cre_date = '".date('Y-m-d H:i:s')."'";
			db($sql);
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">
			<form class=\"stdform\" enctype=\"multipart/form-data\">   
				<div id=\"formInput\" class=\"subcontent\">
					...
					<p>
						<div style=\"float:right; margin-top:10px; margin-right:150px;\"><a href=\"download.php?d=tmpKesehatan\" class=\"detil\">* download template.xls</a></div>                
					</p>
				</div>
				...
				<div id=\"progresEnd\" class=\"progress\" style=\"margin-top:30px; display:none;\">                
					<a href=\"download.php?d=logKesehatan\" class=\"btn btn1 btn_inboxi\"><span>Download Result</span></a>               
...
function uploadBukti($id) {
	...
	$fileUpload = $_FILES["hlt_filename"]["tmp_name"];
	$fileUpload_name = $_FILES["hlt_filename"]["name"];
	if (($fileUpload != "") and ( $fileUpload != "none")) {
		fileUpload($fileUpload, $fileUpload_name, $dFile);
		$foto_file = "health-" . time() . "." . getExtension($fileUpload_name);
		fileRename($dFile, $fileUpload_name, $foto_file);
	}
	if (empty($foto_file))
		$foto_file = getField("select hlt_filename from emp_health where id ='$id'");
...
function hapusFoto() {
	...
	$foto_file = getField("select hlt_filename from emp_health where id='$par[id]'");
	if (file_exists($dFile . $foto_file) and $foto_file != "")
		unlink($dFile . $foto_file);

	$sql = "update emp_health set hlt_filename='' where id='$par[id]'";
	db($sql);
...
function tambah(){
	...
	$id = getField("select id from emp_health order by id desc limit 1") + 1;
	$inp[hlt_filename] = uploadBukti($id);
	$inp[hlt_date] = setTanggal($inp[hlt_date]);	

	$sql = "insert into emp_health (id, parent_id, hlt_place, hlt_doctor, hlt_date, hlt_note, hlt_filename, hlt_remark, cre_date, cre_by) values ('$id', '$inp[parent_id]', '$inp[hlt_place]', '$inp[hlt_doctor]', '$inp[hlt_date]', '$inp[hlt_note]', '$inp[hlt_filename]', '$inp[hlt_remark]','".date('Y-m-d H:i:s')."', '$cUsername')";
	db($sql);
...
function ubah(){
	...
	$hlt_filename = uploadBukti($par[id]);
	$inp[hlt_date] = setTanggal($inp[hlt_date]);	
	$sql = "update emp_health set hlt_filename='$hlt_filename', parent_id='$inp[parent_id]',hlt_place='$inp[hlt_place]',hlt_doctor='$inp[hlt_doctor]', hlt_date='$inp[hlt_date]', hlt_note='$inp[hlt_note]',hlt_remark='$inp[hlt_remark]', upd_by = '$cUsername', upd_date = '".date('Y-m-d H:i:s')."' where id='$par[id]'";
	db($sql);
...
function lData(){
	...
	$sql="select t1.id, t1.hlt_date, t1.hlt_place, t1.hlt_doctor, t1.hlt_remark, t1.hlt_filename, t2.name, t2.reg_no from emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by $orderBy $sLimit";
	$res=db($sql);

	$json = array(
		"iTotalRecords" => mysql_num_rows($res),
		"iTotalDisplayRecords" => getField("select count(t1.id) from emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere"),
		"aaData" => array(),
	);
	...
	while($r=mysql_fetch_array($res)){
		...
		$download = empty($r[hlt_filename]) ? " - " : "<a href=\"download.php?d=emphlt&f=$r[idKerja]\"><img src=\"".getIcon($r[hlt_filename])."\" align=\"center\" style=\"vertical-align:middle; margin-top: 7px;\" ></a>";
		...
		$data=array(
			...
			"<div align=\"left\">".getTanggal($r[hlt_date])."</div>",
			"<div align=\"left\">".$r[hlt_place]."</div>",
			"<div align=\"left\">".$r[hlt_doctor]."</div>",
			"<div align=\"center\">".$r[hlt_remark]."</div>",
...
function form(){
	global $s, $par, $arrTitle, $dFile, $tab;
		...
		<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" onsubmit=\"return validation(document.form);\" enctype=\"multipart/form-data\">
			...
			<fieldset>
				...
				<p>
					...
					<div class=\"field\">
						<input type=\"text\" id=\"inp[hlt_date]\" name=\"inp[hlt_date]\"  value=\"".getTanggal($r[hlt_date])."\" class=\"hasDatePicker\" maxlength=\"150\"/>
					</div>
				</p>
					<p>
					<label class=\"l-input-small\">Nama Tempat</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[hlt_place]\" name=\"inp[hlt_place]\" class=\"smallinput\" value=\"$r[hlt_place]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Dokter</label>
					<div class=\"field\">
						<input type=\"text\" id=\"inp[hlt_doctor]\" name=\"inp[hlt_doctor]\" class=\"smallinput\" value=\"$r[hlt_doctor]\" />	
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Penjelasan Hasil</label>
					<div class=\"field\">
						<textarea id=\"inp[hlt_note]\" name=\"inp[hlt_note]\"  rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:60%;\">$r[hlt_note]</textarea>
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Keterangan</label>
					<div class=\"field\">
						<textarea id=\"inp[hlt_remark]\" name=\"inp[hlt_remark]\"  rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:60%;\">$r[hlt_remark]</textarea>
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">File</label>
					<div class=\"field\">";
						$text.=empty($r[hlt_filename])?
						"<input type=\"text\" id=\"fotoTemp\" name=\"fotoTemp\" class=\"input\" style=\"width:295px;\" maxlength=\"100\" />
						<div class=\"fakeupload\">
							<input type=\"file\" id=\"hlt_filename\" name=\"hlt_filename\" class=\"realupload\" size=\"50\" onchange=\"this.form.fotoTemp.value = this.value;\" />
						</div>":
						"<img src=\"".$dFile."".$r[hlt_filename]."\" align=\"left\" height=\"25\" style=\"padding-right:5px; padding-bottom:5px;\">
...
function xls(){		
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('D4','MULAI');
    $objPHPExcel->getActiveSheet()->setCellValue('E4','NAMA TEMPAT');
    $objPHPExcel->getActiveSheet()->setCellValue('F4','DOKTER');
	$objPHPExcel->getActiveSheet()->setCellValue('G4','PENJELASAN HASIL');
	...
	$sql="SELECT t1.*, t2.name, t2.reg_no from emp_health t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $sWhere order by t2.name";
    $res = db($sql);
    $no=0;
    while ($r = mysql_fetch_array($res)) {
		$no++;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, getTanggal($r[hlt_date]));
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $r[hlt_place]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[hlt_doctor]);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $r[hlt_note]);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.$rows, $r[hlt_remark]);
