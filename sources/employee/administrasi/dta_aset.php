...
function form()
{
    ...
    $sql = "SELECT t1.*, t2.reg_no, t2.status as statusPegawai, t2.join_date, t3.pos_name, t3.rank, t2.name as namaPegawai, t2.pic_filename FROM emp_asset t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') WHERE t1.id = '$par[id]'";
    $res = db($sql);
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            <tbody>
                <?php
                    ...
                    $sql = "SELECT t1.*, t2.name, t2.reg_no FROM emp_asset t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
                    $res = db($sql);
                    $no = 0;
                    while ($r = mysql_fetch_assoc($res)) {
                        $no++;
                        $r[status] = $r[status] == "t" ? "<img src=\"styles/images/t.png\" title=\"Aktif\">" : "<img src=\"styles/images/f.png\" title=\"Tidak Aktif\">";
...
function xls()
{
    ...
    $field = array("no", "pegawai", "nik", "aset", "no seri", "kategori", "tipe", "tanggal", "status");
    ...
    while ($r = mysql_fetch_assoc($res)) {
        ...
        $data[] = array(
            ...
            $r[ast_name] . "\t left",
            $r[ast_no] . "\t left",
            $arrMaster[$r[ast_cat]] . "\t left",
            $arrMaster[$r[ast_type]] . "\t left",
            getTanggal($r[ast_date]) . "\t right",
            $r[status] . "\t left",
        );
    }
