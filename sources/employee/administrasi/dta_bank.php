...
function setData()
{
    ...
    if ($par[rowData] <= $highestRow) {
        ...
        if (!in_array(trim(strtolower($dta[1])), array("", "ID"))) {
            ...
            $bank_id = $arrMaster[trim(strtolower($dta[4]))];
            $branch = $dta[5];
            $account_no = $dta[6];
            $account_name = $dta[7];
            $status = 't';
            ...
            $exist = getField("SELECT id FROM emp_bank WHERE parent_id = '$parentId' AND account_no = '$account_no'");
            if(!$exist) {
                $sql = "insert into emp_bank set id = '$idBank', parent_id = '$parentId', bank_id = '$bank_id', branch = '$branch', 
                account_no = '$account_no', account_name = '$account_name', status = '$status', create_by = 'migrasi', 
                create_date = '" . date('Y-m-d H:i:s') . "'";
                db($sql);
            }else{
                $sql = "UPDATE emp_bank set bank_id = '$bank_id', branch = '$branch', account_no = '$account_no', account_name = '$account_name', 
                status = '$status', update_by = 'migrasi', update_date = '" . date('Y-m-d H:i:s') . "' WHERE id = '$exist'";
                db($sql);
            }
...
function tambah()
{
    ...
    $sql = "insert into emp_bank set id = '$id', parent_id = '$inp[parent_id]', bank_id = '$inp[bank_id]', branch = '$inp[branch]', account_no = '$inp[account_no]', account_name = '$inp[account_name]', status = '$inp[status]', filename = '$inp[filename]', remark = '$inp[remark]', create_by = '$inp[create_by]', create_date = '$inp[create_date]'";
    db($sql);
        ...
        <div id="contentwrapper" class="contentwrapper">
            <form class="stdform" enctype="multipart/form-data">
                <div id="formInput" class="subcontent">
                    ...
                    <p>
                        <div style="float:right; margin-top:10px; margin-right:150px;"><a href="download.php?d=tmpBank" class="detil">* download template.xls</a></div>
                    </p>
                </div>
                ...
                <div id="progresEnd" class="progress" style="margin-top:30px; display:none;">
                    ...
                    <input type="button" class="cancel radius2" style="float:right" value="Close" onclick="window.parentlocation='index.php?<?= getPar($par, 'mode') ?>';" />
                </div>
                <br clear="all">
...
function form()
{
    ...
    $sql = "SELECT t1.*, t1.status as statusBank, t2.reg_no, t2.status, t2.join_date, t3.pos_name, t3.rank, t2.name as namaPegawai, t2.pic_filename FROM emp_bank t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') WHERE t1.id = '$par[id]'";
    $res = db($sql);
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>" onsubmit="return validation(document.form);" enctype="multipart/form-data">
            ...
            <fieldset>
                ...
                <p><?= $ui->createField("No Rekening", "inp[account_no]", $r[account_no], "") ?></p>
                <p><?= $ui->createField("Atas Nama", "inp[account_name]", $r[account_name], "") ?><p>
                <p><?= $ui->createFile("File", "filename", $r[filename], "", "", "empBank", $r[id], "delFile") ?> </p>
                <p><?= $ui->createTextArea("Keterangan", "inp[remark]", $r[remark]) ?></p>
                <p><?= $ui->createRadio("Status", "inp[status]", array("t" => "Aktif", "f" => "Tidak Aktif"), $r[statusBank]) ?></p>
            </fieldset>
        </form>
    </div>
<?php
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            <tbody>
                <?php
                    ...
                    $sql = "SELECT t1.id, t1.bank_id, t1.account_no, t1.branch, t1.status, t2.name, t2.reg_no, t2.kode FROM emp_bank t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
                    $res = db($sql);
                    $no = 0;
                    while ($r = mysql_fetch_assoc($res)) {
                        $no++;
                        $r[status] = $r[status] == "t" ? "<img src=\"styles/images/t.png\" title=\"Aktif\">" : "<img src=\"styles/images/f.png\" title=\"Tidak aAktif\">";
...
function xls()
{
    ...
    $field = array("no","id", "pegawai", "nik", "nama bank", "cabang", "no rekening", "atas nama", "status");
    ...
    if (!empty($par[idJabatan]))
        $filter .= " and t3.pos_name = '$par[idJabatan]'";
    $sql = "SELECT t1.bank_id, t1.branch, t1.account_no, t1.account_name, t1.status, t2.name, t2.reg_no, t2.kode FROM emp_bank t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
    $res = db($sql);
    $no = 0;
    while ($r = mysql_fetch_assoc($res)) {
        $no++;
