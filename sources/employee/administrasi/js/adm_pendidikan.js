function getJurusan(getPar) {
    kodeFakultas = document.getElementById('inp[edu_fac]');
    kodeJurusan = document.getElementById('inp[edu_dept]');
    var xmlHttp = getXMLHttp();

    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            for (var i = kodeJurusan.options.length - 1; i >= 0; i--) {
                kodeJurusan.remove(i);
            }
            if (xmlHttp.responseText) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    jQuery("#inp\\[edu_dept\\]").trigger("chosen:updated");
                }
            }
        }
    }
    xmlHttp.open("GET", "ajax.php?par[mode]=jurusan&par[kodeFakultas]=" + kodeFakultas.value + getPar, true);
    xmlHttp.send(null);
    return false;
}
