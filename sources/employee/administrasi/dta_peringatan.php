...
$fLog = "files/log/logPeringatan.log";
...
function setData()
{
    ...
    if ($par[rowData] <= $highestRow) {
        ...
        if (!in_array(trim(strtolower($dta[1])), array("", "ID"))) {
            ...
            $pnh_agency = $dta[6];
            $pnh_year = $dta[7];
            $pnh_type = $arrMaster[strtolower(trim($dta[8]))];
            $pnh_date_start = implode('-', array_reverse(explode('/', $dta[9])));
            $pnh_date_end = implode('-', array_reverse(explode('/', $dta[10])));
            $remark = $dta[11];

            $exist = getField("SELECT id FROM emp_punish WHERE parent_id = '$parentId' AND pnh_no = '".$pnh_no."'");

            if($exist) {
                $sql = "UPDATE emp_punish SET pnh_no = '$pnh_no', pnh_subject = '$pnh_subject', pnh_agency = '$pnh_agency', pnh_year = '$pnh_year', 
                pnh_type = '$pnh_type', pnh_date_start = '$pnh_date_start', pnh_date_end = '$pnh_date_end', remark = '$remark',
                update_by = 'migrasi', update_date = '" . date('Y-m-d H:i:s') . "' WHERE id = '$exist'";
                db($sql);
            }else {
                $id = getLastId("emp_punish", "id");
                $sql = "insert into emp_punish set id = '$id', parent_id = '$parentId', pnh_no = '$pnh_no', pnh_subject = '$pnh_subject', 
                pnh_agency = '$pnh_agency', pnh_year = '$pnh_year', pnh_type = '$pnh_type', pnh_date_start = '$pnh_date_start', 
                pnh_date_end = '$pnh_date_end', remark = '$remark', create_by = 'migrasi', create_date = '" . date('Y-m-d H:i:s') . "'";
//            $result.=$sql;
                db($sql);
            }
...
function form()
{
    ...
    $sql = "SELECT t1.*, t2.reg_no, t2.status, t2.join_date, t3.pos_name, t3.rank, t2.name as namaPegawai, t2.pic_filename FROM emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') WHERE t1.id = '$par[id]'";
    $res = db($sql);
    ...
    setValidation("is_null", "inp[pnh_no]", "Anda Harus Mengisi Nomor Peringatan");
    setValidation("is_null", "inp[pnh_subject]", "Anda Harus Mengisi Perihal");
    setValidation("is_null", "inp[pnh_agency]", "Anda Harus Mengisi Penerbit");
    setValidation("is_null", "inp[pnh_year]", "Anda Harus Mengis Tahun");
    setValidation("is_null", "inp[pnh_type]", "Anda Harus Mengisi Tipe Peringatan");
    setValidation("is_null", "inp[pnh_date_start]", "Anda Harus Mengisi Tanggal Mulai");
    setValidation("is_null", "inp[pnh_date_end]", "Anda Harus Mengisi Tanggal Selesai");
    echo getValidation();
    ?>
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>" onsubmit="return validation(document.form);" enctype="multipart/form-data">
            ...
            <fieldset>
                ...
                <p><?= $ui->createField("Nomor Peringatan", "inp[pnh_no]", $r[pnh_no], "") ?></p>
                <p><?= $ui->createField("Perihal", "inp[pnh_subject]", $r[pnh_subject], "") ?></p>
                <p><?= $ui->createField("Penerbit", "inp[pnh_agency]", $r[pnh_agency], "") ?></p>
                <p><?= $ui->createField("Tahun", "inp[pnh_year]", $r[pnh_year], "", "", "style=\"width:50px\"", "onkeyup=\"cekPhone(this);\"", "4") ?></p>
                <p><?= $ui->createComboData("Tipe", $queryType, "id", "description", "inp[pnh_type]", $r[pnh_type], "", "", "t", "") ?></p>
                <p><?= $ui->createField("Mulai", "inp[pnh_date_start]", getTanggal($r[pnh_date_start]), "", "", "", "", "", "", "t") ?></p>
                <p><?= $ui->createField("Selesai", "inp[pnh_date_end]", getTanggal($r[pnh_date_end]), "", "", "", "", "", "", "t") ?></p>
...
function lihat()
{
    ...
    $queryGroup = "SELECT kodeData id, namaData description, kodeInduk from mst_data where statusData = 't' and kodeCategory='" . $arrParameter[49] . "' order by kodeIndu,urutanData";
    $queryJabatan = "SELECT DISTINCT(pos_name) description, pos_name id FROM emp_phist ORDER BY pos_name ASC";
    $queryType = "SELECT kodeData id, namaData description from mst_data where kodeCategory = 'S20' and statusData = 't' order by `urutanData`";

    $tglMulai = implode("-", array(date('Y'), '01', '01'));
    $tglSelesai = implode("-", array(date('Y'), '12', '31'));

    $par[tanggalMulai] = empty($par[tanggalMulai]) ? getTanggal($tglMulai) : $par[tanggalMulai];
    $par[tanggalSelesai] = empty($par[tanggalSelesai]) ? getTanggal($tglSelesai) : $par[tanggalSelesai];

    $arrMaster = arrayQuery("select kodeData, namaData from mst_data where kodeCategory = 'S20' and statusData = 't'");
    ?>
    <div class="pageheader">
        <h1 class="pagetitle"><?= $arrTitle[$s] ?></h1>
        <?= getBread() ?>
        ...
        <form action="?_submit=1<?= getPar($par) ?>" method="post" id="form" class="stdform">
            ...
            <fieldset id="form_filter" style="display:none">
                <table width="100%">
                    <tr>
                        <td width="50%">
                            <p><?= $ui->createField("Mulai", "par[tanggalMulai]", $par[tanggalMulai], "", "t", "", "","","","t") ?></p>
                            ...
                            <p><?= $ui->createComboData("Tipe", $queryType, "id", "description", "par[idTipe]", $par[idTipe], "", "", "t", "", "t") ?></p>
                        </td>
                        <td width="50%">
                            <p><?= $ui->createField("Selesai", "par[tanggalSelesai]", $par[tanggalSelesai],"", "t","","","","","t") ?></p>
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            <tbody>
                <?php
                    ...
                    if (!empty($par[idTipe]))
                        $filter .= " and t1.pny_type = '$par[idTipe]'";

                    $sql = "SELECT t1.id, t1.pnh_no, t1.pnh_subject, t1.pnh_agency, t1.pnh_type, t1.pnh_year, t1.filename, t2.name, t2.reg_no, t2.kode FROM emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
                    $res = db($sql);
                    $no = 0;
                    ...
                    <tr>
                        ...
                        <td align="center"><a href="#" onclick="openBox('view.php?doc=empPunish&id=<?= $r[id].getPar($par, 'mode') ?>',800,600)"><img src=<?= getIcon($r[filename]) ?>></a></td>
                        <?= $control ?>
                    </tr>
                <?php
                    }
...
function xls()
{
    ...
    $field = array("no", "id", "pegawai", "npp", "nomor peringatan", "perihal", "penerbit", "tahun", "tipe", "mulai", "selesai");
    ...
    if (!empty($par[idJabatan]))
        $filter .= " and t3.pos_name = '$par[idJabatan]'";
    $sql = "SELECT t1.*, t2.name, t2.reg_no, t2.kode FROM emp_punish t1 join emp t2 on t1.parent_id = t2.id join emp_phist t3 on (t2.id = t3.parent_id AND t3.status = '1') $filter order by t2.name";
    $res = db($sql);
    $no = 0;
    while ($r = mysql_fetch_assoc($res)) {
        $no++;
        ...
        $data[] = array(
            ...
            getTanggal($r[pnh_date_start]) . "\t left",
            getTanggal($r[pnh_date_end]) . "\t left",
        );
    }
