...
function gLimit(){
	...
	$rmb_limit = 0;
		if($urutanData == 1){	# Rawat Jalan
			$sql__="select * from pay_pengobatan where idPegawai='".$id."'";
			$res__=db($sql__);
			$r__=mysql_fetch_array($res__);
		...
		// echo "select sum(rmb_val) from emp_rmb where parent_id='".$id."' and year(rmb_date)='".$rmb_year."' and rmb_date<='".$rmb_date."' and rmb_cat='".$rmb_cat."' and status='1' and rmb_jenis='k' and id!='".$par[id]."'";
	...
	function tambah(){
		global $db,$s,$inp,$par,$cUsername;				
		repField();				
		$cek = getField("select id from emp_rmb where rmb_date ='".setTanggal($inp[rmb_date])."' AND parent_id = '$inp[parent_id]' AND status !='1' and rmb_jenis = 'k'");
		if($cek){
			...
			<div id=\"contentwrapper\" class=\"contentwrapper\">			
				...
				<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" id=\"dyntable\">
					...
					<tbody>";
						...
						$filter = "where year(t1.rmb_date)='$par[tahun]' and rmb_jenis='k' ".($cGroup != "1" && $cGroup != "20" ? " and (leader_id='".$par[idPegawai]."' or administration_id='".$par[idPegawai]."')" : "");			
						if(!empty($par[filter]))		
							$filter.= " and (
