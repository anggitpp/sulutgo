<?php
	...
	function approve(){
		...
		$sql="update att_sakit set idPegawai='$inp[idPegawai]', idPerawatan='$inp[idPerawatan]', nomorSakit='$inp[nomorSakit]', tanggalSakit='".setTanggal($inp[tanggalSakit])."', mulaiSakit='".setTanggal($inp[mulaiSakit])."', selesaiSakit='".setTanggal($inp[selesaiSakit])."', keteranganSakit='$inp[keteranganSakit]', fileSakit='$fileSakit', catatanSakit='$inp[catatanSakit]', persetujuanSakit='$inp[persetujuanSakit]', approveBy='$cUsername', approveTime='".date('Y-m-d H:i:s')."' where idSakit='$par[idSakit]'";
		db($sql);
	...
	function form(){
		...
		$true = $r[persetujuanSakit] == "t" ? "checked=\"checked\"" : "";
		$false = $r[persetujuanSakit] == "f" ? "checked=\"checked\"" : "";
		$revisi = $r[persetujuanSakit] == "r" ? "checked=\"checked\"" : "";
		
		$sTrue = $r[sdmSakit] == "t" ? "checked=\"checked\"" : "";
		$sFalse = $r[sdmSakit] == "f" ? "checked=\"checked\"" : "";
		$sRevisi = $r[sdmSakit] == "r" ? "checked=\"checked\"" : "";
			...
			$text.="<div class=\"widgetbox\">
					...
					<td width=\"45%\">
						<p>
							...
							<div class=\"fradio\">
								<input type=\"radio\" id=\"true\" name=\"inp[persetujuanSakit]\" value=\"t\" $true /> <span class=\"sradio\">Disetujui</span>
								<input type=\"radio\" id=\"false\" name=\"inp[persetujuanSakit]\" value=\"f\" $false /> <span class=\"sradio\">Ditolak</span>
								<input type=\"radio\" id=\"revisi\" name=\"inp[persetujuanSakit]\" value=\"r\" $revisi /> <span class=\"sradio\">Diperbaiki</span>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Keterangan</label>
							<div class=\"field\">
								<textarea id=\"inp[catatanSakit]\" name=\"inp[catatanSakit]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:300px;\">$r[catatanSakit]</textarea>
							</div>
						</p>
					</td>
					<td width=\"55%\">&nbsp;</td>
			...
			$text.="<div class=\"widgetbox\">
					...
					<td width=\"45%\">
						<p>
							<label class=\"l-input-small\">Status</label>
							<div class=\"fradio\">
								<input type=\"radio\" id=\"true\" name=\"inp[sdmSakit]\" value=\"t\" $sTrue /> <span class=\"sradio\">Disetujui</span>
								<input type=\"radio\" id=\"false\" name=\"inp[sdmSakit]\" value=\"f\" $sFalse /> <span class=\"sradio\">Ditolak</span>
								<input type=\"radio\" id=\"revisi\" name=\"inp[sdmSakit]\" value=\"r\" $sRevisi /> <span class=\"sradio\">Diperbaiki</span>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Keterangan</label>
							<div class=\"field\">
								<textarea id=\"inp[noteSakit]\" name=\"inp[noteSakit]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:300px;\">$r[noteSakit]</textarea>
							</div>
						</p>
					</td>
					<td width=\"55%\">&nbsp;</td>
	...
	function lihat(){
		...
		$filter = "where year(t1.tanggalSakit)='$par[tahunSakit]' and (leader_id='".$par[idPegawai]."' or administration_id='".$par[idPegawai]."')";
		if(!empty($par[filter]))		
		$filter.= " and (
		...
		$sql="select * from att_sakit t1 left join dta_pegawai t2 on (t1.idPegawai=t2.id) $filter order by t1.nomorSakit";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
			...
			$persetujuanLink = isset($menuAccess[$s]["edit"]) ? "?par[mode]=app&par[idSakit]=$r[idSakit]".getPar($par,"mode,idSakit") : "#";			
			#$sdmLink = isset($menuAccess[$s]["edit"]) ? "?par[mode]=sdm&par[idSakit]=$r[idSakit]".getPar($par,"mode,idSakit") : "#";
			$sdmLink = "#";
			...
			$text.="<tr>
					...
					<td align=\"center\"><a href=\"".$persetujuanLink."\" title=\"Detail Data\">$persetujuanSakit</a></td>
					<td align=\"center\"><a href=\"".$sdmLink."\" title=\"Detail Data\">$sdmSakit</a></td>
					<td align=\"center\">
						<a href=\"?par[mode]=det&par[idSakit]=$r[idSakit]".getPar($par,"mode,idSakit")."\" title=\"Detail Data\" class=\"detail\"><span>Detail</span></a>
					</td>";
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			$persetujuanSakit = $r[persetujuanSakit] == "r" ? "Diperbaiki" : $persetujuanSakit;		
			...
			$sdmSakit = $r[sdmSakit] == "r" ? "Diperbaiki" : $sdmSakit;
