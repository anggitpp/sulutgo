function save(getPar){
	...
	rmb_limit=document.getElementById("inp[rmb_limit]");
	
	if(convert(rmb_val.value) * 1 > convert(rmb_balance.value) * 1 && convert(rmb_limit.value) > 0){
...
function getLimit(getPar){
	parent_id=document.getElementById("inp[parent_id]");
	rmb_cat=document.getElementById("inp[rmb_cat]");
	rmb_date=document.getElementById("rmb_date");
	rmb_limit=document.getElementById("inp[rmb_limit]");
	rmb_balance=document.getElementById("inp[rmb_balance]");
	// alert(rmb_date.value);
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			...
			if(response){
				...
				rmb_limit.value = dta[0];
				rmb_balance.value = dta[1];
				// alert(dta[0]);
				// alert(dta[1]);
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=lmt&parent_id=" + parent_id.value + "&rmb_cat=" + rmb_cat.value + "&rmb_date=" + rmb_date.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
