<?php
	...
	$text.="<div class=\"pageheader\">
		...
		$filter = "where year(t1.rmb_date)='$par[tahun]' and rmb_jenis='p' and (leader_id='".$par[idPegawai]."' or administration_id='".$par[idPegawai]."')";			
		if(!empty($par[filter]))		
		$filter.= " and (
	...
	function pegawai(){
		...
		while($r=mysql_fetch_array($res)){
			...
			$text.="<tr>
					...
					<td align=\"center\">
						<a href=\"#\" title=\"Pilih Data\" class=\"check\" onclick=\"setPegawai('".$r[reg_no]."', '".getPar($par, "mode, nikPegawai, parent_id,rmb_date,rmb_cat")."')\"><span>Detail</span></a>
					</td>
				</tr>";
		}	
