<?php
	...
	function sdm(){
		...
		$sql="update ess_cuti set idPegawai='$inp[idPegawai]', nomorCuti='$inp[nomorCuti]', tanggalCuti='".setTanggal($inp[tanggalCuti])."', namaCuti='$inp[namaCuti]', uangCuti='$inp[uangCuti]', ambilCuti='$inp[ambilCuti]', keteranganCuti='$inp[keteranganCuti]', noteCuti='$inp[noteCuti]', sdmCuti='$inp[sdmCuti]', sdmBy='$cUsername', sdmTime='".date('Y-m-d H:i:s')."' where idCuti='$par[idCuti]'";
		db($sql);
	...
	function lihat(){
		...
		$filter = "where year(t1.tanggalCuti)='$par[tahunCuti]' and (leader_id='".$par[idPegawai]."' or administration_id='".$par[idPegawai]."')";		
		if(!empty($par[filter]))		
		$filter.= " and (
		...
		$sql="select * from ess_cuti t1 left join dta_pegawai t2 on (t1.idPegawai=t2.id) $filter order by t1.nomorCuti";
		$res=db($sql);
