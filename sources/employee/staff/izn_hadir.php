...
function hapus()
{
    ...
    echo "<script>alert('DATA BERHASIL DIHAPUS');window.location='?" . getPar($par, "mode,idHadir") . "';</script>";
}
...
function ubah()
{
    ...
    echo "<script>alert('DATA BERHASIL DISIMPAN');window.location='?" . getPar($par, "mode,idHadir") . "';</script>";
}
...
function detail()
{
    ...
    <div class="contentwrapper">
        <form id="form" name="form" class="stdform">
            <p class="btnSave">
                <input type=\"button\" class=\"cancel radius2\" value=\"Kembali\" onclick=\"window.location='?" .
        ...
        <form id="form" name="form" method="post" class="stdform" action="?_submit=1<?= getPar($par) ?>"
            ...
            <p class="btnSave">
                ...
                <input type="button" class="cancel radius2" value="Batal"
                       onclick="window.location='?<?= getPar($par, 'mode,idPegawai') ?>';"/>
            </p>
            <br clear="all"/>
            <div id="general">
                <table width="100%">
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form method="post" class="stdform">
            <div id="pos_l" style="float:left;">
                <input placeholder="Search.." type="text" id="par[filter]" name="par[filter]" style="width:250px;"
                       value="<?= $par[filter] ?>" class="mediuminput"/>
                <?= comboYear("par[tahunHadir]", $par[tahunHadir]) ?>
                <input type="submit" value="GO" class="btn btn_search btn-small"/>
            </div>
            <div id="pos_r">
                <?php if (isset($menuAccess[$s]["add"])) ?> <a href="?par[mode]=add<?= getPar($par, 'mode,idHadir') ?> "
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            while ($r = mysql_fetch_array($res)) {
                ...
                if (isset($menuAccess[$s]["edit"]) || isset($menuAccess[$s]["delete"])) {
                    if (isset($menuAccess[$s]["edit"]) && $r[persetujuanHadir] != 't')
                        $control .= "<a href=\"?par[mode]=edit&par[idHadir]=$r[idHadir]" . getPar($par, "mode,idHadir") . "\" title=\"Edit Data\" class=\"edit\"><span>Edit</span></a>";
                    if (isset($menuAccess[$s]["delete"]) && $r[persetujuanHadir] != 't')
                        $control .= "<a href=\"?par[mode]=del&par[idHadir]=$r[idHadir]" . getPar($par, "mode,idHadir") . "\" onclick=\"return confirm('are you sure to delete data ?');\" title=\"Delete Data\" class=\"delete\"><span>Delete</span></a>";
                }
                $control .= "</td>";
                ...
                <tr>
                    ...
                    <td align="center"><a
                                href="?par[mode]=det&par[idHadir]=<?= $r[idHadir] . getPar($par, "mode,idHadir") ?>"
