...
$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai t1 join mst_data t2 on t1.dept_id = t2.kodeData WHERE location IN ($areaCheck) AND cat = '532' AND dept_id IN($departemen) order by id";
		$res=db($sql);
				...
				<tbody>
					...
					if(is_array($arrDepartemen)){
						...
						while(list($id, $namaDepartemen) = each($arrDepartemen)){
							$r[persen] = ($jmlDept[$id] / $jumlahPegawai) * 100;
							// echo $jmlDept[$id]. " a ".$total."<br>";
							$no++;
							$showValue = setAngka($jmlDept[$id]) > 0 ? 1 : 0;
							// $text.="<set value=\"".setAngka($jmlDept[$id])."\" label=\"".$namaDepartemen."\" showValue=\"".$showValue."\" />";
							$text.="<tr>
							<td>$no.</td>
							<td>".$namaDepartemen."</td>
							<td>".setAngka($jmlDept[$id])."</td>
							<td>".round($r[persen],2)."</td>
							</tr>";
							// $total += $jmlDept[$idPendidikan];
	...
	function xls(){		
		...
		$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai WHERE location IN ($areaCheck) AND cat = '532' AND dept_id IN($departemen) order by id";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
				if($r[status] == $status){
				$jmlDept["$r[dept_id]"]++;	
		...
		$arrMaster = arrayQuery("select kodeData, namaData from mst_data");
					if(is_array($arrDepartemen)){
						reset($arrDepartemen);
						while(list($id, $namaPangkat) = each($arrDepartemen)){
							$r[persen] = ($jmlDept[$id] / $jumlahPegawai) * 100;
							// echo $jmlDept[$id]. " a ".$total."<br>";
							$no++;
							...
							$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, $jmlDept[$id]);
