...
$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai WHERE location IN ($areaCheck) AND cat ='3197' AND location IN($location) order by id";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
				if($r[status] == $status){
				$jmlLocation["$r[location]"]++;	
				$jumlahPegawai++;	
			}	
		}
				...
				<tbody>
					...
					if(is_array($arrLocation)){
						...
						while(list($id, $namaLocation) = each($arrLocation)){
							if($jmlLocation[$id]!=0){
							$r[persen] = ($jmlLocation[$id] / $jumlahPegawai) * 100;
							// echo $jmlLocation[$id]. " a ".$total."<br>";
							$no++;
							$showValue = setAngka($jmlLocation[$id]) > 0 ? 1 : 0;
							// $text.="<set value=\"".setAngka($jmlLocation[$id])."\" label=\"".$namaLocation."\" showValue=\"".$showValue."\" />";
							$text.="<tr>
							<td>$no.</td>
							<td>".$namaLocation."</td>
							<td>".setAngka($jmlLocation[$id])."</td>
		...
		if($par[mode] == "xls"){			
			xls();			
			$text.="<iframe src=\"download.php?d=exp&f=PROSENTASE JUMLAH PENGEMUDI.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
		}	
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PROSENTASE JUMLAH PENGEMUDI');
		...
		$arrMaster = arrayQuery("select kodeData, namaData from mst_data");
					if(is_array($arrLocation)){
						...
						while(list($id, $namaLocation) = each($arrLocation)){
							...
							$objPHPExcel->getActiveSheet()->setCellValue('B'.$rows, $arrMaster[$id]);
							$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, $jmlLocation[$id]);
	...
	$objPHPExcel->getActiveSheet()->getStyle('A6:D'.$rows)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		...
		$objPHPExcel->getActiveSheet()->setTitle("PROSENTASE JUMLAH PENGEMUDI");
		$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$objWriter->save($fFile."PROSENTASE JUMLAH PENGEMUDI.xls");
	}
