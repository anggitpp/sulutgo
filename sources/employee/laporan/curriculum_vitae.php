...
function lihat(){
    ...
    <div id="contentwrapper" class="contentwrapper">
		...
		<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            if(!empty($par[filter]))
                $filter.= " and (
                lower(t1.reg_no) like '%".strtolower($par[filter])."%'	
                or lower(t1.name) like '%".strtolower($par[filter])."%'	
                )";
            $sql="select t1.id, t1.name, t1.reg_no, t1.birth_date, t2.pos_name, 
            replace( case when coalesce(t1.leave_date,NULL) IS NULL THEN
            ...
            END,' 0 bln','') masaKerja from emp t1 join emp_phist t2 on (t1.id = t2.parent_id AND t2.status = '1') $filter order by name";
            $res=db($sql);
            while($r=mysql_fetch_array($res)){
                ...
                <tr>
                    ...
                    <td align="center">
                        <a href="#" class="print" title="Cetak Form" onclick="openBox('ajax.php?par[mode]=print&par[id]=<?= $r[id].getPar($par,"mode,id") ?>',1200,600);" ><span>Cetak</span></a>
                    </td>
                </tr>
            <?php
            }
...
function pdf(){
    ...
    $sql="select *,
    (CASE WHEN gender = 'M' THEN 'Laki-Laki' ELSE (CASE WHEN gender = 'F' THEN 'Perempuan' ELSE '' END) END) as gender 
    from emp where id = '$par[id]'";
    $res=db($sql);
