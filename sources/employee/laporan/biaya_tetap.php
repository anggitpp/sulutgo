	...
	function lihat(){
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">			
			...
			$sql="SELECT * FROM pay_proses_".$par[tahunProses].$par[bulanProses]." t1 JOIN dta_komponen t2 ON (t1.idKomponen=t2.idKomponen) JOIN dta_pegawai t3 ON t1.`idPegawai` = t3.id WHERE idProses='$idProses' AND cat = '531' AND dept_id IN($departemen)";
			// echo $sql;
			$res=db($sql);
	...
	if($par[mode] == "xls"){			
		xls();			
		$text.="<iframe src=\"download.php?d=exp&f=PROSENTASE BIAYA TETAP.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
	}	
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PROSENTASE BIAYA TETAP');
	...
	$objPHPExcel->getActiveSheet()->setTitle("PROSENTASE BIAYA TETAP");
	$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	$objWriter->save($fFile."PROSENTASE BIAYA TETAP.xls");
}
