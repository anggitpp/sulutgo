...
function lihat(){
    ...
    $arrStatus = arrayQuery("SELECT kodeData, concat(namaData, '-', keteranganData) FROM mst_data WHERE kodeCategory ='S04' AND statusData = 't' order by urutanData");
	...
	<div id="contentwrapper" class="contentwrapper">
		...
		<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick" id="dyntable">
            ...
            $sql = "SELECT t1.id, gender, cat FROM emp t1 join emp_phist t2 on (t1.id = t2.parent_id AND t2.status = '1') WHERE t1.status = '535' $filter";
            $res = db($sql);
            while ($r = mysql_fetch_assoc($res)){
                $arrData[$r[gender]][$r[cat]]++;
            }
            $no=0;
            foreach ($arrStatus as $key => $value){
                $no++;
                list($r[namaData], $r[keteranganData]) = explode("-", $value);
                ?>
                <tr>
                    <td><?= $no ?>.</td>
                    <td><?= $r[keteranganData] ?></td>
                    <td><?= $r[namaData] ?></td>
...
function xls()
{
    ...
    $field = array("no", "kode", "status", "L", "P","JUMLAH");
