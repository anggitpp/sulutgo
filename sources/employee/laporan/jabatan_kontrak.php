	...
	function lihat(){
		...
		$arrJabatan = arrayQuery("select kodeData, namaData from mst_data where statusData='t' and kodeCategory='S09' order by urutanData");
		$text="
...
$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai WHERE location IN ($areaCheck) AND cat = '532' AND rank !='' order by id";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
			...
			<table cellpadding=\"0\" style=\"width:700px;float:left;\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" >
				...
				<tbody>
					";
					if(is_array($arrJabatan)){
						reset($arrJabatan);
						while(list($id, $namaPangkat) = each($arrJabatan)){
							$r[persen] = ($jmlPangkat[$id] / $jumlahPegawai) * 100;
							// echo $jmlPangkat[$id]. " a ".$total."<br>";
							$no++;
							$showValue = setAngka($jmlPangkat[$id]) > 0 ? 1 : 0;
							// $text.="<set value=\"".setAngka($jmlPangkat[$id])."\" label=\"".$namaPangkat."\" showValue=\"".$showValue."\" />";
							$text.="<tr>
							<td>$no.</td>
							<td>".$namaPangkat."</td>
							<td>".setAngka($jmlPangkat[$id])."</td>
							<td>".round($r[persen],2)."</td>
							</tr>";
							// $total += $jmlPangkat[$idPendidikan];
		...
		if($par[mode] == "xls"){			
			xls();			
			$text.="<iframe src=\"download.php?d=exp&f=PERSENTASE JABATAN KONTRAK.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
		}	
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PERSENTASE JABATAN KONTRAK');
		...
		$arrMaster = arrayQuery("select kodeData, namaData from mst_data");
					if(is_array($arrJabatan)){
						...
						while(list($id, $namaPangkat) = each($arrJabatan)){
							...
							$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, $jmlPangkat[$id]);
	...
	$objPHPExcel->getActiveSheet()->getStyle('A6:D'.$rows)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		...
		$objPHPExcel->getActiveSheet()->setTitle("PERSENTASE JABATAN KONTRAK");
		$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$objWriter->save($fFile."PERSENTASE JABATAN KONTRAK.xls");
	}
