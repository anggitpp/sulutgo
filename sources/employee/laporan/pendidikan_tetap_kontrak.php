	...
	function lihat(){
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">			
			...
			$sql="select t1.id, t1.id_pendidikan from emp t1 join emp_phist t2 on (t1.id = t2.parent_id AND t2.status = '1') where location IN ($areaCheck) and cat IN(531,532)";
			// echo $sql;
					$res=db($sql);
					while($r=mysql_fetch_array($res)){
						$arrEducation["$r[id]"]=$r[id_pendidikan];
					}
		...
		if($par[mode] == "xls"){			
			xls();			
			$text.="<iframe src=\"download.php?d=exp&f=PENDIDIKAN TETAP KONTRAK.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
		}	
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PENDIDIKAN TETAP KONTRAK');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A6:D'.$rows)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		...
		$objPHPExcel->getActiveSheet()->setTitle("PENDIDIKAN TETAP KONTRAK");
		$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$objWriter->save($fFile."PENDIDIKAN TETAP KONTRAK.xls");
	}
