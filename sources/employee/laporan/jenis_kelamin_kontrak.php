...
function lihat(){
	...
	<div id=\"contentwrapper\" class=\"contentwrapper\">			
		...
		$pria = getField("select count(t1.id) from emp t1 join emp_phist t2 on t1.id = t2.parent_id where upper(t1.gender) = 'M' AND cat = '532' $filter");
		$wanita = getField("select count(t1.id) from emp t1 join emp_phist t2 on t1.id = t2.parent_id where upper(t1.gender) = 'F' AND cat = '532' $filter");
	...
	if($par[mode] == "xls"){			
		xls();			
		$text.="<iframe src=\"download.php?d=exp&f=PERSENTASE GENDER KONTRAK.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
	}	
...
function xls(){		
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PERSENTASE GENDER KONTRAK');
	...
	$rows = 5;
		...
		$pria = getField("select count(t1.id) from emp t1 join emp_phist t2 on t1.id = t2.parent_id where upper(t1.gender) = 'M' t1.cat ='532' $filter");
		$wanita = getField("select count(t1.id) from emp t1 join emp_phist t2 on t1.id = t2.parent_id where upper(t1.gender) = 'F' t1.cat ='532' $filter");
...
$objPHPExcel->getActiveSheet()->setTitle("PERSENTASE GENDER KONTRAK");
$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
$objWriter->save($fFile."PERSENTASE GENDER KONTRAK.xls");
}
