	...
	function lihat(){
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">			
			...
			<table cellpadding=\"0\" style=\"width:700px;float:left;\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" >
				...
				<tbody>
					...
					WHERE location IN ($areaCheck) AND cat = '532' order by id";
					...
					while($r=mysql_fetch_array($res)){
						if($r[status] == $status){
							...
							if($r[empWorkDuration] <= 2)
								$masaKerja["&lt;2"]++;
							else if($r[empWorkDuration] <= 4)
								$masaKerja["2-4"]++;
							else
								$masaKerja["&gt;4"]++;
		...
		if($par[mode] == "xls"){			
			xls();			
			$text.="<iframe src=\"download.php?d=exp&f=PERSENTASE MASA KERJA KONTRAK.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
		}	
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PERSENTASE MASA KERJA KONTRAK');
	...
	$objPHPExcel->getActiveSheet()->getStyle('A6:D'.$rows)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		...
		$objPHPExcel->getActiveSheet()->setTitle("PERSENTASE MASA KERJA KONTRAK");
		$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		$objWriter->save($fFile."PERSENTASE MASA KERJA KONTRAK.xls");
	}
