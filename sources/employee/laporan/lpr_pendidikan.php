...
function lihat(){
    ...
    $arrPendidikan = arrayQuery("SELECT kodeData, namaData FROM mst_data WHERE kodeCategory ='R11' AND statusData = 't' order by urutanData");
    ...
    $sql = "SELECT t1.id, gender, id_pendidikan FROM emp t1 join emp_phist t2 on (t1.id = t2.parent_id AND t2.status = '1') WHERE t1.status = '535' $filter";
    $res = db($sql);
    while ($r = mysql_fetch_assoc($res)){
        $arrData[$r[gender]][$r[id_pendidikan]]++;
    }
    $no=0;
...
function xls()
{
    ...
    $field = array("no", "pendidikan", "L", "P","JUMLAH");
