...
$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai t1 join mst_data t2 on t1.dept_id = t2.kodeData WHERE location IN ($areaCheck) AND cat = '531' AND dept_id IN($departemen) order by id";
		$res=db($sql);
	...
	function xls(){		
		...
		$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			$sql="select * from dta_pegawai WHERE location IN ($areaCheck) AND cat = '531' AND dept_id IN($departemen) order by id";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){
				if($r[status] == $status){
				$jmlDept["$r[dept_id]"]++;	
