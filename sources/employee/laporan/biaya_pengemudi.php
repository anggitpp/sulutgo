	...
	function lihat(){
		...
		$arrLocation = arrayQuery("select kodeData, namaData from mst_data where statusData='t' and kodeCategory='S06' order by namaData");

		$location = getField("SELECT GROUP_CONCAT(DISTINCT(location)) FROM emp_phist");
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">			
			...
			$sql="SELECT * FROM pay_proses_".$par[tahunProses].$par[bulanProses]." t1 JOIN dta_komponen t2 ON (t1.idKomponen=t2.idKomponen) JOIN dta_pegawai t3 ON t1.`idPegawai` = t3.id WHERE idProses='$idProses' AND cat = '3197' ";
			// echo $sql;
			$res=db($sql);
			while($r=mysql_fetch_array($res)){
					$arrNilaiLocation[$r[location]]	+= $r[nilaiProses];
					// $totalNilai += $arrNilaiLocation[$r[location]];
			}
			...
			<table cellpadding=\"0\" style=\"width:500px;float:left;\" cellspacing=\"0\" border=\"0\" class=\"stdtable stdtablequick\" >
				...
				<tbody>
					...
					if(is_array($arrLocation)){
						reset($arrLocation);
						while(list($id, $namaLocation) = each($arrLocation)){
							if($arrNilaiLocation[$id]!=0){
								$r[persen] = ($arrNilaiLocation[$id] / array_sum($arrNilaiLocation)) * 100;
							// echo $arrNilaiLocation[$id]. " a ".$total."<br>";
								$no++;
								$showValue = setAngka($arrNilaiLocation[$id]) > 0 ? 1 : 0;
							// $text.="<set value=\"".setAngka($arrNilaiLocation[$id])."\" label=\"".$namaLocation."\" showValue=\"".$showValue."\" />";
								$text.="<tr>
								<td>$no.</td>
								<td>".$namaLocation."</td>
								<td align=\"right\">".getAngka($arrNilaiLocation[$id])."</td>
								<td align=\"center\">".round($r[persen],2)."</td>
							</tr>";
							$totalPersen += $r[persen];
							// $totalNilai += $arrNilaiLocation[$id];
				...
				$text.="
					...
					<tr>
						...
						<td align=\"right\">".getAngka(array_sum($arrNilaiLocation))."</td>
	...
	if($par[mode] == "xls"){			
		xls();			
		$text.="<iframe src=\"download.php?d=exp&f=PROSENTASE BIAYA PENGEMUDI.xls\" frameborder=\"0\" width=\"0\" height=\"0\"></iframe>";
	}	
...
function xls(){		
	...
	$idProses = getField("select idProses from pay_proses where bulanProses = '$par[bulanProses]' AND tahunProses = '$par[tahunProses]'");
	$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
	$sql="SELECT * FROM pay_proses_".$par[tahunProses].$par[bulanProses]." t1 JOIN dta_komponen t2 ON (t1.idKomponen=t2.idKomponen) JOIN dta_pegawai t3 ON t1.`idPegawai` = t3.id WHERE idProses='$idProses' AND cat = '531' ";
			// echo $sql;
	$res=db($sql);
	while($r=mysql_fetch_array($res)){
		$arrNilaiLocation[$r[location]]	+= $r[nilaiProses];
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PROSENTASE BIAYA PENGEMUDI');
	...
	if(is_array($arrLocation)){
		...
		while(list($id, $namaLocation) = each($arrLocation)){
			if($arrNilaiLocation[$id]!=0){
				$r[persen] = ($arrNilaiLocation[$id] / $totalNilai) * 100;
							// echo $arrNilaiLocation[$id]. " a ".$total."<br>";
				$no++;
				...
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$rows, $namaLocation);
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$rows, getAngka($arrNilaiLocation[$id]));
	...
	$objPHPExcel->getActiveSheet()->setTitle("PROSENTASE BIAYA PENGEMUDI");
	$objPHPExcel->setActiveSheetIndex(0);

		// Save Excel file
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	$objWriter->save($fFile."PROSENTASE BIAYA PENGEMUDI.xls");
}
