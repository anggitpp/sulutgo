<?php
	...
	function setData(){
		...
		if(!empty($idPegawai)){								
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='f' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) and t1.idPegawai='".$idPegawai."'";
			$res=db($sql);			
	...
	function form(){
		...
		$nilaiInsentif = array();		
		$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='f' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null)";
		$res=db($sql);			
		while($r=mysql_fetch_array($res)){	
	...
	function lihat(){
		...
		for($i=1; $i<=12; $i++){
			...
			$pegawaiInsentif = array();
			$nilaiInsentif = array();
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$mulaiPeriode."' and '".$selesaiPeriode."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='f' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) and t2.group_id in ( $areaCheck ) $filter";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
				$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
				$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiAngsuran] > $r[totalAngsuran] ? $r[nilaiAngsuran] : $r[totalAngsuran];							
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='f' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) ".$filter;
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
