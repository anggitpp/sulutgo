<?php
	...
	function lihat(){
		...
		if(empty($par[bulanPerubahan])) $par[bulanPerubahan] = date('m');
		if(empty($par[tahunPerubahan])) $par[tahunPerubahan] = date('Y');		
		...
		$text.="<div class=\"pageheader\">
							...
							<table>
								<tr>
									...
									<td>
										...
										".comboMonth("par[bulanPerubahan]", $par[bulanPerubahan], "onchange=\"document.getElementById('form').submit();\"")." ".comboYear("par[tahunPerubahan]", $par[tahunPerubahan], "", "onchange=\"document.getElementById('form').submit();\"")."
			...
			$filter = "where month(t1.tanggalPokok)='$par[bulanPerubahan]' and year(t1.tanggalPokok)='$par[tahunPerubahan]' AND t2.group_id IN ( $areaCheck ) and t2.status = '535'";
			...
			$sql="select t1.*, t2.name, t2.reg_no from pay_pokok t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) $filter";
			$res=db($sql);
			while($r=mysql_fetch_array($res)){		
				...
				$text.="<tr>
						...
						<td>".nl2br($r[keteranganPokok])."</td>
						</tr>";	
				$totalNilai+=$r[nilaiPokok];
			}			
			$text.="</tbody>
			<tfoot>
			<tr>
	...
	function xls(){		
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LAPORAN PERUBAHAN GAJI');
		$objPHPExcel->getActiveSheet()->setCellValue('A2', getBulan($par[bulanPerubahan])." ".$par[tahunPerubahan]);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('E4', 'NO. SK');		
		$objPHPExcel->getActiveSheet()->setCellValue('F4', 'GAJI POKOK');
		...
		while($r=mysql_fetch_array($res)){		
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$rows, getTanggal($r[tanggalPokok]));
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$rows, $r[nomorPokok]);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $r[nilaiPokok]);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$rows, $r[keteranganPokok]);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$rows, $totalNilai);
