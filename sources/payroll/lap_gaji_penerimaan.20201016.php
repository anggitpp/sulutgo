	...
	if(empty($par[tahunProses])) $par[tahunProses] = date('Y');		
		...
		<div class=\"pageheader\">
				...
				<div style=\"position: absolute; right: " . ($isUsePeriod ? "-3px" : "20px" ) . "; top: " . ($isUsePeriod ? "0px" : "10px" ) . "; vertical-align:top; padding-top:2px; width: 800px;\">
						<div style=\"position:absolute; right: 0px;\">
							<table>
								<tr>
									<td>
										Lokasi Proses : ".comboData("select * from mst_data where statusData='t' and kodeCategory='".$arrParameter[7]."' and kodeData in ($areaCheck) order by urutanData","kodeData","namaData","par[idLokasi]","All",$par[idLokasi],"onchange=\"document.getElementById('form').submit();\"", "310px")."
									</td>
			...
			<tbody>";
				...
				$filter = "where id is not null and (leave_date >= '".$par[tahunProses]."-".$par[bulanProses]."-01' or leave_date is null or leave_date='0000-00-00' or status='".$status."') and group_id in ($areaCheck)";		
				...
				if(getField("select idProses from pay_proses where detailProses='pay_proses_".$par[tahunProses].str_pad($par[bulanProses], 2, "0", STR_PAD_LEFT)."'")){
					...
					while($r=mysql_fetch_array($res)){
						...
						if(in_array($r[idKomponen], array(39,43,45,28,7,38,40,41,46,47,44,49,50,48))){
