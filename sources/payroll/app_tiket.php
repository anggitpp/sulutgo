<?php
	...
	function all(){
		...
		$filter = "where nomorTiket is not null AND t2.group_id IN ( $areaCheck )";		
		if(!empty($par[bulanTiket]))
			$filter.= " and month(t1.tanggalTiket)='$par[bulanTiket]'";
		if(!empty($par[tahunTiket]))
			$filter.= " and year(t1.tanggalTiket)='$par[tahunTiket]'";				
		...
		$persetujuanTiket = $par[mode] == "allSdm" ? "sdmTiket" : "persetujuanTiket";
		$keteranganTiket = $par[mode] == "allSdm" ? "noteTiket" : "catatanTiket";
		$approveBy = $par[mode] == "allSdm" ? "sdmBy" : "approveBy";
		$approveTime = $par[mode] == "allSdm" ? "sdmTime" : "approveTime";				
		
		
		$sql="update ess_tiket t1 left join dta_pegawai t2 on (t1.idPegawai=t2.id) set $keteranganTiket='".$inp[$keteranganTiket]."', $persetujuanTiket='".$inp[$persetujuanTiket]."', $approveBy='$cUsername', $approveTime='".date('Y-m-d H:i:s')."' $filter";
		db($sql);
	...
	function tambah(){
		...
		$sql="insert into ess_tiket (idTiket, idPegawai, idKategori, idTipe, nomorTiket, tanggalTiket, berangkatTiket, berangkatTiket_asal, berangkatTiket_tujuan, pulangTiket, pulangTiket_asal, pulangTiket_tujuan, keteranganTiket, fileTiket, persetujuanTiket, sdmTiket, createBy, createTime) values ('$idTiket', '$inp[idPegawai]', '$inp[idKategori]', '$inp[idTipe]', '$inp[nomorTiket]', '".setTanggal($inp[tanggalTiket])."', '$berangkatTiket', '$inp[berangkatTiket_asal]', '$inp[berangkatTiket_tujuan]', '$pulangTiket', '$inp[pulangTiket_asal]', '$inp[pulangTiket_tujuan]', '$inp[keteranganTiket]', '$fileTiket', 'p', 'p', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
		...
		$text.="</h1>
				...
				<div id=\"general\" class=\"subcontent\">										
					...
					<p>
						...
						<div class=\"fradio\">
							<input type=\"radio\" id=\"true\" name=\"inp[".$persetujuanTiket."]\" value=\"t\" checked=\"checked\" /> <span class=\"sradio\">Disetujui</span>
							<input type=\"radio\" id=\"false\" name=\"inp[".$persetujuanTiket."]\" value=\"f\" > <span class=\"sradio\">Ditolak</span>
							<input type=\"radio\" id=\"revisi\" name=\"inp[".$persetujuanTiket."]\" value=\"r\" /> <span class=\"sradio\">Diperbaiki</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
						<div class=\"field\">
							<textarea id=\"inp[".$keteranganTiket."]\" name=\"inp[".$keteranganTiket."]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:300px;\"></textarea>
						</div>
					</p>					
					<p>
	...
	function form(){
		...
		$sFalse = $r[sdmTiket] == "f" ? "checked=\"checked\"" : "";
		$sRevisi = $r[sdmTiket] == "r" ? "checked=\"checked\"" : "";
	...
	function lihat(){
						...
						<div style=\"position:absolute; right: 0px;\">
							<table>
								<tr>
									...
									<td>
										<span style=\"margin-left:30px;\">Periode :</span> ".comboMonth("par[bulanTiket]", $par[bulanTiket], "onchange=\"document.getElementById('form').submit();\"", "", "t")." ".comboYear("par[tahunTiket]", $par[tahunTiket], "", "onchange=\"document.getElementById('form').submit();\"", "", "t")."
									</td>
								</tr>
							</table>
						</div>
		...
		if(isset($menuAccess[$s]["apprlv1"])) $text.="<a href=\"#\" class=\"btn btn1 btn_edit\" onclick=\"openBox('popup.php?par[mode]=allAts".getPar($par,"mode,idTiket")."',725,400);\"><span>All Atasan</span></a> ";
		if(isset($menuAccess[$s]["apprlv2"])) $text.="<a href=\"#\" class=\"btn btn1 btn_edit\" onclick=\"openBox('popup.php?par[mode]=allSdm".getPar($par,"mode,idTiket")."',725,400);\"><span>All SDM</span></a> ";		
		$text.="</div>
			</form>
			<br clear=\"all\" />
		...
		$filter = "where nomorTiket is not null and t1.persetujuanTiket='t' AND t2.group_id IN ( $areaCheck )";
		if(!empty($par[bulanTiket]))
			$filter.= " and month(t1.tanggalTiket)='$par[bulanTiket]'";
		if(!empty($par[tahunTiket]))
			$filter.= " and year(t1.tanggalTiket)='$par[tahunTiket]'";				
		...
		while($r=mysql_fetch_array($res)){			
			...
			$sdmLink = (isset($menuAccess[$s]["apprlv2"]) && $r[persetujuanTiket] == "t") ? "?par[mode]=sdm&par[idTiket]=$r[idTiket]".getPar($par,"mode,idTiket") : "#";
			...
			$text.="<tr>
					...
					<td align=\"center\"><a href=\"".$sdmLink."\" title=\"Detail Data\">$sdmTiket</a></td>
					<td align=\"center\">
