<?php
	...
	function setTable(){
		...
		$sql="delete from ".$detailInsentif." where flagProses='8'";
		db($sql);
		...
		$sql="insert into ".$detailTemp." (idInsentif, idPegawai, createBy, createTime) select '".$idInsentif."' as idInsentif, idPegawai, '$cUsername' as createBy, '".date('Y-m-d H:i:s')."' as createTime from ess_pinjaman t1 join ess_angsuran t2 on (t1.idPinjaman=t2.idPinjaman) where date(t2.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='t' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) group by t1.idPegawai order by t1.idPegawai";		
		db($sql);
	}
	...
	function setData(){
		...
		if(!empty($idPegawai)){								
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='t' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) and t1.idPegawai='".$idPegawai."'";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){									
				$nilaiInsentif+=$r[nilaiAngsuran] > $r[totalAngsuran] ? $r[nilaiAngsuran] : $r[totalAngsuran];
			}
			
			if($nilaiInsentif > 0){
				$sql="insert into ".$detailInsentif." (idProses, idPegawai, idKomponen, nilaiProses, flagProses, createBy, createTime) values ('$idProses', '$idPegawai', '$idKomponen', '".setAngka($nilaiInsentif)."', '8', '$cUsername', '".date('Y-m-d H:i:s')."')";
				db($sql);
	...
	function form(){
		...
		$nilaiInsentif = array();		
		$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='t' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null)";
		$res=db($sql);			
		while($r=mysql_fetch_array($res)){	
			$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
			$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiAngsuran] > $r[totalAngsuran] ? $r[nilaiAngsuran] : $r[totalAngsuran];
		}		
		
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">Insentif Pinjaman</h1>
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<div id=\"prosesBtn\" align=\"center\">						
						<input type=\"button\" class=\"cancel radius2\" value=\"Proses Pinjaman\" onclick=\"setInsentif('".getPar($par,"mode")."');\"/>
					</div>
	...
	function lihat(){
		...
		for($i=1; $i<=12; $i++){
			...
			$pegawaiInsentif = array();
			$nilaiInsentif = array();
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$mulaiPeriode."' and '".$selesaiPeriode."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='t' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) and t2.group_id in ( $areaCheck ) $filter";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
				$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
				$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiAngsuran] > $r[totalAngsuran] ? $r[nilaiAngsuran] : $r[totalAngsuran];
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.tipePinjaman='t' and (t1.lunasPinjaman!='t' or t1.lunasPinjaman is null) ".$filter;
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
		...
		$text.="<div class=\"pageheader\">
			...
			<div id=\"contentwrapper\" class=\"contentwrapper\">
				...
				<div id=\"general\" class=\"subcontent\">				
					...
					<thead>
						<tr>							
							<th style=\"text-align:left;\">INSENTIF</th>							
						</tr>
					</thead>
					<tbody>";
			...
			$no=1;
			while($r=mysql_fetch_array($res)){
				$nilaiAngsuran = $r[nilaiAngsuran] > $r[totalAngsuran] ? $r[nilaiAngsuran] : $r[totalAngsuran];
				$text.="<tr>
							<td style=\"padding:3px 20px; border-top:1px solid #ddd; border-right:0px;\">
							<span style=\"float:left; width:30px;\">$no .</span>
							<span style=\"float:left; padding-right:30px;\">".$r[nomorPinjaman]."</span>
							<span style=\"float:left;\">".$r[keteranganPinjaman]."</span>
							<span style=\"float:right; text-align:right; width:125px;\">".getAngka($nilaiAngsuran)."</span>
							<span style=\"float:right;\">Rp.</span>
							</td>
					</tr>";
				$nilaiInsentif+=$nilaiAngsuran;
				$no++;
			}
