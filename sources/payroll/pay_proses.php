<?php
	...
	function setTable(){
		...
		//$filter.=" and t1.id='1224'";
		
		$arrId= arrayQuery("select t1.id from emp t1 left join emp_phist t2 on (t1.id=t2.parent_id) where t1.status='".$status."' and t2.status='1' ".$filter." and t2.group_id is not null order by t1.id");
		...
		$sql="insert into ".$detailTemp." (idProses, idPegawai, createBy, createTime) select '".$idProses."' as idProses, t1.id, '$cUsername' as createBy, '".date('Y-m-d H:i:s')."' as createTime from emp t1 left join emp_phist t2 on (t1.id=t2.parent_id) where t1.status='".$status."' and t2.status='1' ".$filter." and t2.group_id is not null order by t1.id";
		db($sql);
	}
	...
	function setData(){
		...
		if(!empty($idPegawai)){	
			...
			while($r__ = mysql_fetch_array($res__)){
				$kodeKomponen___ = getField("select kodeKomponen from dta_komponen where idKomponen='".$r__[idKomponen]."'");
				$sql___ = "select * from dta_komponen where kodeKomponen='".$kodeKomponen___."'";
				$res___ = db($sql___);
				while($r___ = mysql_fetch_array($res___)){
					$arrManual["".$r___[idKomponen].""]+=$r__[nilaiUpload];
				}
			}
			...
			$fieldFungsional = $r[location] == getField("select kodeData from mst_data where kodeCategory='".$arrParameter[7]."' order by urutanData limit 1") ? "pusatFungsional" : "cabangFungsional";			
			$nilaiFungsional = getField("select sum(t1.".$fieldFungsional.") from pay_fungsional t1 join mst_data t2 join mst_data t3 on (t1.idPangkat=t2.kodeData and t1.idGrade=t3.kodeData) where idPangkat='".$r[rank]."' and idGrade='".$r[grade]."'");
			...
			$sql="select t1.*, t2.kodeKomponen from pay_komponen t1 join dta_komponen t2 on (t1.idKomponen=t2.idKomponen) where t1.idPegawai='".$idPegawai."' and t2.idJenis='".$par[idJenis]."' order by t2.tipeKomponen desc, t2.kodeKomponen, t2.urutanKomponen,t2.createTime desc";
			$res=db($sql);
			while($r=mysql_fetch_array($res)){
				if($r[kodeKomponen] != $kodeKomponen){
			...
			$sql="select * from dta_komponen where statusKomponen='t' order by tipeKomponen desc,urutanKomponen";
			$res=db($sql);
			while($r=mysql_fetch_array($res)){				
			...
			if(is_array($arrData)){
				...
				while (list($idKomponen, $tipeKomponen) = each($arrData)){					
					...
					//$result.=$idKomponen." ".$r[namaKomponen]."<br>";
					$nilaiKomponen = 0;
					...
					*/
					...
					if($r[flagKomponen] == 0){
						...
						if($r[dasarKomponen] < 2 ){	
							...
							if(count($arrFormula) == 1 && !empty($___idKomponen)){
								...
								if(is_array($arrFormula)){
									...
									while (list($detFormula) = each($arrFormula)){	
										...
										$kodeKomponen__ = getField("select kodeKomponen from dta_komponen where idKomponen='".$___idKomponen."'");
										$idKomponen__ = arrayQuery("select idKomponen from dta_komponen where kodeKomponen='".$kodeKomponen__."'");
										
										$___nilaiFormula = getField("select nilaiProses from ".$detailProses." where idPegawai='".$idPegawai."' and idKomponen in ('".implode("', '", $idKomponen__)."')");
										
										if($operasiDet == "*")
											$nilaiFormula = $nilaiDet * $___nilaiFormula;
										if($operasiDet == "/")
											$nilaiFormula = $nilaiDet / $___nilaiFormula;
										if($operasiDet == "+")
											$nilaiFormula = $nilaiDet + $___nilaiFormula;
							...
							if($r[kodeKomponen] == "GPF")
								$nilaiKomponen = ceil( $nilaiKomponen / 100 ) * 100;
						}
					}
					...
					if(!in_array($r[dasarKomponen], array(3)) && $arrStatus["$r[idKomponen]"]["$r[tipeKomponen]"] != "f" && ($nilaiKomponen > 0 || in_array($r[flagKomponen], array(4)))){
						db($sql);
					}else{
						$nilaiKomponen = 0;
					}
					...
					if(in_array($r[dasarKomponen], array(3))){
						$_idKomponen = preg_match("/lembur/i", $r[namaKomponen]) ? 61 : $idKomponen;
						$nilaiKomponen = getField("select nilaiProses from ".$detailProses." where idPegawai='".$idPegawai."' and idKomponen='".$_idKomponen."' and flagProses > 0");
					}					
					...
					if($cnt == count($arrData)){
						...
						$sql="update dta_komponen t1 join ".$detailProses." t2 on (t1.idKomponen=t2.idKomponen) set t2.nilaiProses='".setAngka($defPph)."' where t1.idKomponen in ('3','8','173','176') and t2.idPegawai='".$idPegawai."'";
						db($sql);
						*/
	...
	function approval(){
		...
		repField();
		$inp[approveTime] = setTanggal($inp[approveDate])." ".date("H:i:s");
		$sql="update pay_proses set statusProses = '$inp[statusProses]', keteranganProses = '$inp[keteranganProses]', approveBy = '$inp[approveBy]', approveTime = '$inp[approveTime]' where bulanProses = '$par[bulanProses]' AND tahunProses = '$par[tahunProses]' and idLokasi='".$par[idLokasi]."' and idJenis='".$par[idJenis]."'";
		db($sql);
	...
	function formApprove(){
		...
		$sql="select * from pay_proses where bulanProses='$par[bulanProses]' AND tahunProses = '$par[tahunProses]' and idLokasi='".$par[idLokasi]."' and idJenis='".$par[idJenis]."'";
		$res=db($sql);
		$r=mysql_fetch_array($res);	

		$false = $r[statusProses] == "f" ? "checked=\"checked\"" : "";
		$true = empty($false) ? "checked=\"checked\"" : "";

		$r[approveBy] = empty($r[approveBy]) ? $cUsername : $r[approveBy];
		list($approveDate, $approveTime) = explode(" ", $r[approveTime]);
		if(in_array($approveDate, array("","0000-00-00"))) $approveDate = date("Y-m-d");
		if(in_array($approveTime, array("","00:00:00"))) $approveTime = date("H:i:s");
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">
			<form id=\"form\" enctype=\"multipart/form-data\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\">	
				...
				<p>           
					...
					<div class=\"field\">           
						<input type=\"hidden\" id=\"inp[approveBy]\" name=\"inp[approveBy]\"  value=\"$r[approveBy]\" readonly/>
						<input type=\"text\" id=\"inp[namaUser]\" name=\"inp[namaUser]\"  value=\"".getField("select namaUser from app_user where username='".$r[approveBy]."'")."\" style=\"width:300px;\" readonly/>
					</div>          
				</p>
				<p>
					<label class=\"l-input-small\">Tanggal & Waktu</label>
					<div class=\"fieldC\">
						<input type=\"hidden\" id=\"tp\" name=\"inp[approveDate]\" size=\"10\" maxlength=\"10\" value=\"" . getTanggal($approveDate) . "\" />
						<input type=\"text\" id=\"inp[approveTime]\" name=\"inp[approveTime]\"  value=\"".getTanggal($approveDate,"t")." ".$approveTime."\" class=\"mediuminput\" style=\"width:300px;\" readonly/>
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Status</label>
					<div class=\"fradio\">
						<input type=\"radio\" id=\"true\" name=\"inp[statusProses]\" value=\"t\" $true /> <span class=\"sradio\">Disetujui</span>
						<input type=\"radio\" id=\"false\" name=\"inp[statusProses]\" value=\"f\" $false /> <span class=\"sradio\">Pending</span>
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Keterangan</label>
					<span class=\"fieldB\">
						<textarea style=\"width: 300px;\" id=\"inp[keteranganProses]\" name=\"inp[keteranganProses]\">$r[keteranganProses]</textarea>
					</span>
				</p>
			</form>	";
		return $text;
	...
	function detailApprove(){
		global $s,$inp,$par,$menuAccess,$fFile,$cUsername;	
		$sql="select * from pay_proses where bulanProses='$par[bulanProses]' AND tahunProses = '$par[tahunProses]' and idJenis='".$par[idJenis]."'";
		$res=db($sql);
		$r=mysql_fetch_array($res);	

		$statusApprove = $r[statusProses] == "t" ? "Disetujui" : "Pending";
		...
		<div id=\"contentwrapper\" class=\"contentwrapper\">
			<form id=\"form\" enctype=\"multipart/form-data\" name=\"form\" method=\"post\" class=\"stdform\" >	
				<p style=\"position:absolute;right:5px;top:5px;\">
				...
				<p>
					...
					<span class=\"field\">
						<input type=\"hidden\" id=\"tp\" name=\"inp[approveDate]\" size=\"10\" maxlength=\"10\" value=\"" . getTanggal($approveDate) . "\" />
						".getTanggal($approveDate,"t")." ".$approveTime."&nbsp;
					</span>
				</p>
				<p>
					<label class=\"l-input-small\">Status</label>
	...
	function lihat(){
		...
		if(empty($par[idLokasi])) $par[idLokasi] = getField("select kodeData from mst_data where statusData='t' and kodeCategory='X03' order by urutanData limit 1");
			...
			\">FILTER</legend>
				...
				<table width=\"100%\">
					...
					<p>
						...
						<div class=\"field\">
							".comboData("select * from mst_data where statusData='t' and kodeCategory='X03' order by urutanData","kodeData","namaData","par[idLokasi]","",$par[idLokasi],"onchange=\"document.getElementById('form').submit();\"", "90%", "chosen-select")."
						</div>
					</p>
					<p>
					...
					<p>
						...
						<span class=\"field\">".getField("select count(*) from mst_data where statusData='t' and kodeCategory='X03'")." / ".getField("select count(*) from (select idLokasi from pay_proses where tahunProses='".$par[tahunProses]."' and bulanProses='".$bulanProses."' group by 1) as t")."&nbsp;</span>
					</p>
					<p>
						<label class=\"l-input-small\">Proses</label>
		...
		if(is_array($arrDetail)){
			...
			*/
			$arrNilai=arrayQuery("select periodeProses, sum(case when tipeKomponen='t' then nilaiProses else nilaiProses * -1 end) as jumlahProses from (".implode(" union ", $arrDetail).") as t1 join dta_komponen t2 join emp_phist t3 join emp t4 on (t1.idKomponen=t2.idKomponen and t1.idPegawai=t3.parent_id and t3.parent_id=t4.id) where t3.status='1' AND t3.group_id is not null ".$filter." and t2.realisasiKomponen='t' group by 1");
			$arrJumlah=arrayQuery("select periodeProses, count(*) from (select periodeProses, idPegawai from (".implode(" union ", $arrDetail).") as t1 join dta_komponen t2 join emp_phist t3 join emp t4 on (t1.idKomponen=t2.idKomponen and t1.idPegawai=t3.parent_id and t3.parent_id=t4.id) where t3.status='1' AND t3.group_id is not null ".$filter." and t2.realisasiKomponen='t' group by 1, 2) as t group by 1");
		}		
		...
		for($i=1; $i<=12; $i++){
			...
			$statusProses = $r[statusProses] == "t"? "<img src=\"styles/images/t.png\" title=\"Disetujui\">" : "<img src=\"styles/images/p.png\" title=\"Belum Diproses\">";
			$approveLink = (isset($menuAccess[$s]["apprlv1"]) && in_array($r[approveBy], array("", $cUsername))) ?
			"<a href=\"#\" onclick=\"openBox('popup.php?par[mode]=app&par[bulanProses]=$r[bulanProses]".getPar($par,"mode, bulanProses")."',600,350);\">$statusProses</a>":
			"<a href=\"#\" onclick=\"openBox('popup.php?par[mode]=sts&par[bulanProses]=$r[bulanProses]".getPar($par,"mode, bulanProses")."',600,350);\">".$statusProses."</a>";
			
			if($r[progesProses] != 100)
				$approveLink = $statusProses;
			...
			$text.="<tr>
					...
					<td align=\"center\">".$approveLink."</td>
					<td>$r[namaUser]</td>
					</tr>";
		}
		$text.="</tbody>
		...
		$text.="<div class=\"pageheader\">
			...
			\">FILTER</legend>
				...
				<p>
					<label class=\"l-input-small\">Lokasi Proses</label>
					<div class=\"field\">
						".comboData("select * from mst_data where statusData='t' and kodeCategory='X03' order by urutanData","kodeData","namaData","par[idLokasi]","",$par[idLokasi],"onchange=\"document.getElementById('form').submit();\"", "325px", "chosen-select")."
					</div>
				</p>
				<p>
					<label class=\"l-input-small\">Kategori Gaji</label>
			...
			$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");
			//$filter = "where t1.status='".$status."' AND t2.group_id is not null";		
			$filter = "where t2.group_id is not null";		
	...
	function data(){
		...
		$res=db($sql);
		$r=mysql_fetch_array($res);
		$iuranPensiun = $r[cat_date] < "2009-01-01" ? "PPMP" : "PPIP";
	...
	function getContent($par){
		...
		switch($par[mode]){
			...
			break;
			case "app":
				if(isset($menuAccess[$s]["apprlv1"])) $text = empty($_submit) ? formApprove() : approval(); else $text = lihat();
			break;
			case "set":
