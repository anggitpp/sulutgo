<?php
	...
	function setData(){
		...
		if(!empty($idPegawai)){								
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.idPegawai='".$idPegawai."'";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){									
				$nilaiInsentif+=$r[nilaiAngsuran];
			}
	...
	function form(){
		...
		$nilaiInsentif = array();		
		$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t'";
		$res=db($sql);			
		while($r=mysql_fetch_array($res)){	
			$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
			$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiAngsuran];
		}		
	...
	function lihat(){
		...
		for($i=1; $i<=12; $i++){
			...
			$pegawaiInsentif = array();
			$nilaiInsentif = array();
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$mulaiPeriode."' and '".$selesaiPeriode."' and t1.pembayaranPinjaman='t' AND t2.group_id IN ( $areaCheck ) $filter";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
				$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
				$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiAngsuran];							
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t3.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t'".$filter;
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
	...
	function data(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_pinjaman t1 join ess_angsuran t3 join dta_pegawai t2 on (t1.idPinjaman=t3.idPinjaman and t1.idPegawai=t2.id) where date(t1.tanggalAngsuran) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.pembayaranPinjaman='t' and t1.idPegawai='".$par[idPegawai]."'";
			$res=db($sql);			
			$no=1;
			while($r=mysql_fetch_array($res)){									
				$text.="<tr>
							...
							<span style=\"float:right; text-align:right; width:125px;\">".getAngka($r[nilaiAngsuran])."</span>
