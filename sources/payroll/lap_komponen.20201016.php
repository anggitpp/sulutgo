<?php
	...
	function lihat(){
		...
		$arrLokasi = arrayQuery("select kodeData, namaData from mst_data where statusData='t' and kodeCategory='".$arrParameter[7]."' AND kodeData IN ( $areaCheck ) ".$filter." order by urutanData");
		...
		if(getField("select idProses from pay_proses where tahunProses='".$par[tahunProses]."' and bulanProses='".intval($par[bulanProses])."'")){
			$filter = " AND t2.group_id IN ( $areaCheck ) and t2.status = '535'";
			...
			$arrNilai = arrayQuery("select t2.location, t0.kodeKomponen, sum(t1.nilaiProses) from dta_komponen t0 join ".$detailProses." t1 join dta_pegawai t2 on (t0.idKomponen=t1.idKomponen and t1.idPegawai=t2.id) where t0.tipeKomponen='$par[tipeKomponen]' ".$filter." group by 1,2");
		}
