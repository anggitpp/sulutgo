<?php
	...
	function gPlafon(){
		...
		$sql="select * from pay_kacamata where idKategori='".$par[rmb_cat]."' and idTipe='".$par[rmb_type]."'";
		$res=db($sql);
		$r=mysql_fetch_array($res);
	...
	function paymentAll(){
		...
		$filter = "where rmb_jenis='m' AND t2.group_id IN ( $areaCheck )";		
		if(!empty($par[bulan]))
		...
		$text.="<div class=\"centercontent contentpopup\">
				...
				<div id=\"general\" class=\"subcontent\">										
					<p>
						...
						<div class=\"field\">
							<input type=\"text\" id=\"apr_date\" name=\"inp[apr_date]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal(date('Y-m-d'))."\" class=\"vsmallinput hasDatePicker\"/>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Nama</label>
						<div class=\"field\">								
							<input type=\"text\" id=\"inp[apr_by]\" name=\"inp[apr_by]\"  value=\"".getField("select namaUser from ".$db['setting'].".app_user where username='$cUsername' ")."\" class=\"mediuminput\" style=\"width:300px;\" readonly=\"readonly\" />
						</div>
					</p>
					<p>
	...
	function lihat(){
		...
		$filter = "where rmb_jenis='m' and t1.status='1' AND t2.group_id IN ( $areaCheck )";	
		if(!empty($par[bulan]))
			$filter.= " and month(t1.rmb_date)='$par[bulan]'";
		if(!empty($par[tahun]))
			$filter.= " and year(t1.rmb_date)='$par[tahun]'";				
	...
	function pdf(){
		...
		$pdf->Row(array("\tf", "NAMA\tf",":\tf",$r[name]."\tf","\tf","KATEGORI\tf",":\tf",getField("select namaData from mst_data where kodeData='".$r[rmb_cat]."'")." - ".getField("select namaData from mst_data where kodeData='".$r[rmb_type]."'")."\tf", "\tf"));
