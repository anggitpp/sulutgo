<?php
	...
	function setTable(){
		...
		$kodeKomponen = getField("select nilaiParameter from pay_parameter where idParameter='6'");
	...
	function setData(){
		...
		if(!empty($idPegawai)){									
			...
			$sql="select t1.*, t2.id as idPegawai from emp_rmb t1 join emp t2 on (t1.parent_id=t2.id) where year(t1.rmb_date)='$par[tahunInsentif]' and month(t1.rmb_date)='$par[bulanInsentif]' and t1.parent_id='".$idPegawai."'";
			$res=db($sql);
			$nilaiKomponen=0;
			while($r=mysql_fetch_array($res)){			
				$nilaiKomponen+= empty($r_[idPengali]) ? 
				$r[rmb_val] * $r_[nilaiKomponen] * $r_[maxKomponen]:
				$r[rmb_val] * $r_[nilaiKomponen] * $r_[maxKomponen] * $dasarKomponen["$r[idPegawai]"]["$r_[idPengali]"];
			}
			
			if($nilaiKomponen > 0){
				$sql="insert into ".$detailInsentif." (idProses, idPegawai, idKomponen, nilaiProses, flagProses, createBy, createTime) values ('$idProses', '$idPegawai', '$idKomponen', '".setAngka($nilaiKomponen)."', '5', '$cUsername', '".date('Y-m-d H:i:s')."')";
				db($sql);
	...
	function form(){
		...
		$sql="select t1.*, t2.id as idPegawai from emp_rmb t1 join emp t2 on (t1.parent_id=t2.id) where year(t1.rmb_date)='$par[tahunInsentif]' and month(t1.rmb_date)='$par[bulanInsentif]'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
		...
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">Insentif Reimbers</h1>
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<div id=\"prosesBtn\" align=\"center\">						
						<input type=\"button\" class=\"cancel radius2\" value=\"Proses Reimbers\" onclick=\"setInsentif('".getPar($par,"mode")."');\"/>
					</div>
