<?php
	...
	function setTable(){
		...
		$sql="delete from ".$detailInsentif." where flagProses='3'";
		db($sql);
		...
		$kodeKomponen = getField("select nilaiParameter from pay_parameter where idParameter='4'");
	...
	function setData(){
		...
		if(!empty($idPegawai)){									
			...
			$sql="select * from att_dinas t1 join emp t2 on (t1.idPegawai=t2.id) where year(t1.mulaiDinas)='$par[tahunInsentif]' and month(t1.mulaiDinas)='$par[bulanInsentif]' and t1.idPegawai='".$idPegawai."' and t1.persetujuanDinas='t' and t1.sdmDinas='t'";
			$res=db($sql);
			$nilaiKomponen=0;
			while($r=mysql_fetch_array($res)){			
			...
			if($nilaiKomponen > 0){
				$sql="insert into ".$detailInsentif." (idProses, idPegawai, idKomponen, nilaiProses, flagProses, createBy, createTime) values ('$idProses', '$idPegawai', '$idKomponen', '".setAngka($nilaiKomponen)."', '3', '$cUsername', '".date('Y-m-d H:i:s')."')";
				db($sql);
	...
	function form(){
		...
		$sql="select * from att_dinas t1 join emp t2 on (t1.idPegawai=t2.id) where year(t1.mulaiDinas)='$par[tahunInsentif]' and month(t1.mulaiDinas)='$par[bulanInsentif]' and t1.persetujuanDinas='t' and t1.sdmDinas='t'";
		$res=db($sql);
		while($r=mysql_fetch_array($res)){			
		...
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">Insentif Dinas</h1>
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<div id=\"prosesBtn\" align=\"center\">						
						<input type=\"button\" class=\"cancel radius2\" value=\"Proses Dinas\" onclick=\"setInsentif('".getPar($par,"mode")."');\"/>
					</div>
