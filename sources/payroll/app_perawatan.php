<?php
	...
	function gPlafon(){
		...
		$sql_="select * from emp where id='".$par[parent_id]."'";
		$res_=db($sql_);
		$r_=mysql_fetch_array($res_);
		
		$sql__="select * from emp_phist where parent_id='".$par[parent_id]."' and status='1'";
		$res__=db($sql__);
		$r__=mysql_fetch_array($res__);
		
		$sql="select * from pay_pengobatan where idPangkat='".$r__[rank]."' and idGrade='".$r__[grade]."'";
		$res=db($sql);
		$r=mysql_fetch_array($res);
		
		$r[nilaiKesehatan] = getField("select kodeData from mst_data where kodeData='".$r_[marital]."' and lower(namaData) like '%tk%'") ? $r[lajangPengobatan] : $r[keluargaPengobatan];
		
		$data[rmb_limit] = getAngka($r[nilaiKesehatan]);
		
		$rmb_val = getField("select sum(rmb_val) from emp_rmb where parent_id='".$par[parent_id]."' and year(rmb_date)='".$rmb_year."' and status='1' and rmb_jenis='p'");
				
		$data[rmb_balance] = getAngka($r[nilaiKesehatan] - $rmb_val);
	...
	function paymentAll(){
		...
		$sql="update emp_rmb t1 left join dta_pegawai t2 on (t1.parent_id=t2.id) set t1.pay_remark='$inp[pay_remark]', t1.pay_status='$inp[pay_status]', t1.pay_by='$cUsername', t1.sdm_date='".date('Y-m-d')."' $filter";
		db($sql);		
	...
	function approveSdm(){
		...
		$sql="update emp_rmb t1 left join dta_pegawai t2 on (t1.parent_id=t2.id) set t1.sdm_remark='$inp[sdm_remark]', t1.sdm_status='$inp[sdm_status]', t1.sdm_by='$cUsername', t1.sdm_date='".date('Y-m-d')."' $filter";
		db($sql);		
	...
	function approveAll(){
		...
		$sql="update emp_rmb t1 left join dta_pegawai t2 on (t1.parent_id=t2.id) set t1.apr_remark='$inp[apr_remark]', t1.status='$inp[status]', t1.apr_by='$cUsername', t1.apr_date='".date('Y-m-d')."' $filter";
		db($sql);		
	...
	function payment(){
		...
		$sql="update emp_rmb set parent_id='$inp[parent_id]', rmb_no='$inp[rmb_no]', rmb_date='".setTanggal($inp[rmb_date])."', rmb_cat='$inp[rmb_cat]', rmb_type='$inp[rmb_type]', rmb_val='".setAngka($inp[rmb_val])."', remark='$inp[remark]', pay_remark='$inp[pay_remark]', pay_status='$inp[pay_status]', pay_by='$cUsername', pay_date='".setTanggal($inp[pay_date])."' where id='$par[id]'";
		db($sql);
	...
	function sdm(){
		...
		$sql="update emp_rmb set parent_id='$inp[parent_id]', rmb_no='$inp[rmb_no]', rmb_date='".setTanggal($inp[rmb_date])."', rmb_cat='$inp[rmb_cat]', rmb_type='$inp[rmb_type]', rmb_val='".setAngka($inp[rmb_val])."', remark='$inp[remark]', sdm_remark='$inp[sdm_remark]', sdm_status='$inp[sdm_status]', sdm_by='$cUsername', sdm_date='".date('Y-m-d')."' where id='$par[id]'";
		db($sql);
	...
	function tambah(){
		...
		$sql="insert into emp_rmb (id, parent_id, rmb_jenis, rmb_no, rmb_date, rmb_cat, rmb_type, rmb_val, remark, status, cre_by, cre_date) values ('$id', '$inp[parent_id]', 'p', '$inp[rmb_no]', '".setTanggal($inp[rmb_date])."', '$inp[rmb_cat]', '$inp[rmb_type]', '".setAngka($inp[rmb_val])."', '$inp[remark]', '0', '$cUsername', '".date('Y-m-d H:i:s')."')";
		db($sql);
	...
	function formPay(){
		...
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">All SDM</h1>
					".getBread(ucwords($par[mode]." sdm"))."
				</div>
				...
				<div id=\"general\" class=\"subcontent\">										
					<p>
						...
						<div class=\"fradio\">
							<input type=\"radio\" id=\"true\" name=\"inp[pay_status]\" value=\"1\" checked=\"checked\" /> <span class=\"sradio\">Disetujui</span>
							<input type=\"radio\" id=\"false\" name=\"inp[pay_status]\" value=\"2\" > <span class=\"sradio\">Ditolak</span>
							<input type=\"radio\" id=\"revisi\" name=\"inp[pay_status]\" value=\"3\" /> <span class=\"sradio\">Diperbaiki</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
						<div class=\"field\">
							<textarea id=\"inp[pay_remark]\" name=\"inp[pay_remark]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:300px;\">$r[pay_remark]</textarea>
						</div>
					</p>					
					<p>
		...
		$text.="<div class=\"centercontent contentpopup\">
				...
				<div id=\"general\" class=\"subcontent\">										
					<p>
						<label class=\"l-input-small\">Status</label>
						<div class=\"fradio\">
							<input type=\"radio\" id=\"true\" name=\"inp[sdm_status]\" value=\"1\" checked=\"checked\" /> <span class=\"sradio\">Disetujui</span>
							<input type=\"radio\" id=\"false\" name=\"inp[sdm_status]\" value=\"2\" > <span class=\"sradio\">Ditolak</span>
							<input type=\"radio\" id=\"revisi\" name=\"inp[sdm_status]\" value=\"3\" /> <span class=\"sradio\">Diperbaiki</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
						<div class=\"field\">
							<textarea id=\"inp[sdm_remark]\" name=\"inp[sdm_remark]\" rows=\"3\" cols=\"50\" class=\"longinput\" style=\"height:50px; width:300px;\">$r[sdm_remark]</textarea>
						</div>
					</p>					
					<p>
						<input type=\"submit\" class=\"submit radius2\" name=\"btnSimpan\" value=\"Save\"/>
	...
	function formAll(){
		...
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">All Atasan</h1>
					".getBread(ucwords($par[mode]." atasan"))."
				</div>
				<div id=\"contentwrapper\" class=\"contentwrapper\">
				<form id=\"form\" name=\"form\" method=\"post\" class=\"stdform\" action=\"?_submit=1".getPar($par)."\" enctype=\"multipart/form-data\">	
				<div id=\"general\" class=\"subcontent\">										
					<p>
						<label class=\"l-input-small\">Status</label>
						<div class=\"fradio\">
							<input type=\"radio\" id=\"true\" name=\"inp[status]\" value=\"1\" checked=\"checked\" /> <span class=\"sradio\">Disetujui</span>
							<input type=\"radio\" id=\"false\" name=\"inp[status]\" value=\"2\" > <span class=\"sradio\">Ditolak</span>
							<input type=\"radio\" id=\"revisi\" name=\"inp[status]\" value=\"3\" /> <span class=\"sradio\">Diperbaiki</span>
						</div>
					</p>
					<p>
						<label class=\"l-input-small\">Keterangan</label>
	...
	function form(){
		...
		$sTrue = $r[sdm_status] == "1" ? "checked=\"checked\"" : "";
		$sFalse = $r[sdm_status] == "2" ? "checked=\"checked\"" : "";
		$sRevisi = $r[sdm_status] == "3" ? "checked=\"checked\"" : "";
		
		$pTrue = $r[pay_status] == "1" ? "checked=\"checked\"" : "";
		$pFalse = $r[pay_status] == "2" ? "checked=\"checked\"" : "";
		$pRevisi = $r[pay_status] == "3" ? "checked=\"checked\"" : "";
		...
		$r[rmb_balance] = $r[rmb_limit] - getField("select sum(rmb_val) from emp_rmb where parent_id='".$r[parent_id]."' and year(rmb_date)='".$rmb_year."' and rmb_date<='".$r[rmb_date]."' and status='1' and rmb_jenis='p' and id!='".$par[id]."'");
		...
		$text.="<div class=\"pageheader\">
				...
				<div id=\"general\" style=\"margin-top:20px;\">
					...
					<td width=\"55%\">
						<p>
							...
							<div class=\"field\">
								<input type=\"text\" id=\"rmb_date\" name=\"inp[rmb_date]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[rmb_date])."\" class=\"vsmallinput hasDatePicker\" onchange=\"getNomor('".getPar($par,"mode")."'); getPlafon('".getPar($par,"mode,parent_id,rmb_date,rmb_cat")."');\"/>
							</div>
						</p>
						<p>
					...
					<div class=\"widgetbox\">
						<div class=\"title\" style=\"margin-top:10px; margin-bottom:0px;\"><h3>DATA KLAIM PERAWATAN</h3></div>
					</div>
					<table width=\"100%\">
					<tr>
					<td width=\"45%\" style=\"vertical-align:top;\">
						<p>
							...
							<div class=\"field\">
								".comboData("select * from mst_data where statusData='t' and kodeCategory='".$arrParameter[22]."' order by urutanData","kodeData","namaData","inp[rmb_cat]"," ",$r[rmb_cat],"", "310px")."
							</div>
						</p>						
						<p>
			...
			if($par[mode] == "sdm"){
			$sdm_by = empty($r[sdm_by]) ? $cUsername : $r[sdm_by];
			list($r[sdm_date]) = explode(" ",$r[sdm_date]);
			if(empty($r[sdm_date]) || $r[sdm_date] == "0000-00-00") $r[sdm_date] = date('Y-m-d');
			$text.="<div class=\"widgetbox\">
						<div class=\"title\" style=\"margin-top:10px; margin-bottom:0px;\"><h3>APPROVAL</h3></div>
					</div>			
					<table width=\"100%\">
					...
					<td width=\"45%\">
						<p>
							<label class=\"l-input-small\">Tanggal</label>
							<div class=\"field\">
								<input type=\"text\" id=\"sdm_date\" name=\"inp[sdm_date]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[sdm_date])."\" class=\"vsmallinput hasDatePicker\"/>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Nama</label>
							<div class=\"field\">								
								<input type=\"text\" id=\"inp[sdm_by]\" name=\"inp[sdm_by]\"  value=\"".getField("select namaUser from ".$db['setting'].".app_user where username='$sdm_by' ")."\" class=\"mediuminput\" style=\"width:300px;\" readonly=\"readonly\" />
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Status</label>
							<div class=\"fradio\">
								<input type=\"radio\" id=\"true\" name=\"inp[sdm_status]\" value=\"1\" $true /> <span class=\"sradio\">Disetujui</span>
								<input type=\"radio\" id=\"false\" name=\"inp[sdm_status]\" value=\"2\" $false /> <span class=\"sradio\">Ditolak</span>
								<input type=\"radio\" id=\"revisi\" name=\"inp[sdm_status]\" value=\"3\" $revisi /> <span class=\"sradio\">Diperbaiki</span>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Keterangan</label>
			...
			if($par[mode] == "pay"){
				$pay_by = empty($r[pay_by]) ? $cUsername : $r[pay_by];
				if(empty($r[pay_date])) $r[pay_date] = date('Y-m-d');
				...
				$text.="<div class=\"widgetbox\">
					...
					<td width=\"45%\">
						<p>
							<label class=\"l-input-small\">Tanggal</label>
							<div class=\"field\">
								<input type=\"text\" id=\"pay_date\" name=\"inp[pay_date]\" size=\"10\" maxlength=\"10\" value=\"".getTanggal($r[pay_date])."\" class=\"vsmallinput hasDatePicker\"/>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Nama</label>
							<div class=\"field\">								
								<input type=\"text\" id=\"inp[pay_by]\" name=\"inp[pay_by]\"  value=\"".getField("select namaUser from ".$db['setting'].".app_user where username='$pay_by' ")."\" class=\"mediuminput\" style=\"width:300px;\" readonly=\"readonly\" />
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Status</label>
							<div class=\"fradio\">
								<input type=\"radio\" id=\"true\" name=\"inp[pay_status]\" value=\"1\" $pTrue /> <span class=\"sradio\">Disetujui</span>
								<input type=\"radio\" id=\"false\" name=\"inp[pay_status]\" value=\"2\" $pFalse /> <span class=\"sradio\">Ditolak</span>
								<input type=\"radio\" id=\"revisi\" name=\"inp[pay_status]\" value=\"3\" $pRevisi /> <span class=\"sradio\">Diperbaiki</span>
							</div>
						</p>
						<p>
							<label class=\"l-input-small\">Keterangan</label>
	...
	$text.="<div class=\"pageheader\">
			...
			<div style=\"position:absolute; right:0; top:0; margin-top:10px; margin-right:20px;\">
				Lokasi Kerja : ".comboData("select * from mst_data where statusData='t' and kodeCategory='".$arrParameter[7]."' order by urutanData","kodeData","namaData","par[lokasi]","All",$par[lokasi],"onchange=\"document.getElementById('form').submit();\"", "310px")." <span style=\"margin-left:30px;\">Periode :</span> ".comboMonth("par[bulan]", $par[bulan], "onchange=\"document.getElementById('form').submit();\"", "", "t")." ".comboYear("par[tahun]", $par[tahun], "", "onchange=\"document.getElementById('form').submit();\"", "", "t")."
			</div>
			...
			<div id=\"pos_r\">";
		if(isset($menuAccess[$s]["apprlv1"])) $text.="<a href=\"#\" class=\"btn btn1 btn_edit\" onclick=\"openBox('popup.php?par[mode]=all".getPar($par,"mode,id")."',725,400);\"><span>All Atasan</span></a> ";
		if(isset($menuAccess[$s]["apprlv2"])) $text.="<a href=\"#\" class=\"btn btn1 btn_edit\" onclick=\"openBox('popup.php?par[mode]=allSdm".getPar($par,"mode,id")."',725,400);\"><span>All SDM</span></a> ";
		if(isset($menuAccess[$s]["apprlv3"])) $text.="<a href=\"#\" class=\"btn btn1 btn_edit\" onclick=\"openBox('popup.php?par[mode]=allPay".getPar($par,"mode,id")."',725,400);\"><span>All Bayar</span></a> ";
		$text.="</div>
			</form>
			<br clear=\"all\" />
			...
			<thead>
				<tr>
					...
					<th rowspan=\"2\" style=\"width:100px;\">NPP</th>
					<th rowspan=\"2\" style=\"width:100px;\">Nomor</th>
					<th rowspan=\"2\" width=\"75\">Tanggal</th>
					<th rowspan=\"2\" style=\"min-width:150px;\">Kategori/Tipe</th>
					<th rowspan=\"2\" width=\"100\">Nilai</th>
					<th colspan=\"2\" width=\"100\">Approval</th>
					<th colspan=\"2\" width=\"100\">Kasir</th>
		...
		$filter = "where rmb_jenis='p' and t1.status='1'";
		if(!empty($par[bulan]))
			$filter.= " and month(t1.rmb_date)='$par[bulan]'";
		if(!empty($par[tahun]))
			$filter.= " and year(t1.rmb_date)='$par[tahun]'";				
		...
		while($r=mysql_fetch_array($res)){			
			...
			$sdmLink = (isset($menuAccess[$s]["apprlv2"]) && $r[status] == "1") ? "?par[mode]=sdm&par[id]=$r[id]".getPar($par,"mode,id") : "#";
			$payLink = (isset($menuAccess[$s]["apprlv3"]) && $r[status] == "1" && $r[sdm_status] == "1") ? "?par[mode]=pay&par[id]=$r[id]".getPar($par,"mode,id") : "#";
			...
			$text.="<tr>
					...
					<td align=\"center\"><a href=\"".$sdmLink."\">$sdm_status</a></td>
					<td align=\"center\"><a href=\"".$payLink."\">$pay_status</a></td>
					<td align=\"center\">";
			if($r[pay_status] == 1)
	...
	function pdf(){
		...
		$pdf->Cell(100,7,'BANTUAN PERAWATAN',0,0,'L');		
		$pdf->Ln();		
	...
	function getContent($par){
		...
		switch($par[mode]){
			...
			break;
			case "all":
				if(isset($menuAccess[$s]["apprlv1"])) $text = empty($_submit) ? formAll() : approveAll(); else $text = lihat();
			break;
			case "sdm":
				if(isset($menuAccess[$s]["apprlv2"])) $text = empty($_submit) ? form() : sdm(); else $text = lihat();
			break;
			case "allSdm":
				if(isset($menuAccess[$s]["apprlv2"])) $text = empty($_submit) ? formSdm() : approveSdm(); else $text = lihat();
			break;
			case "pay":
				if(isset($menuAccess[$s]["apprlv3"])) $text = empty($_submit) ? form() : payment(); else $text = lihat();
			break;
			case "allPay":
				if(isset($menuAccess[$s]["apprlv3"])) $text = empty($_submit) ? formPay() : paymentAll(); else $text = lihat();
			break;
			default:
				$text = lihat();
			break;
