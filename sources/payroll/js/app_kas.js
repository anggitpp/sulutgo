...
function getNomor(getPar){
	tanggalKas=document.getElementById("tanggalKas");
	var xmlHttp = getXMLHttp();
	xmlHttp.onreadystatechange = function(){	
		if(xmlHttp.readyState == 4 && xmlHttp.status==200){			
			if(xmlHttp.responseText){
				document.getElementById("inp[nomorKas]").value = xmlHttp.responseText;
			}
		}
	}
	xmlHttp.open("GET", "ajax.php?par[mode]=no&tanggalKas=" + tanggalKas.value + getPar, true);
	xmlHttp.send(null);
	return false;
}
