<?php
	...
	function setTable(){
		...
		$sql="delete from ".$detailInsentif." where flagProses='7'";
		db($sql);
		...
		$kodeKomponen = getField("select nilaiParameter from pay_parameter where idParameter='8'");
		...
		$sql="insert into ".$detailTemp." (idInsentif, idPegawai, createBy, createTime) select '".$idInsentif."' as idInsentif, idPegawai, '$cUsername' as createBy, '".date('Y-m-d H:i:s')."' as createTime from ess_duka where date(sdmTime) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and sdmDuka='t' group by idPegawai order by idPegawai";		
		db($sql);
	}
	...
	function setData(){
		...
		if(!empty($idPegawai)){								
			...
			$sql="select * from ess_duka t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where date(t1.sdmTime) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.sdmDuka='t' and t1.idPegawai='".$idPegawai."'";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){									
				$nilaiInsentif+=$r[nilaiDuka];
			}
			
			if($nilaiInsentif > 0){
				$sql="insert into ".$detailInsentif." (idProses, idPegawai, idKomponen, nilaiProses, flagProses, createBy, createTime) values ('$idProses', '$idPegawai', '$idKomponen', '".setAngka($nilaiInsentif)."', '7', '$cUsername', '".date('Y-m-d H:i:s')."')";
				db($sql);
	...
	function form(){
		...
		$nilaiInsentif = array();		
		$sql="select * from ess_duka t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where date(t1.sdmTime) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.sdmDuka='t'";
		$res=db($sql);			
		while($r=mysql_fetch_array($res)){	
			$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
			$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiDuka];
		}		
		
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">Insentif Duka</h1>
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<div id=\"prosesBtn\" align=\"center\">						
						<input type=\"button\" class=\"cancel radius2\" value=\"Proses Duka\" onclick=\"setInsentif('".getPar($par,"mode")."');\"/>
					</div>
	...
	function lihat(){
		...
		for($i=1; $i<=12; $i++){
			...
			$pegawaiInsentif = array();
			$nilaiInsentif = array();
			$sql="select * from ess_duka t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where date(t1.sdmTime) between '".$mulaiPeriode."' and '".$selesaiPeriode."' and t1.sdmDuka='t'";
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
				$pegawaiInsentif["$r[idPegawai]"]=$r[idPegawai];				
				$nilaiInsentif["$r[idPegawai]"]+=$r[nilaiDuka];							
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_duka t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where date(t1.sdmTime) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.sdmDuka='t'".$filter;
			$res=db($sql);			
			while($r=mysql_fetch_array($res)){	
	...
	function data(){
		...
		$text.="<div class=\"pageheader\">
			...
			$sql="select * from ess_duka t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where date(t1.sdmTime) between '".$par[mulaiPeriode]."' and '".$par[selesaiPeriode]."' and t1.sdmDuka='t' and t1.idPegawai='".$par[idPegawai]."'";
			$res=db($sql);			
			$no=1;
			while($r=mysql_fetch_array($res)){									
				$text.="<tr>
							...
							<span style=\"float:left;\">".$r[keteranganDuka]."</span>
							<span style=\"float:right; text-align:right; width:125px;\">".getAngka($r[nilaiDuka])."</span>
