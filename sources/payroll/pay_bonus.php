<?php
	...
	function setTable(){
		...
		$sql="delete from ".$detailInsentif." where flagProses='4'";
		db($sql);
		...
		$kodeKomponen = getField("select nilaiParameter from pay_parameter where idParameter='5'");
	...
	function setData(){
		...
		if(!empty($idPegawai)){									
			...
			$r_[nilaiKomponen] * $r_[maxKomponen] * $dasarKomponen[$idPegawai]["$r_[idPengali]"];			
			
			if($nilaiKomponen > 0){
				$sql="insert into ".$detailInsentif." (idProses, idPegawai, idKomponen, nilaiProses, flagProses, createBy, createTime) values ('$idProses', '$idPegawai', '$idKomponen', '".setAngka($nilaiKomponen)."', '4', '$cUsername', '".date('Y-m-d H:i:s')."')";
				db($sql);
	...
	function form(){
		...
		$text.="<div class=\"centercontent contentpopup\">
				<div class=\"pageheader\">
					<h1 class=\"pagetitle\">Insentif Bonus</h1>
				...
				<div id=\"general\" class=\"subcontent\">
					...
					<div id=\"prosesBtn\" align=\"center\">						
						<input type=\"button\" class=\"cancel radius2\" value=\"Proses Bonus\" onclick=\"setInsentif('".getPar($par,"mode")."');\"/>
					</div>
