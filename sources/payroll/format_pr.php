<?php
	...
	function setData(){
		...
		if($par[rowData] <= $highestRow){	
			...
			if(is_numeric(trim(strtolower($dta[0])))){
				...
				$namaUpload = trim($dta[3]);
				// $nilaiUpload = setAngka(trim($dta[4]));

				$keteranganUpload = "format pr";
				...
				// $i = 4;
				foreach($arrKomponen as $i => $idKomponen) {
					
					$nilaiKomponen = setAngka(trim($dta[$i]));
					if($nilaiKomponen != ''){
					if($idPegawai > 0){					
						$sql="delete from pay_upload where bulanUpload='".$bulanUpload."' and tahunUpload='".$tahunUpload."' and idPegawai='".$idPegawai."' and idKomponen='".$idKomponen."' ";
						db($sql);										
						$sql="insert into pay_upload (bulanUpload, tahunUpload, idPegawai, idKomponen, nilaiUpload, keteranganUpload, createBy, createTime) values ('".$bulanUpload."', '".$tahunUpload."', '".$idPegawai."', '".$idKomponen."', '".$nilaiKomponen."', '".$keteranganUpload."', '".$cUsername."', '".date('Y-m-d H:i:s')."')";
						db($sql);										
					}				

					$idUpload = getField("select idUpload from tmp_upload order by idUpload desc limit 1") + 1;
					$sql="insert into tmp_upload (idUpload, bulanUpload, tahunUpload, nikUpload, namaUpload, nilaiUpload, keteranganUpload, statusUpload, createBy, createTime) values ('".$idUpload."', '".$bulanUpload."', '".$tahunUpload."', '".$nikPegawai."', '".$namaUpload."', '".setAngka($nilaiKomponen)."', '".$keteranganUpload."', '".$statusUpload."', '".$cUsername."', '".date('Y-m-d H:i:s')."')";
					db($sql);
	...
	function formUpload(){
		...
		$text.="<div class=\"centercontent contentpopup\">
				...
				<div id=\"formInput\" class=\"subcontent\">
					...
					<p>
						<div style=\"float:right; margin-top:10px; margin-right:150px;\"><a href=\"download.php?d=fmtUploadPR\" class=\"detil\">* download template.xlsx</a></div>
	...
	function lihat(){
		...
		for($i=1; $i<=12; $i++){
			$r = $arrProses[$i];
			$jumlahPegawai = getField("select count(DISTINCT(idPegawai)) from pay_upload t1 join dta_pegawai t2 on (t1.idPegawai=t2.id) where t1.tahunUpload='$par[tahunUpload]' and t1.bulanUpload='".$i."' AND t2.location IN ( $areaCheck ) ".$filter." AND keteranganUpload = 'format pr'");			
	...
	function detail(){
		...
		$text.="<div class=\"pageheader\">
			...
			<br clear=\"all\" />
			";
			$arrKomponen = array('61','2','222','359','97','360','59','361','66','362','364','363','60','365','366','367','368','84','369','83','28','38','370','7');

			$arrNamaKomponen = arrayQuery("select idKomponen, namaKomponen from dta_komponen");
			$arrNilai = arrayQuery("select idPegawai, idKomponen, nilaiUpload from pay_upload where bulanUpload = '$par[bulanUpload]' AND tahunUpload = '$par[tahunUpload]'");
// echo "<pre>";
// 			print_r($arrNilai);
// echo "</pre>";
			$text.="
			...
			<thead>
				<tr>
					...
					foreach ($arrKomponen as $key => $value) {
						$text.="<th>".$arrNamaKomponen[$arrKomponen[$key]]."</th>";
					}
					$text.="
					<th>Total</th>
			...
			$status = getField("select kodeData from mst_data where statusData='t' and kodeCategory='".$arrParameter[6]."' order by urutanData limit 1");			
			$filter= " where t1.status='".$status."' and t2.bulanUpload='".$par[bulanUpload]."' and t2.tahunUpload='".$par[tahunUpload]."' and t2.keteranganUpload = 'format pr'";
			...
			$sql="select * from dta_pegawai t1 join pay_upload t2 on (t1.id=t2.idPegawai) $filter and t1.location IN ( $areaCheck ) order by t1.name";
			// echo $sql;
			$res=db($sql);
			while($r=mysql_fetch_array($res)){				
				$arrDetail[] = $r;
			...
			$sql_="select * from dta_pegawai t1 join pay_upload t2 on (t1.id=t2.idPegawai) $filter and t1.location IN ( $areaCheck ) group by t1.id order by t1.name";
			// echo $sql;
			...
			while($r_=mysql_fetch_array($res_)){			
				...
				<td>$r_[name]</td>
				<td>$r_[reg_no]</td>
				<td>".$arrMaster[getField("select bank_id from emp_bank where parent_id = '$r_[id]'")]."</td>
				<td>".getField("select account_no from emp_bank where parent_id = '$r_[id]'")."</td>
				";
				foreach ($arrKomponen as $key => $value) {
						$text.="<td>".getAngka($arrNilai[$r_[id]][$arrKomponen[$key]])."</td>";
						$totalNilaiPerPegawai[$r_[id]] += $arrNilai[$r_[id]][$arrKomponen[$key]];
					}
				$text.="
				<td>".getAngka($totalNilaiPerPegawai[$r_[id]])."</td>
				</tr>



				";	
				$totalSemuaNilai += $totalNilaiPerPegawai[$r_[id]];			
			}
			...
			<tfoot>
				<tr>
					...
					<td style=\"padding:5px 10px; text-align:right;\">".getAngka($totalSemuaNilai)."</td>
